...
{   
    ...
    else
    {
               ...
               startwaiting(document.frmOffice) ;
                var url="../../../../../ViewStaffStrengthDetails.con?command=nil&ID="+id;
                //alert(url);
...
function OfficeLevel()
{
    ...
    startwaiting(document.frmOffice) ;
    url="../../../../../ViewStaffStrengthDetails.con?command=OfficeLevel&OfficeId="+Office_Id;
    //alert(url);
    var req=getTransport();
    req.open("GET",url,true);         
    req.onreadystatechange=function()
   ...
   if(req.readyState==4)
    {
          ...
          {     
                ...
                if(flag=="failure")
                {
                    alert("There is No Template Values");
...
function callServer(command,param)
{
    ...
    if(command=="TableView")
    {
        ...
        startwaiting(document.frmOffice) ;
        url="../../../../../ViewStaffStrengthDetails.con?command=TableView&TemplateId="+Office_Id+"&finyear="+finyear;
        var req=getTransport();
        req.open("GET",url,true);
        req.onreadystatechange=function()
        {
   ...
   else if(command=="ServiceGroup")
    {
        ...
        startwaiting(document.frmOffice) ;
        url="../../../../../ViewStaffStrengthDetails.con?command=ServiceGroup&ServiceGroup="+ServiceGroup;
        var req=getTransport();
        req.open("GET",url,true);        
        req.onreadystatechange=function()
        {
                          ...
                          for(var i=0;i<value.length;i++)
                          {
                              var tmpoption=value.item(i);
                               ...
                               // var EmploymentStatusName=response.getElementsByTagName("employmentstatusname")[i].firstChild.nodeValue;
                                ...
                                //anc.href=url;
                                var txtedit=document.createTextNode(j);
                                anc.appendChild(txtedit);
                                cell1.appendChild(anc);
                                var hidden1=document.createElement("input");
                                hidden1.type="hidden";
...
{
        startwaiting(document.frmOffice) ;
       url="../../../../../ViewStaffStrengthDetails.con?command=ServiceGroup&ServiceGroup="+ServiceGroup+"&PostRank="+PostRank;
        var req=getTransport();
        req.open("GET",url,true);        
        req.onreadystatechange=function()
        {
