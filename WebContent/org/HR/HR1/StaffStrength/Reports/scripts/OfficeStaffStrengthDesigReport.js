...
function getTransport()
 ...
 function getOfficesByLevel()
    ...
    function loadOfficesByLevel(level)
    {
        ...
        var url="../../../../../../OfficeStaffStrengthDesigServlet.con?Command=level&level="+level;
 ...
 function getOfficesByType()
    {
        ...
        {
                var level=document.OfficeSpecificReport.cmbControllingLevel.options[document.OfficeSpecificReport.cmbControllingLevel.selectedIndex].value;
                var url="../../../../../../OfficeStaffStrengthDesigServlet.con?Command=own&level="+level+"&own="+own;
                var req=getTransport();
                req.open("GET",url,true);        
                req.onreadystatechange=function()
                {
...
if(v && items5)
{
        ...
        if(s=='')
        {
             if((document.OfficeSpecificReport.cmbFinancialYear.value=="") ||(document.OfficeSpecificReport.cmbFinancialYear.value.length<=0)|| (document.OfficeSpecificReport.cmbFinancialYear.value==0))
            {
            alert("Please Select Financial Year");
            document.OfficeSpecificReport.cmbFinancialYear.focus();
            return false;
        ...
        else
        {
            ...
            document.OfficeSpecificReport.action="../../../../../../OfficeStaffStrengthDesigServlet.con";
          document.OfficeSpecificReport.method="POST";
          document.OfficeSpecificReport.submit();
            return true;
        }
 	...
 	if (parseInt(cmn) <= 2)
        {
                ...
                document.OfficeSpecificReport.cmbFinancialYear.length=5;
                ...
                for (var i = 0 ; i < 2; i++) 
                {
                  ...
                  //document.OfficeSpecificReport.cmbFinancialYear.options[i].text=(cyr-2)+"-"+(cyr-1);
                  //document.OfficeSpecificReport.cmbFinancialYear.options[i].value=(cyr-2)+"-"+(cyr-1);
                  var id=(cyr-2)+"-"+(cyr-1);
                  var option=document.createElement("OPTION");
                  option.text=id;
                  option.value=id;
 	...
 	else 
 	{
           ...
           try
            ...
            if(cmn>=11)
            {
                ...
                for (var i = 0 ; i < 2; i++) 
                {
                  //document.OfficeSpecificReport.cmbFinancialYear.options[i].text=(cyr-1)+"-"+(cyr);
                  //document.OfficeSpecificReport.cmbFinancialYear.options[i].value=(cyr-1)+"-"+(cyr);
                  var id=(cyr-1)+"-"+(cyr);
                  var option=document.createElement("OPTION");
                  option.text=id;
                  option.value=id;
