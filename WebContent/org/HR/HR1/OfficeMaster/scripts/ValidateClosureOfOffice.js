   ...
   if(req.readyState==4)
    {
          ...
          {      
                ...
                if(flag=="success")
                {
                    ...
                    var lvl=response.getElementsByTagName("level")[0].firstChild.nodeValue;
                    document.frmClosure.txtOfficeName.value=name;
                    
//START - ADDED PART 
                    var accounts=response.getElementsByTagName("accounts")[0].firstChild.nodeValue;
                    if(accounts=="disable")
                    {
                        document.frmClosure.acc_unit_id.selectedIndex=0;        //ADDED.. To reset/empty 'Accounts Transfer to' combo before disabling it
                        document.getElementById("acc_unit_id").disabled=true;
                        accUnit=false;
                    }  
                    else
                    {
                        document.getElementById("acc_unit_id").disabled=false;
                        accUnit=false;
                    }
//END - ADDED PART  
                    ...
                    if(lvl=="Region")
                    document.frmClosure.hid_level.value='RN';
                    else if(lvl=="Circle")
                    document.frmClosure.hid_level.value='CL';
                    else if(lvl=="Division")
                    document.frmClosure.hid_level.value='DN';
                    else if(lvl=="Sub-Division")
                    document.frmClosure.hid_level.value='SD';
                ...
                else
                {
                    ...
                    document.frmClosue.submit.disabled=false;
...
function nullCheckMaster()
{
    ...
    /*if((document.frmClosure.cmbRecordStatus.value=="0") || (document.frmClosure.cmbRecordStatus.selectedIndex<=0))
      {
          alert("Please Select Record Status");
          document.frmClosure.cmbRecordStatus.focus();
          return false;
      }*/ 
        ...
        if(req.readyState==4)
        {
          ...
          {      
                ...
                if(flag=="success")
                {
                    ...
                    var acct_unit=response.getElementsByTagName("acct_unit_id")[0].firstChild.nodeValue;
                ...
                document.frmClosure.acc_unit_id.value=acct_unit;
...
function funclear()
{
    ...
    document.frmClosure.submit.disabled=false;
    document.frmClosure.acc_unit_id.disabled=false;    //ADDED.. To enable 'Accounts Tranfer to' combo on clicking 'Clear All' button
}
...
function checkLevel()
{
  //alert('inside checkLevel');
  var lv=document.frmClosure.hid_level.value;
  ...
  {    
    document.frmClosure.acc_unit_id.selectedIndex=0;        //ADDED.. To reset/empty 'Accounts Transfer to' combo before disabling it
    document.frmClosure.acc_unit_id.disabled=true;
    document.frmClosure.acc_unit_id.options[document.frmClosure.acc_unit_id.selectedIndex].value='0';
  }
  else if(accUnit)
  {
    document.frmClosure.acc_unit_id.disabled=false;
  }
}