...
<html>
  <head>
    ...
    <script type="text/javascript" src="../scripts/ReliModeScript.js"></script>
    <script type="text/javascript" src="<%=request.getContextPath()%>/org/Library/scripts/comJS.js"></script>

    <link href="../../../../../css/Sample3.css" rel="stylesheet" media="screen"/>
    
    <title>Master HRM Relieval Modes Details</title>
  </head>
  <body>
  <%
  ...
  %>
  <form name="RModeForm" id="RModeForm">
      <table cellspacing="2" cellpadding="3" border="1" width="75%"
             align="center">
        <tr>
          <td colspan="2" class="tdH">
            <div align="center">
              <strong>Relieval Modes Details</strong>
            </div></td>
        </tr>
        <tr>
          <td class="table">Relieval Mode Id</td>
          <td class="table">
           
                   <input type="TEXT" name="htxtRModeId" maxlength="3"
                   id="htxtRModeId"  readonly size="3"/>System Generated
          </td>
        </tr>
        <tr>
          <td class="table">Relieval Mode Desc</td>
          <td class="table">
            <input type="text" name="txtRModeDesc" size="40" maxlength="60"
                   id="txtRModeDesc"/>
          </td>
        </tr>
        <tr>
      ...
      <table id="Existing" cellspacing="2" cellpadding="3" border="1" width="75%"
        ...
        <tr class="tdH">
          ...
          try
          {
            ...
            String sql="select * from HRM_MST_RELIEVAL_MODES ORDER BY RELIEVAL_MODE_ID";
             results=statement.executeQuery(sql);
            try
            {
              while(results.next())
              {
                String strRModeId=results.getString("RELIEVAL_MODE_ID");
	        String strRModeDesc=results.getString("RELIEVAL_MODE_DESC");
                if(strRModeDesc==null)
                                       {
                                           strRModeDesc="Undefined Record Found";
                                           
                                       }
                                       else {
                                           strRModeDesc=strRModeDesc;
                                           
                                       }
                out.println("<tr id='" + strRModeId + "'>");   
                out.println("<td><a href=\"javascript:loadValuesFromTable('" + strRModeId + "')\">Edit</a></td>");
                out.println("<td>" + strRModeId + "</td>");
	 out.println("<td>" + strRModeDesc + "</td></tr>");
