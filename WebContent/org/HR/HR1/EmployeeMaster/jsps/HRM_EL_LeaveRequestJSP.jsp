<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
"http://www.w3.org/TR/html4/loose.dtd">
<%@ page session="false" contentType="text/html;charset=windows-1252"%>
<%@ page import="java.sql.*,java.util.*,Servlets.Security.classes.UserProfile"%>
<%@ include file="//org/Security/jsps/Check_SessionJSPF.jspf" %>
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=windows-1252"/>
        <title>HRM_EL_LeaveRequestJSP</title>
        <script type="text/javascript" src="../scripts/HRM_EL_LeaveRequestJS.js"> </script>
        <link href="../../../../../css/Sample3.css" rel="stylesheet" media="screen"/>
        <link href="../../../../../css/CalendarControl.css" rel="stylesheet" media="screen"/>  
        <script type="text/javascript" src="../../../../Library/scripts/checkDate.js"></script>
        <script type="text/javascript" src="../../../../Library/scripts/CalendarControl.js"></script>
  </head>
  <body onload="clearAll()">
    <form name=LeaveRequest>
    <div align=center>
    <table border=1 width="80%">
    <tr class="tdH">
        <td colspan=2 align=center height="40">
              <strong>
                   Leave Request Form
              </strong> </td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Employee Id </td>
        <td height="35" align="left"><input type="text" name=eid id=eid size="15" onchange="getFunction(this.value)">
            <img src="../../../../../images/c-lovi.gif" width="20" height="20" alt="empList" onclick="servicepopup();">
        </td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Employee Name </td>
        <td height="35" align="left"><input type="text" name=ename id=ename size="25" disabled="true"></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Designation </td>
        <td height="35" align="left"><input type="text" name=design id=design size="25" disabled="true"></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Balance Available EL </td>
        <td height="35" align="left"><input type="text" name=balanceEL id=balanceEL size="18" disabled="true"></td>
    </tr>
    <tr class="tdH">
        <td colspan="2" align="center" height="35">Current Leave Request Details</td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Leave Type </td>
        <td height="35" align="left"><select name=ltype onchange="balancecalc()">
            <option value="0">Select Leave Type</option>
            <option value="EL">EL</option>
            <option value="CL">CL</option>
            <option value="ML">ML</option>
            <option value="RL">RL</option>
        </select></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Medical Certificate Attached </td>
        <td height="35" align="left">
            <input type="radio" value="Y" name="mc" id="mc1"></input>
                       Yes&nbsp;               
            <input type="radio" value="N" name="mc" id="mc2"></input>
                       No&nbsp;
        </td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Leave Request Id </td>
        <td height="35" align="left"><input type="text" name=rid id=rid size="15" disabled="true">
    </tr>
    <tr class="table">
        <td height="35" align="left">Period From<font color="Gray">(dd/mm/yyyy)</font></td>
        <td height="35" align="left"><input type="text" name=periodFrom id=periodFrom size="20" onfocus="ordercheck()" onkeypress="return calinsert(event,this)">
        <img src="../../../../../images/calendr3.gif" onclick="if(ordercheck()==true)showCalendarControl(document.LeaveRequest.periodFrom);" alt="Show Calendar"></img></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Period TO<font color="Gray">(dd/mm/yyyy)</font></td>
        <td height="35" align="left"><input type="text" name=periodTo id=periodTo size="20" onfocus="ordercheck()" onkeypress="return calinsert(event,this)">
        <img src="../../../../../images/calendr3.gif" onclick="if(ordercheck()==true)showCalendarControl(document.LeaveRequest.periodTo);" alt="Show Calendar"></img></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">No.of Prefix Holidays </td>
        <td height="35" align="left"><input type="text" name=ph id=ph size="15" onfocus="datefun()"></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">No.of Suffix Holidays </td>
        <td height="35" align="left"><input type="text" name=sh id=sh size="15"></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">No.of Leave Days </td>
        <td height="35" align="left"><input type="text" name=leaveDays id=leaveDays size="15" disabled="true" ></td>
    </tr>
    <tr class="table">
        <td align="left">Address During Leave Period </td>
        <td align="left"><textarea rows="2" name="address" cols="20"></textarea></td>
    </tr>
    <tr class="table">
        <td height="35" align="left">Date Applied<font color="Gray">(dd/mm/yyyy)</font> </td>
        <td height="35" align="left"><input type="text" name=dateApplied id=dateApplied size="20" onkeypress="return calinsert(event,this)">
        <img src="../../../../../images/calendr3.gif" onclick="if(ordercheck()==true)showCalendarControl(document.LeaveRequest.dateApplied);" alt="Show Calendar"></img></td>
    </tr>
    <tr class="table">
        <td align="left">Purpose</td>
        <td align="left"><textarea rows="2" name="purpose" cols="20"></textarea></td>
    </tr>
    <tr class="tdH">
        <td colspan="2" align="center"><input type="button" name="submit" value="Submit" onclick="insertfun()">
        <input type="button" name=cancel value="Cancel" onclick="javascript:self.close();"></td>
    </tr>
  </table>
  </div>
  </form>
  </body>
</html>