...
function doFunction(Command,param)
{
   // alert("command:"+Command);
    ...
    if(Command=='loadempedit')
    {
        ...
        if(check )
        {
               ...
               //alert("office id is:"+offid);
                var url="../../../../../RevokeAdditionalCharge.av?Command=loadempedit&txtEmployeeid="+empid+"&offid="+offid;
    ...
    {
         var url="../../../../../RevokeAdditionalCharge.av?Command=loadOffices&txtEmployeeid="+empid;
               // var req=getTransport();
              // alert(req);
             // alert(url);
                req.open("GET",url,true);
       ...
       // alert(check);
        if(check)
        {
               statuswaiting(document.frmCurrentPosting) ;
                 var offid=document.frmCurrentPosting.cmbAddlChargeOffId.value;
              // alert("office id is:"+offid);
                    document.frmCurrentPosting.action="../../../../../RevokeAdditionalCharge.av?Command=Delete&txtEmployeeid="+empid+"&offid="+offid;
       ...
       // alert(check);
        if(check)
        {
               statuswaiting(document.frmCurrentPosting) ;
                 var offid=document.frmCurrentPosting.cmbAddlChargeOffId.value;
              // alert("office id is:"+offid);
                    document.frmCurrentPosting.action="../../../../../RevokeAdditionalCharge.av?Command=Validate&txtEmployeeid="+empid+"&offid="+offid;
                         document.frmCurrentPosting.method="POST";
                         document.frmCurrentPosting.submit();
                       return;   
  ...
  /*  else if(Command=="Delete")
    {
        ...
        {
                statuswaiting(document.frmCurrentPosting) ;
                var url="../../../../../RevokeAdditionalCharge.av?Command=Delete&txtEmployeeid="+empid;
                 url=url+"&txtSNo="+slno;
                req.open("GET",url,true);
                req.onreadystatechange=handleResponse;
                if(window.XMLHttpRequest)
...
function loadEmp(baseResponse)
{
    ...
    else if(flag=="success")
    {
           ...
           clr();
            ...
            var addlstatus=  baseResponse.getElementsByTagName("addlstatus")[0].firstChild.nodeValue;
            var CLOSURE_DATE=baseResponse.getElementsByTagName("CLOSURE_DATE")[0].firstChild.nodeValue;
            var session_to=baseResponse.getElementsByTagName("session_to")[0].firstChild.nodeValue;
            ...
            if(addlstatus=='CL')
            {
                    document.frmCurrentPosting.txtclosuredate.value=CLOSURE_DATE;
                   if(session_to=='FN') 
                      document.frmCurrentPosting.optclosuredate[0].checked=true;
                    else
                      document.frmCurrentPosting.optclosuredate[1].checked=true;
                    document.frmCurrentPosting.cmdedit.disabled=true;
             }
           }
           else
          {
                alert("Record has to be Validate before Revokation");
                document.frmCurrentPosting.cmdedit.disabled=true;
                //clr();
                document.frmCurrentPosting.txtEmployeeid.value="";
                document.frmCurrentPosting.txtEmployee.value="";
...
function  loadOff(req1)
{
               ...
               // var response=req1.responseXML.getElementsByTagName("response")[0];
                ...
                if(flag=="failure")
                {
                    alert("No Entry on Additional Charges ");
                }
                else
                {   
                 var processflow=req1.getElementsByTagName("processflow")[0].firstChild.nodeValue;
               /*  var addlstatus=req1.getElementsByTagName("addlstatus")[0].firstChild.nodeValue;
                 if(addlstatus=='CL')
                 {
                    alert("Employee has already Revoked from additional charge");
                 }
                 else*/
                 {
                // if(processflow=='Yes')
     ...
     /*   else
          {
         alert("No Record Validated. before Revokation Validate the records");
          document.frmCurrentPosting.cmdedit.disabled=true;
          }*/
        }
  }
...
{
//alert(val);
       var url="../../../../../RevokeAdditionalCharge.av?Command=SerGroup&cmbdes="+val;
                req.open("GET",url,true);
                req.onreadystatechange=handleResponse;
                if(window.XMLHttpRequest)
                        req.send(null);
