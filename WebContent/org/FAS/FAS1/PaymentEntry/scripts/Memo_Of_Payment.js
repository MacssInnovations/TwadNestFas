...
function initialLoad(path) {
	...
	var url = path + "/MemoOfPayment?command=gett";
...
function forBillNo(path) {
	...
	var url = path + "/MemoOfPayment?command=BillNo&cboOffice_code="
...
function forBillNo1(baseResponse) {
	...
	//document.getElementById("cboBillNo").value="";
	//
	//
	...
	se.appendChild(op);
		for ( var i = 0; i < len55; i++) {
			...
			var casyr = baseResponse.getElementsByTagName("casyr")[i].firstChild.nodeValue;
			var casmnt = baseResponse.getElementsByTagName("casmnt")[i].firstChild.nodeValue;
			var se = document.getElementById("cboBillNo");
			var op = document.createElement("OPTION");
		//	alert(billNo+"/"+casyr+"/"+casmnt);
			op.value = billNo+"/"+casmnt+"/"+casyr;
			var month="";
			if(casmnt==1)month="Jan";
			else if(casmnt==2)month="Feb";
			else if(casmnt==3)month="Mar";
			else if(casmnt==4)month="Apr";
			else if(casmnt==5)month="May";
			else if(casmnt==6)month="June";
			else if(casmnt==7)month="July";
			else if(casmnt==8)month="Aug";
			else if(casmnt==9)month="Sep";
			else if(casmnt==10)month="Oct";
			else if(casmnt==11)month="Nov";
			else if(casmnt==12)month="Dec";
			
			
			var txt = document.createTextNode(billNo+" -( "+month+" / "+casyr+" )");
			op.appendChild(txt);
			se.appendChild(op);
		}
	} else {
...
function getDetails(path) {
	...
	/*var cboCashBook_Year = document.getElementById("cboCashBook_Year").value;
	var cboCashBook_Month = document.getElementById("cboCashBook_Month").value;*/
	var cboBillCmb = document.getElementById("cboBillNo").value;
	var cboBillNo=cboBillCmb.split("/")[0];
	var cboCashBook_Year = cboBillCmb.split("/")[2];
	var cboCashBook_Month = cboBillCmb.split("/")[1];
	if(document.frm_Memo_Of_Payment.sancidwith[0].checked==true)
	{
		sancidwith ="with_sanc"; 
	}
	...
	//without_sanc_GPF
	var url = path + "/MemoOfPayment?command=getDetails&cboOffice_code="
			+ cboOffice_code + "&cboAcc_UnitCode=" + cboAcc_UnitCode
			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
...
function getDetails1(baseResponse) {
	...
	//	alert("billMajorType"+billMajorType);
		...
		document.frm_Memo_Of_Payment.txtSanctionProceedingid.value = proceedingNo;
...
function addMst(path) {
	...
	/*else if ((document.frm_Memo_Of_Payment.cmbMas_SL_type.value == "")
		...
		var url = path + "/MemoOfPayment?command=addMst&cmbAcc_UnitCode="
...
function deletee(path) {
	...
	if (r == true) {
		var url = path + "/MemoOfPayment?command=deleted&slNo=" + slNo;
		var xmlrequest = AjaxFunction();
		xmlrequest.open("POST", url, true);
		xmlrequest.onreadystatechange = function() {
			manipulate(xmlrequest);
...
function refresh() {
	...
	document.frm_Memo_Of_Payment.cboCashBook_Month.value = "";
	document.frm_Memo_Of_Payment.cboBillNo.length = 1;
	document.frm_Memo_Of_Payment.txtBillMajorType.value = "";
	document.frm_Memo_Of_Payment.txtBillDate.value = "";
	document.frm_Memo_Of_Payment.txtSanctionProceedingNo.value = "";
...
function refresh1() {
	...
	document.frm_Memo_Of_Payment.cboCashBook_Year.value = year;
	//document.frm_Memo_Of_Payment.cboCashBook_Month.value = "s";
	document.frm_Memo_Of_Payment.cboCashBook_Month.value = month;
	document.frm_Memo_Of_Payment.cboBillNo.length = 1;
	document.frm_Memo_Of_Payment.txtBillMajorType.value = "";
	document.frm_Memo_Of_Payment.txtBillDate.value = "";
	document.frm_Memo_Of_Payment.txtSanctionProceedingNo.value = "";
