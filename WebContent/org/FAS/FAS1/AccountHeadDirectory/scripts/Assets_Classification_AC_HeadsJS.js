      ...
      if(req.readyState==4)
        {
          ...
          {    
              ...
              if(flag=="Success")
              {
                                ...
                                //alert( document.Asset_classifications.total.value=count.length);  
                                record1=new Array(); record2=new Array(); record3=new Array();
                                for(i=0;i<count.length;i++)
                                {
		                                    record1[i]=count[i].getElementsByTagName("Major_Desc")[0].firstChild.nodeValue;
		                                    record2[i]=count[i].getElementsByTagName("head_code")[0].firstChild.nodeValue;
		                                    record3[i]=count[i].getElementsByTagName("head_desc")[0].firstChild.nodeValue;
		                                    //alert(record3[i]=count[i].getElementsByTagName("head_desc")[0].firstChild.nodeValue);
...
function loadRecordVal(page)
{
	    ...
	    for(i=p;i<record1.length && c<__pagination;i++)
	    {
	            ...
	            var url="javascript:loadValuesFromTable("+record2[i]+")";
	            anc.href=url;
	            //alert(url);
...
function loadValuesFromTable(head_code)
{
       //alert(head_code);
		...
		//alert(document.getElementById("ac_head_code").text=head_code);
