...
<html>
  <head>
   ...
   // alert(rcells.item(8).firstChild.nodeValue);
      cheqno=rcells.item(8).firstChild.nodeValue;
      cheqdate=rcells.item(9).firstChild.nodeValue;
...
%>
  ...
  <table cellspacing="2" cellpadding="3" width="100%" >
      <tr class="tdH">
        <td colspan="2">
          <div align="center">
            <strong>List of Fund Receipt from Region/Circle/Division Voucher
 </strong>
          </div>
        </td>
      </tr>
     ...
     <table id="mytable" align="center"  cellspacing="3" 
         ...
         <tbody id="tbody" class="table">
          <%
           ...
           try
           {
           String sql= "select m.RECEIPT_NO,to_char(m.RECEIPT_DATE,'DD/MM/YYYY') as rec_date," +
   "trim(to_char(m.TOTAL_AMOUNT,'99999999999999.99')) as TOTAL_AMOUNT ,m.PARTICULARS,m.HO_REF_NO,to_char(m.HO_REF_DATE,'DD/MM/YYYY') as ref_date," +
   " m.CHEQUE_OR_DD,m.CHEQUE_DD_NO,to_char(m.CHEQUE_DD_DATE,'DD/MM/YYYY') as cheq_dd_date,bk.BANK_NAME ||'-'|| br.BRANCH_NAME ||'-'|| br.CITY_TOWN_NAME AS BK_BR_CITY  " +
   "from FAS_FUND_RECEIPT_BY_OFFICE m,FAS_MST_BANKS bk,FAS_MST_BANK_BRANCHES br" +
   " where m.ACCOUNTING_UNIT_ID="+cmbAcc_UnitCode+" and " +
   "m.ACCOUNTING_FOR_OFFICE_ID="+cmbOffice_code+" and m.CASHBOOK_YEAR="+yr+" and m.CASHBOOK_MONTH="+mon+"  and m.RECEIPT_STATUS='L' and m.CHEQUE_OR_DD='C' " +
   ...
   " order by m.RECEIPT_NO,rec_date ";
            ...
            while(rs2.next())
            {
             cnt++;
                ...
                out.println("<td align='left'>"+rs2.getInt("RECEIPT_NO")+"</td>");
               ...
               // out.println("<td align='left'>"+rs2.getString("REMITTANCE_TYPE")+"</td>");
                ...
                out.println("<td align='left'>"+"---"+"</td>");
                 ...
                 if (rs2.getString("HO_REF_NO")!=null){
                out.println("<td align='left'>"+rs2.getString("HO_REF_NO")+"</td>");
                }
                else
                out.println("<td align='left'>"+"---"+"</td>");
                ...
                out.println("<td align='left'>"+"   --  "+"</td>");
                 if (rs2.getString("CHEQUE_OR_DD") != null)
                {
                 out.println("<td align='left'>"+rs2.getString("CHEQUE_OR_DD")+"</td>");
                }
                else
                {
                   out.println("<td align='left'>"+"---"+"</td>");                
                ...
                if (rs2.getString("cheq_dd_date") !=null )
                {
                 out.println("<td align='left'>"+rs2.getString("cheq_dd_date")+"</td>");
                 System.out.println(rs2.getString("cheq_dd_date"));
                }
                else
                {
               ...
               if(rs2.getString("BK_BR_CITY")!=null)
              out.println("<td align='left'>"+rs2.getString("BK_BR_CITY")+"</td>");
                else
                out.println("<td align='left'>"+"   --  "+"</td>");
