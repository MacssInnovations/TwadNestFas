...
var __pagination = 11;
var Command, finYear,minorsortOrder,minorHead, majorsortOrder,majorHead, finYearStart, finYearEnd,service;
...
function returnSubHead(Command, minorsortOrder,minorHead,majorsortOrder,majorHead,finYear,finYearStart,finYearEnd)
{
	 document.getElementById("Command").value=Command;
 document.getElementById("minOrder").value=minorsortOrder;
document.getElementById("minHead").value=minorHead;
 document.getElementById("majOrder").value=majorsortOrder;
document.getElementById("majHead").value=majorHead;
document.getElementById("finYear").value=finYear;
 document.getElementById("finyearStart").value=finYearStart;
document.getElementById("finyearEnd").value=finYearEnd;
load();
	}
...
function load() {
	...
	if (Command = "load_Sub_Heads")
	{
		...
		var url = "../../../../../Accounts_Hierachy.view?Command=load_Sub_Heads&minorsortOrder="+minorsortOrder+"&minorHead="+encodeURIComponent(minorHead)+"&majorsortOrder="+ majorsortOrder+"&majorHead="+encodeURIComponent(majorHead)+"&finYearStart=" + finYearStart + "&finYearEnd=" + finYearEnd;
...
function handleResponse(request) {
	...
	if (request.readyState == 4) {
		if (request.status == 200) {
			...
			if (Command == "load_Sub_Heads" && request.responseXML!=null) {
				loadTable(baseResponse);
			}
			else if(Command == "load_Sub_Heads" && request.responseXML==null)
				{
				baseResponse=xmlDoc;
				loadTable(baseResponse);
				}
...
function loadPage(page) {
	...
	document.frmAccountshierachySubHeadWise.cmbpage.selectedIndex = page - 1;
	var tbody = document.getElementById("tbody");
	try {
		tbody.innerHTML = "";
	} catch (e) {
	...
	if (service) {
		...
		for (i = p; i < service.length && c < __pagination; i++) {
			...
			items[6] = service[i].getElementsByTagName("debit")[0].firstChild.nodeValue;
			items[7] = service[i].getElementsByTagName("credit")[0].firstChild.nodeValue;
			items[8] = service[i].getElementsByTagName("net")[0].firstChild.nodeValue;
			items[9] = service[i].getElementsByTagName("netdisp")[0].firstChild.nodeValue;
			...
			for (j = 5; j < 9; j++) {
				...
				if (j == 5) {
					...
					cell2.setAttribute('align', 'center');
					{
						...
						var url = "javascript:openReport('" + items[j - j]+","+items[j-4].replace(/[^&a-zA-Z ]/g, "")+","+items[j-3]+","
								+items[j-2].replace(/[^&a-zA-Z ]/g, "")+","+items[j-1]+","+items[j].replace(/[^&a-zA-Z ]/g, "")+"')";
						anc.href = url;
...
function openReport(rptType) {
	openFormInWindow("AccountshierachyAccHeadWise.jsp?Command=load_Acc_Heads"
				+ appendOpenerURI(rptType),"Sub_Head");
}
