			...
			//alert(OUTSTANDING_TYPE);
				 ...
				 document.getElementById("Adv_amt").value=BALANCE6;
			    	  ...
			    	  for(var i=0;i<len;i++){
			    		  ...
			    		  var LOCAL_BODY1=baseresponse.getElementsByTagName("LOCAL_BODY1")[i].firstChild.nodeValue;//alert(LOCAL_BODY);
			    		  ...
			    		  var mycurrent_row = document.createElement("TR");
							...
							var cell10=document.createElement("TD");
							  var anc=document.createElement("A");
					            var url="javascript:LoadTextVal("+SLNO+","+PROGRAM_ID+","+BALANCE+","+AMT_RECEIVED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_CUR_MAR+","+AMT_RECEIVED_UPTO_CUR_MAR+","+EXP_ACTUALLY_PAID+",'"+OUTSTANDING_TYPE+"','"+INDIVIDUAL_SCHEME_NAME+"',"+TIE_UP_AMT+","+DISTRICT_ID+",'"+LOCAL_BODY+"','"+LOCAL_BODY1+"','"+LB_TYPE+"')";
					            anc.href=url;
						...
						/*	var BALANCEdesc = document.createElement("input");
							...
							mycurrent_row.appendChild(cell6);*/
							  ...
							  var anc=document.createElement("A");
					            var url="javascript:DelVal("+SLNO+","+PROGRAM_ID+","+BALANCE+","+AMT_RECEIVED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_CUR_MAR+","+AMT_RECEIVED_UPTO_CUR_MAR+","+EXP_ACTUALLY_PAID+",'"+OUTSTANDING_TYPE+"','"+INDIVIDUAL_SCHEME_NAME+"',"+TIE_UP_AMT+","+DISTRICT_ID+",'"+LOCAL_BODY+"','"+LOCAL_BODY1+"','"+LB_TYPE+"')";
					            anc.href=url;
				    	...
				    	//  alert("len....."+len+"  type......."+OUTSTANDING_TYPE);
				    	  ...
				    	  for(var i=0;i<len;i++){
				    		  ...
				    		  var OUTSTANDING_TYPE=baseresponse.getElementsByTagName("OUTSTANDING_TYPE")[i].firstChild.nodeValue;
				    		  var EXP_ACTUALLY_PAID=baseresponse.getElementsByTagName("EXP_ACTUALLY_PAID")[i].firstChild.nodeValue;
				    		  var LOCAL_BODY1=baseresponse.getElementsByTagName("LOCAL_BODY1")[i].firstChild.nodeValue;
				    		  ...
				    		  var mycurrent_row = document.createElement("TR");
								...
								var cell10 = document.createElement("TD");
								  var anc=document.createElement("A");
						            var url="javascript:LoadTextVal("+SLNO+","+PROGRAM_ID+","+BALANCE+","+AMT_RECEIVED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_MAR2011+","+EXP_BOOKED_UPTO_CUR_MAR+","+AMT_RECEIVED_UPTO_CUR_MAR+","+EXP_ACTUALLY_PAID+",'"+OUTSTANDING_TYPE+"','"+INDIVIDUAL_SCHEME_NAME+"',"+TIE_UP_AMT+","+DISTRICT_ID+",'"+LOCAL_BODY+"','"+LOCAL_BODY+"','"+LB_TYPE+"')";
						            anc.href=url;
						        	var Category_ID_code_text = document.createTextNode("EDIT");
						            anc.appendChild(Category_ID_code_text);
						            cell1.appendChild(anc);
...
function LoadTextVal(sno,pid,bal,rec_up,book_upto,book_cur,rec_cur,Act_paid,type,sch_name,tie,district,LOCAL_DESC,LOCAL_BODY,lb_type)
{
	...
	if(lb_type=="C"){
		...
		lb_type="Townpanchayat";
	}else if(lb_type=="O"){
		lb_type="Others";
	}
	
	document.getElementById("Local1").value=lb_type;
	document.getElementById("Dist_val").value=LOCAL_BODY+"-"+district;
	document.getElementById("save_id1").value="update";
		...
		document.getElementById("ALocal1").value=lb_type;
		document.getElementById("Dist_val").value=district+"-"+LOCAL_BODY;
...
function Row_Adding(val,value)
{
	...
	program.setAttribute("style", "width:191px");
	program.setAttribute("width", "191px");
	 var req=getTransport();
	program.id = "pro_id" + c;
   ...
   // alert('12345');
	...
	Add_btn.type="button";
	Add_btn.id="add_oth"+c;
	Add_btn.value="ADD";
	
	Add_btn.setAttribute("onclick","fun_oth('F','"+c+"')");
	...
	div2.id="fin_oth"+c;
	var Ins_btn=document.createElement("input");
	Ins_btn.type="text";
	Ins_btn.id="other_id"+c;	
	div2.appendChild(Ins_btn);
	var Ins_btn=document.createElement("input");
	Ins_btn.type="button";
	Ins_btn.id="insert_oth"+c;
	Ins_btn.value="Insert";
	Ins_btn.setAttribute("onclick","other_Details('F','"+c+"')");
	Ins_btn.setAttribute("style","display:none;");
	div2.appendChild(Ins_btn);
	cell9.appendChild(div2);
...
//alert(c1);
		...
		if(local=="Corporation")
			...
			local="M";
		else if(local=="Townpanchayat")
			local="T";
		else if(local=="Others")
			local="O";
		var Dist_val=document.getElementById("Dist_val").value;
		var dist=Dist_val.split("-");
		var district=dist[1];
		var localbody=dist[0];
		  ...
		  "&pro_id="+pid+"&sch_id="+sch_name+"&AmtRec_id="+rec_cur
			...
			//+"&Bal_id="+bal+"&sno="+sno
			+"&incur_id="+incur+"&sno="+sno+"&tie_id="+tie_id+"&local="+localbody+"&Dist_val="+district+"&lb_type="+local;;
	   // alert(url);
	        var req=getTransport();
	        req.open("GET",url,true);
	        req.onreadystatechange=function()
...
if(but=="Add_Row"){
	...
	var Add_btn=document.createElement("input");
	Add_btn.type="button";
	Add_btn.id="Aadd_oth"+c;
	Add_btn.value="ADD";
	Add_btn.setAttribute("onclick","fun_oth('T','"+c+"')");
	Add_btn.setAttribute("style","display:none;");
	div1.appendChild(Add_btn);
	var a_tag=document.createElement("a");
	a_tag.id="Abackbt"+c;
	...
	var Ins_btn=document.createElement("input");
	Ins_btn.type="text";
	Ins_btn.id="Aother_id"+c;	
	div2.appendChild(Ins_btn);
	var Ins_btn=document.createElement("input");
	Ins_btn.type="button";
	Ins_btn.id="Ainsert_oth"+c;
	Ins_btn.value="Insert";
	Ins_btn.setAttribute("onclick","other_Details('T','"+c+"')");
	Ins_btn.setAttribute("style","display:none;");
	div2.appendChild(Ins_btn);
	cell9.appendChild(div2);
	current_row1.appendChild(cell9);
...
for(var i=1;i<=hid;i++){
	...
	if(main_Cmb=="Corporation")
		...
		{
			main_Type="T";
		}else if(main_Cmb=="Others")
		{
			main_Type="O";
		}
	var sub_Cmb=document.getElementById("fin"+i).value;
	var sub_cnmb_val=sub_Cmb.split("-");
	var sub_menu=sub_cnmb_val[1];
	
	var local_by=sub_cnmb_val[0];
	...
	url += "&pro_id=" + pro_id + "&sch_id=" + sch_id + "&AmtRec_id="
				...
				+ "&main_Cmb=" + main_Cmb+ "&sub_Div=" + sub_menu+ "&main_Type=" + main_Type
				+ "&local_by=" + local_by;
...
function balanceVal(n) {
	...
	var hid=document.getElementById("hid"+n).value;	
...
function Board_Liability(path) {
	...
	//	var Bal_id=document.getElementById("ABal_id"+i).value;		
		...
		var sub_cnmb_val=sub_Cmb.split("-");
		var sub_menu=sub_cnmb_val[1];//DIST_ID
		var local_by=sub_cnmb_val[0];//LC_ID
...
function printReport(){
	...
	var date_val=document.getElementById("date_val").value;
	var fin_year=document.getElementById("f_year").value;
	var rep_head=document.getElementById("rep_head").value;
	
	var url ="../../../../../Twad_report_ser?Command=PDF&fin_year=" + fin_year
	+ "&txtAcc_UnitCode=" + txtAcc_UnitCode;
	
	if(rep_head=="REG_Detail"){
		var sub_rdio_div= document.getElementById("rep_head_rdo").value; 
	var Array_div=sub_rdio_div.split("-");
	sub_rdio_div=Array_div[0];
	var desc=Array_div[1];
		//alert("sub_rdio_div .."+sub_rdio_div);
	 if(sub_rdio_div=="6868" || sub_rdio_div=="6869" || sub_rdio_div=="6870" || sub_rdio_div=="6871" ||  sub_rdio_div=="6777")
	{
		url+="&type="+rep_head+"&sub_div="+sub_rdio_div+"&desc="+desc;		
	}
	}
	else if (rep_head!="REG_Detail"){
		url+="&type="+rep_head;
	}
	//alert(url);
	document.frmMIS_Twad.action = url;
	...
	function chk_sub_div(){
		var rep_head=document.getElementById("rep_head").value;
		//alert(" nnnnn "+rep_head);
		if(rep_head=="REG_Detail"){
			var rep_head=document.getElementById("rep_head").value;
			document.getElementById("sub_rdio_div").style.display="block";
			}
		else{
			document.getElementById("sub_rdio_div").style.display="none";
			printReport();
		}
	}
...
function LoadlcData(val,sno){
	//alert(val+sno);
	var txtAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;
	var url ="../../../../../Twad_report_ser?Command=loadOnchange&txtAcc_UnitCode="+txtAcc_UnitCode;
	var id;
	if(val=="s"){
...
function common_LB(req,val,no){
    ...
    if(req.readyState==4)
    {
     ...
     var baseresponse = req.responseXML.getElementsByTagName("response")[0];
        ...
        var command=baseresponse.getElementsByTagName("command")[0].firstChild.nodeValue;
       var flag=baseresponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
       var LB_Type=baseresponse.getElementsByTagName("LB_Type")[0].firstChild.nodeValue;
      ...
      if(LB_Type!="O")
     {
        ...
        { var cmbAcc_pro_id="";        	
        	if(val=="s"){
        		document.getElementById("fin"+no).style.display="block";
        		document.getElementById("add_oth"+no).style.display="none";
        		
            cmbAcc_pro_id = document.getElementById("fin"+no);  }
        	else if(val=="T"){
        		document.getElementById("Afin"+no).style.display="block";
        		document.getElementById("Aadd_oth"+no).style.display="none";
            ...
            {  
             ...
             //   alert(ID);
                ...
                var DCODE=baseresponse.getElementsByTagName("DCODE")[i].firstChild.nodeValue;
                document.getElementById("Dist_val").value=DCODE+"-"+ID;
                option.text=URBANLB_TYPE_DESC;
                option.value=ID+"-"+DCODE;
    	...
    	if(flag=="success")
    		{
    		if(val=="s"){
    			document.getElementById("fin_oth"+no).style.display="none";
				document.getElementById("fin"+no).style.display="block";
				document.getElementById("add_oth"+no).style.display="block";
                cmbAcc_pro_id = document.getElementById("fin"+no);  }
            	else if(val=="T"){
            		document.getElementById("Afin_oth"+no).style.display="none";
    				document.getElementById("Afin"+no).style.display="block";
    				document.getElementById("Aadd_oth"+no).style.display="block";
            		 cmbAcc_pro_id = document.getElementById("Afin"+no);
            	}else{
            		alert('Error in 2095 th line.......');
            	}
    		 cmbAcc_pro_id.length=0;
             var option=document.createElement("OPTION");
             option.text="Select";
         ...
         //	document.getElementById("Dist_val").value=DCODE+"-"+ID;
             ...
             catch(errorObject)
             {
    			...
    			if(val=="s"){
    				document.getElementById("insert_oth"+no).style.display="block";
    				document.getElementById("add_oth"+no).style.display="none";
    				document.getElementById("fin_oth"+no).style.display="block";
    				
    				document.getElementById("fin"+no).style.display="none";
    				
                   }
                	else if(val=="T"){
                		document.getElementById("Ainsert_oth"+no).style.display="block";
                		document.getElementById("Afin_oth"+no).style.display="block";
        				document.getElementById("Afin"+no).style.display="none";
        				document.getElementById("Aadd_oth"+no).style.display="none";
                	}else{
                		alert('Error in 2134 th line.......');
                	}
    		
    	}else{
    		alert('Error in Loading lOCAL BODY ..... ');
    	}
...
function Loadlhid(val)
{
	// var d=val.split("-");
//val=parseInt(d[0]);
	document.getElementById("Dist_val").value=val;
	}
...
document.getElementById("fin_oth"+sno).style.display="none";
...
document.getElementById("Afin_oth"+sno).style.display="none";
...
function fun_oth(val,no){
	//alert('test'+val+"no ..."+no);
	if(val=="F"){ 
		document.getElementById("fin_oth"+no).style.display="block";
		document.getElementById("insert_oth"+no).style.display="block";
		document.getElementById("fin"+no).style.display="none";
...
function other_Details(val,no){
	var txtAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;
	
	var other_id="";
	if(val=="F"){
	other_id=document.getElementById("other_id"+no).value;}
	else if(val=="T")
		{
		other_id=document.getElementById("Aother_id"+no).value;
		}
	var url = "../../../../../Twad_report_ser?Command=Add_oth&txtAcc_UnitCode="
	+ txtAcc_UnitCode + "&others=" + other_id ;
	//alert(url);
	...
	req.onreadystatechange=function()
	{
	    cmn_others(req,no,val);
	}
	req.send(null);
	
}
function cmn_others(req,no,val){
	 if(req.readyState==4)
	    {
	     if(req.status==200)
	     {  
	...
	if(command=="Add_oth"){
	      ...
	      if(val=='F'){
	     document.getElementById("other_id"+no).value="";
	     document.getElementById("other_id"+no).style.display="none";
	     document.getElementById("insert_oth"+no).style.display="none";
	     LoadlcData('s',no);
	     }else if(val=='T'){
	     document.getElementById("Aother_id"+no).value="";
	     document.getElementById("Aother_id"+no).style.display="none";
	     document.getElementById("Ainsert_oth"+no).style.display="none";
	     LoadlcData('T',no);
	     }
...
function check_User(path){
	var emp_id=document.getElementById("empid_val").value;
	var url = "";
	if(emp_id==0){
		document.getElementById("rep_head").style.display="block";
	}
	else{
	 url = path+"/Twad_report_ser?Command=user_Role&Empid_val="+emp_id;
	 var req=getTransport();
		req.open("GET",url,true);
		req.onreadystatechange=function()
		{
	 ...
	 if(req.readyState==4)
	    {
	     ...
	     var baseresponse = req.responseXML.getElementsByTagName("response")[0];
	       var command=baseresponse.getElementsByTagName("command")[0].firstChild.nodeValue;
	       if(command=="user_Role"){
	       var flag=baseresponse.getElementsByTagName("ID_flag")[0].firstChild.nodeValue;
	     // alert(" >>> "+flag);
	       if(flag==1){
	    	   document.getElementById("rep_head").style.display="block";
	       }else if(flag==0){
	    	   document.getElementById("rep_head").style.display="none";
	       }
	       }
