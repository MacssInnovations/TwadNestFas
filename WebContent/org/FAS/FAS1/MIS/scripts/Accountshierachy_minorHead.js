...
var __pagination = 11;
var Command, finYear, majorsortOrder, finYearStart, finYearEnd,service;
...
function returnMinorHead(Command,finYear,majorsortOrder,finYearStart,finYearEnd)
{
	 document.getElementById("Command").value=Command;
 document.getElementById("majorsortOrder").value=majorsortOrder;
...
function load() {
	...
	majorsortOrder = document.getElementById("majorsortOrder").value;
	...
	if (Command = "load_Minor_Heads")
	{
		...
		var url = "../../../../../Accounts_Hierachy.view?Command=load_Minor_Heads&majorsortOrder="
			+majorsortOrder.trim()+"&finYearStart="+finYearStart.trim()+"&finYearEnd="+finYearEnd.trim();
...
function processResponse(request) {
	...
	if (request.readyState == 4) {
		if (request.status == 200) {
			...
			if (Command == "load_Minor_Heads" && request.responseXML!=null) {
				loadTable(baseResponse);
			}
			else if(Command == "load_Minor_Heads" && request.responseXML==null)
				{
				baseResponse=xmlDoc;
				loadTable(baseResponse);
				}
...
function loadPage(page) {
	...
	document.frmAccountshierachyMinorHeadWise.cmbpage.selectedIndex = page - 1;
	var tbody = document.getElementById("tbody");
	try {
		tbody.innerHTML = "";
	} catch (e) {
	...
	if (service) {
		...
		for (i = p; i < service.length && c < __pagination; i++) {
			...
			items[6] = service[i].getElementsByTagName("majhead")[0].firstChild.nodeValue;

			var tbody = document.getElementById("tbody");
			var mycurrent_row = document.createElement("TR");
			mycurrent_row.setAttribute("id", items[6]);
			...
			for (j = 1; j < 5; j++) {
				...
				if (j == 1) {
					...
					{
						var majorsortOrder = document.getElementById("majorsortOrder").value;
						var anc = document.createElement("A");
						/*var str = "abc's test#s";
						alert(str.replace(/[^a-zA-Z ]/g, ""));*/
						var url = "javascript:openReport('" + items[j - 1]+","+items[j].replace(/[^&a-zA-Z ]/g, "")+","+majorsortOrder+","
								+ items[j+5].replace(/[^a-zA-Z ]/g, "")+"')";
						anc.href = url;
...
function appendOpenerURI(rptType) {
	...
	var uri = "";
	uri = "&finYear=" + getValue("finYear") + "&minOrder="+minOrder+"&minHead="+encodeURIComponent(minHead)+"&majOrder=" + majOrder+"&majHead="+encodeURIComponent(majHead);
...
function openReport(rptType) {
	openFormInWindow("AccountshierachySubHeadWise.jsp?Command=load_Sub_Heads"
				+ appendOpenerURI(rptType),"Minor_Head");
}
...
function migration(id)
{
    ...
    Command =id.trim();
	majorsortOrder = document.getElementById("majorsortOrder").value.trim();
	majorHead = document.getElementById("majorHead").value.trim();
