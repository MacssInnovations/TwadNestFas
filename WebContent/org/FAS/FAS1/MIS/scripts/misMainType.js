...
function changepagesize() {	
	...
	var len = gridrepsonse.getElementsByTagName("MAIN_TYPE_CODE").length;	
...
function changepage() {
	...
	for ( var i = ll; i < ul; i++) {
		...
		var subCode=gridrepsonse.getElementsByTagName("MAIN_TYPE_CODE")[i].firstChild.nodeValue;
		var subDesc=gridrepsonse.getElementsByTagName("MAIN_TYPE_NAME")[i].firstChild.nodeValue;
		...
		if (view == "C") {
        	...
        	sch_id.id="id"+seq;
        	sch_id.value="&majorCode="+majorCode+"&mainTypeCode="+subCode;	       
		...
		var tides = document.createTextNode(subCode);
 ...
 function clearAll(){
	...
	document.getElementById('mainTypeDesc').value="";	
...
function callServer(command) {	 
	 ...
	 var mainTypeDesc=document.getElementById('mainTypeDesc').value;
     var url="";
       if(command=="Add"){
    	   	var flag=nullCheck();    	   	
            if(flag==true){            	
            	url="../../../../../MISMainTypeCode?command=add&mainCategoryId="+categoryDesc+
            	"&mainTypeDesc="+mainTypeDesc;
                ...
                req.onreadystatechange=function(){
                 ...
                 req.send(null);
                    ...
                    if(flag==true){
                    	...
                    	url="../../../../../MISMainTypeCode?command=update&mainCategoryId="+categoryDesc+
                    	"&mainTypeCode="+mainTypeCode+"&mainTypeDesc="+mainTypeDesc;
                    	 var req=getTransport();
                    	 req.open("POST",url,true);        
                    	 req.onreadystatechange=function(){
                    		 processResponse(req);
	        	...
	        	var mainTypeCode=document.getElementById('mainTypeCode').value;
            	url="../../../../../MISMainTypeCode?command=deleted&mainCategoryId="+categoryDesc+
            	"&mainTypeCode="+mainTypeCode+"&mainTypeDesc="+mainTypeDesc;
                var req=getTransport();
                req.open("POST",url,true);        
                req.onreadystatechange=function(){
        		...
        		url="../../../../../MISMainTypeCode?command=getGrid&cateCode="+document.getElementById('categoryCode').value;
                var req=getTransport();
                req.open("POST",url,true);                
                req.onreadystatechange=function(){
                	viewResponse(req);
...
var checkflag=false;
 ...
 function viewDetails(id){
	 ...
	 var url="../../../../../MISMainTypeCode?command=edit"+jid;
     var req=getTransport();
      req.open("POST",url,true);        
      req.onreadystatechange=function()
      {
 ...
 function editResponse(response){
	 var res=response.getElementsByTagName("status")[0].firstChild.nodeValue;
	 	if(res=="success"){ 		
	 		...
	 		document.getElementById('mainTypeCode').value=response.getElementsByTagName("SUB_TYPE_CODE")[0].firstChild.nodeValue;
	 		document.getElementById('mainTypeDesc').value=response.getElementsByTagName("SUB_TYPE_NAME")[0].firstChild.nodeValue;
...
function nullCheck(){
	...
	if(document.getElementById('mainTypeDesc').value==""){
