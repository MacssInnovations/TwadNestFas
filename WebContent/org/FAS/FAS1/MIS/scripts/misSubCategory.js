...
function changepage() {
	...
	for ( var i = ll; i < ul; i++) {
		...
		var subCode=gridrepsonse.getElementsByTagName("SUB_TYPE_CODE")[i].firstChild.nodeValue;
		var subDesc=gridrepsonse.getElementsByTagName("SUB_TYPE_NAME")[i].firstChild.nodeValue;
		...
		if (view == "C") {
        	...
        	sch_id.id="id"+seq;
        	sch_id.value="&majorCode="+majorCode+"&subCode="+subCode;	       
...
function callServer(command) {	 
     ...
     var url="";
       if(command=="Add"){
    	   	var flag=nullCheck();    	   	
            if(flag==true){            	
            	url="../../../../../MISSubCategory?command=add&mainCategoryId="+categoryDesc+
                ...
                req.onreadystatechange=function(){
                 ...
                 req.send(null);
                    ...
                    var flag=nullCheck();
                    if(flag==true){
                    	var subCategoryCode=document.getElementById('subCategoryCode').value;
                    	url="../../../../../MISSubCategory?command=update&mainCategoryId="+categoryDesc+
                    	"&subCategoryId="+subCategoryCode+"&subCategoryName="+subcategoryDesc;
                    	 var req=getTransport();
                    	 req.open("POST",url,true);        
                    	 req.onreadystatechange=function(){
                    		 processResponse(req);
	        	...
	        	var subCategoryCode=document.getElementById('subCategoryCode').value;
	        	var subCategoryCode=document.getElementById('subCategoryCode').value;
            	url="../../../../../MISSubCategory?command=deleted&mainCategoryId="+categoryDesc+
            	"&subCategoryId="+subCategoryCode+"&subCategoryName="+subcategoryDesc;
                var req=getTransport();
                req.open("POST",url,true);        
                req.onreadystatechange=function(){
        		...
        		url="../../../../../MISSubCategory?command=getGrid&cateCode="+document.getElementById('categoryCode').value;
                var req=getTransport();
                req.open("POST",url,true);                
                req.onreadystatechange=function(){
                	viewResponse(req);
...
var checkflag=false;
 ...
 function viewDetails(id){
	 ...
	 var url="../../../../../MISSubCategory?command=edit"+jid;
     var req=getTransport();
      req.open("POST",url,true);        
      req.onreadystatechange=function()
      {
 ...
 function editResponse(response){
	 var res=response.getElementsByTagName("status")[0].firstChild.nodeValue;
	 	if(res=="success"){ 		
	 		...
	 		document.getElementById('subCategoryCode').value=response.getElementsByTagName("SUB_TYPE_CODE")[0].firstChild.nodeValue;
	 		document.getElementById('subCategoryDesc').value=response.getElementsByTagName("SUB_TYPE_NAME")[0].firstChild.nodeValue;
