...
<script type="text/javascript">
function loadValuesFromTable(unit_id,id,val,fromMonth,toMonth,WE_R){
	//alert(">>>> "+val);
...
<table border=1 width="82%" cellpadding="0" cellspacing="0"
	...
	<%
		String sub_qry="", cmd = "",id="", val="",liveFY = "", liveTY = "", fromMonth = "", toMonth = "", qry = "", sup_query = "", unit_id = "", head = "", unit_qry = "";
	...
	int cmbAcc_UnitCode=Integer.parseInt(request.getParameter("cmbAcc_UnitCode"));
	 if(cmbAcc_UnitCode==0){sub_qry="";}
		...
		out.println("<tr>");
		out.println("<td colspan='6' valign='bottom' align='center' style='color:#FFF5EE;height:35px;font-size:18px ;font-weight: bold;background:#3B9C9C;'> NRDWP Details</td>");
		out.println("</tr>");
	...
	int c=0;
		...
		while (rs.next()) {
			...
			out.println("<td><a href=\"javascript:loadValuesFromTable("+cmbAcc_UnitCode+",'" + rs.getString("Annualplanninggroupingid") + "','" + rs.getString("Annualplanninggrouping") + "','"+fromMonth+"','"+toMonth+"','"+WE_R+"')\">"+rs.getString("Annualplanninggrouping")+"</a></td>");
			
			out.println("<td align='Right'>"+rs.getBigDecimal("Regular").setScale(2,BigDecimal.ROUND_CEILING)+"</td>");
			out.println("<td align='Right'>"+rs.getBigDecimal("Sc").setScale(2,BigDecimal.ROUND_CEILING)+"</td>");
			out.println("<td align='Right'>"+rs.getBigDecimal("St").setScale(2,BigDecimal.ROUND_CEILING)+"</td>");
			out.println("<td align='Right'>"+rs.getBigDecimal("OTHERS").setScale(2,BigDecimal.ROUND_CEILING)+"</td>");
			//out.println("<td align='Right'>"+rs.getBigDecimal("Minority").setScale(2,BigDecimal.ROUND_CEILING)+"</td>");
...
<table  width="84%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" align="center">
	<tr class="tdH"> 
		<td colspan="6" width="7%" valign="bottom" align="center">
