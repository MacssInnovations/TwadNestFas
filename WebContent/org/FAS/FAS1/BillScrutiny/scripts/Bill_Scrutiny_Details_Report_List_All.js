...
{   
	  /*  var cmbAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;    
        ...
        if(document.frmBill_Scrutiny_Report_list.optionId[0].checked==true)
	    {
        	optionId=document.frmBill_Scrutiny_Report_list.optionId[0].value;
	    }else{
	    	optionId=document.frmBill_Scrutiny_Report_list.optionId[1].value;
	    }
        if(txtCB_Year=="")
        {
        ...
        else
        {
                ...
                var url="../../../../../Bill_Scrutiny_Details_Report_List_All?Command=searchByMonth&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&optionId="+optionId;			                
                ...
                if(service)
                 {
                	 for(i=0;i<service.length;i++)
                     {
                             ...
                             items[5]=service[i].getElementsByTagName("billscrdate")[0].firstChild.nodeValue;
                             items[6]=service[i].getElementsByTagName("sancamt")[0].firstChild.nodeValue;
                             ...
                             items[9]=service[i].getElementsByTagName("deducted_amount")[0].firstChild.nodeValue;
                             items[10]=service[i].getElementsByTagName("NET_AMOUNT")[0].firstChild.nodeValue;
                             items[11]=service[i].getElementsByTagName("remarks")[0].firstChild.nodeValue;
                             
                             
                             items[12]=service[i].getElementsByTagName("ACCOUNTING_UNIT_ID")[0].firstChild.nodeValue;
                             items[13]=service[i].getElementsByTagName("ACCOUNTING_UNIT_OFFICE_ID")[0].firstChild.nodeValue;
                           ...
                           //  items[4]="Employees";
                            ...
                            var url="";
                            var url="javascript:Show_new('"+items[12]+"','"+items[13]+"','"+CASHBOOK_YEAR+"','"+CASHBOOK_MONTH+"','"+items[2]+"','"+items[4]+"')";
                            anc.href=url;
...
var Voucher_list_SL;
function Show_new(unitcode,offid,yr,mon,bilno,sancno)
{
    ...
    Voucher_list_SL= window.open("../../../../../org/FAS/FAS1/BillScrutiny/jsps/Bill_Scrutiny_Details_Report_SubList.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&bilno="+bilno+"&sancno="+sancno,"ReceiptDateSearch","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    Voucher_list_SL.moveTo(250,250);  
    Voucher_list_SL.focus();
