...
{  
    ...
    { 
        ...
        {  
           ...
           if(Command=="FreezeA52Qty")
            {
            
                addRow(baseResponse);
            }
           
           else if(Command=="UnFreezeA52Qty"){
        	   UnFreezeA52Qty1(baseResponse);
           }
...
/*function checkStatusA52AA52Register1(baseResponse){
	...
	if(flag=="success"){
     ...
     //alert(A52_STATUS+"--------"+AA52_STATUS);
      if((A52_STATUS=='Y')){
    	  document.frmQty_Freeze_A52Register.a52_option.disabled=true; 
      }
...
{  
    ...
    /*else
    {
    	verifya52aa52=document.frmQty_Freeze_A52Register.a52_option[1].value;
    //	verifya52aa52=document.getElementById("a52_option").value;
   }
    */
        ...
        if(Command=="FreezeA52Qty")
        {
           if(nullcheck())
            {
        	   if(document.frmQty_Freeze_A52Register.a52_option.checked==true)
        	    {
        	    	verifya52aa52=document.getElementById("a52_option").value;
        	    }
                var url="../../../../../Freeze_Qty_A52?command=FreezeA52Qty"+
            ...
            if(nullcheck())
             {
         	   if(document.frmQty_Freeze_A52Register.a52_option.checked==true)
                 ...
                 var url="../../../../../Freeze_Qty_A52?command=UnFreezeA52Qty"+
                 	"&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"" +
                 	"&cmbFinancialYear="+cmbFinancialYear+
                 	"&verifya52aa52="+verifya52aa52;
                // alert("url "+url);
...
/*function checkStatus(){
	document.frmQty_Freeze_A52Register.a52_option.checked=false;
   // document.frmQty_Freeze_A52Register.a52_option[1].checked=false;
    document.frmQty_Freeze_A52Register.a52_option.disabled=false;
  //  document.frmQty_Freeze_A52Register.a52_option[1].disabled=false;
	
	var cmbAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;
	var cmbOffice_code=document.getElementById("cmbOffice_code").value;
	var cmbFinancialYear=document.getElementById("cmbFinancialYear").value;
	 if(document.frmQty_Freeze_A52Register.cmbFinancialYear.value.length==0 )
	    {
	        alert("select finanical year");
	        return false;
	    } 
	else{
    var url="../../../../../Freeze_Qty_A52?command=checkStatusA52AA52Register"+
	"&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+
	"&cmbFinancialYear="+cmbFinancialYear;
		//alert("url "+url);
		var req=getTransport();
...
function nullcheck()
{
    ...
    if((document.frmQty_Freeze_A52Register.a52_option.checked==false) )//&& (document.frmQty_Freeze_A52Register.a52_option[1].checked==false)
    {
...
function addRow(baseResponse)
{
 var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
    if(flag=="success")
    {   
         alert("successfully Freeze A52 Qty");
         clearall();
...
function UnFreezeA52Qty1(baseResponse)
{
 var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
    if(flag=="success")
    {   
         alert("successfully unFreeze A52 Qty");
         clearall();
        
    } 
   else if(flag=="CircleFreezed")
    {
        alert("your circle is Freezed Done,So you cant Unfreeze");
        clearall();
        
    }
   else if(flag=="FirstFreeze")
   {
       alert("You didnt freeze,First Freeze it");
   } 
    else if(flag=="failure")
...
{
       document.frmQty_Freeze_A52Register.a52_option.checked=false;
       document.frmQty_Freeze_A52Register.cmbFinancialYear.value="2011-12";
       document.frmQty_Freeze_A52Register.a52_option.disabled=false;
