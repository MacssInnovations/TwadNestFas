...
<%@page import="java.math.BigDecimal"%><html>
  ...
  <body class="table"><form name="frmJournal_ListAll_SL" method="POST">
      <%
  Connection con=null;
  ResultSet rs=null;
      ...
      <table id="mytable" align="center" cellspacing="3" cellpadding="2"
        ...
        <tbody id="tbody" class="table">
          ...
          /*String sql="select SL_NO, ACCOUNT_HEAD_CODE ,CR_DR_INDICATOR ,"+
            "SUB_LEDGER_TYPE_CODE ,(select SL_CODENAME from SL_TYPE_CODE_NAME_VIEW where SL_TYPE=SUB_LEDGER_TYPE_CODE and SL_CODE=SUB_LEDGER_CODE) as SLNAME ,AMOUNT, PARTICULARS,SUB_LEDGER_CODE "+
            ...
            String sql="select SL_NO, ACCOUNT_HEAD_CODE ,CR_DR_INDICATOR ,"+
            "SUB_LEDGER_TYPE_CODE ,(select SL_CODENAME from SL_TYPE_CODE_NAME_VIEW_M where SL_TYPE=SUB_LEDGER_TYPE_CODE and SL_CODE=SUB_LEDGER_CODE) as SLNAME ,AMOUNT, PARTICULARS,SUB_LEDGER_CODE "+
            "from FAS_JOURNAL_TRANSACTION where ACCOUNTING_UNIT_ID="+cmbAcc_UnitCode+" and ACCOUNTING_FOR_OFFICE_ID="+
            cmbOffice_code+"and CASHBOOK_YEAR="+yr+"and CASHBOOK_MONTH="+mon+"and  VOUCHER_NO="+recNo;
		...
		/*if(rs2.getInt("SUB_LEDGER_TYPE_CODE")==15)
            	   {
                         //   System.out.println("15555555555555555555555555");
            		   ...
            		   if(rs5.next())
            		   {
            			  // xml=xml+"<desc_type>"+rs4.getString("BENEFICIARY_NAME")+"</desc_type>"; 
            			   out.println("<td align='left'>"+rs5.getString("ACCOUNTING_UNIT_NAME")+"</td>");
            		   }else{
            			   out.println("<td align='left'>"+"--"+"</td>");
            		   }
            		   ps5.close();
                  ...
                  /* if(rs2.getInt("SUB_LEDGER_TYPE_CODE")==14)
            	   {
            		   ...
            		   //cmbOffice_code
            		   ps4=con.prepareStatement("SELECT BENEFICIARY_NAME FROM PMS_DCB_MST_BENEFICIARY	WHERE BENEFICIARY_SNO=?	AND OFFICE_ID=? ");
            		   ...
            		   {
            			  // xml=xml+"<desc_type>"+rs4.getString("BENEFICIARY_NAME")+"</desc_type>"; 
            			   out.println("<td align='left'>"+rs4.getString("BENEFICIARY_NAME")+"</td>");
            		   }else{
            			   out.println("<td align='left'>"+"--"+"</td>");
            		   }
            		   ps4.close();
                    ...
                    if(rs2.getInt("SUB_LEDGER_TYPE_CODE")==18)
            	   {
            		   ...
            		   //cmbOffice_code
            		   ps4=con.prepareStatement("SELECT pensioner_name FROM hrm_pen_mst_details WHERE ppo_no=? and process_status ='VALIDATE'	AND Payment_Office_Id=? ");
            		   ps4.setInt(1, rs2.getInt("SUB_LEDGER_CODE"));
            		   ps4.setInt(2,cmbOffice_code);
            		   rs4=ps4.executeQuery();
            		   if(rs4.next())
            		   {
            			  // xml=xml+"<desc_type>"+rs4.getString("BENEFICIARY_NAME")+"</desc_type>"; 
            			   out.println("<td align='left'>"+rs4.getString("pensioner_name")+"</td>");
            		   }else{
            			   out.println("<td align='left'>"+"--"+"</td>");
            		   }
            		   ps4.close();
                   ...
                   else if(rs2.getInt("SUB_LEDGER_TYPE_CODE")==19)
            	   {
                 
            		   //cmbOffice_code
            		   ps4=con.prepareStatement("SELECT FPENSIONER_NAME FROM HR_PEN_MST_FAMILY WHERE ppo_no=? and process_status ='VALIDATE'	AND Payment_Office_Id=? ");
            		   ps4.setInt(1, rs2.getInt("SUB_LEDGER_CODE"));
            		   ps4.setInt(2,cmbOffice_code);
            		   rs4=ps4.executeQuery();
            		   if(rs4.next())
            		   {
            			  // xml=xml+"<desc_type>"+rs4.getString("BENEFICIARY_NAME")+"</desc_type>"; 
            			   out.println("<td align='left'>"+rs4.getString("FPENSIONER_NAME")+"</td>");
            		   }else{
            			   out.println("<td align='left'>"+"--"+"</td>");
            		   }
            		   ps4.close();
                   ...
                   else if(rs2.getString("SLNAME")!=null)
