...
%>
  <body onload="call_clr();foc();Check_Supplement_No();LoadAccountingUnitID_Create('LIST_ALL_UNITS');setTimeout('loadDate()', 300);SJV_TB_Check();" bgcolor="rgb(255,255,225)">
            ...
            <table cellspacing="1" cellpadding="2" border="1" width="100%">
               ...
               <tr class="table">
                ...
                <td>
                  <div align="left">
                    ...
                    <select size="1" name="cmbAcc_UnitCode" id="cmbAcc_UnitCode" tabindex="1" onChange="common_LoadOffice_New(this.value);SJV_TB_Check()">
              ...
              <tr class="table">
                ...
                <td>
                  ...
                  String cash_year="";
                  try{
               /*    PreparedStatement ps_dte=con.prepareStatement(" SELECT sup.CASHBOOK_YEAR " +
                		  " FROM FAS_SUPPLEMENT_GJV sup " +
                		  " WHERE STATUS               ='L' " +
                		  " AND sup.cashbook_year NOT IN " +
                		  "  (SELECT MAX(sjv.CASHBOOK_YEAR) " +
                		  "  FROM FAS_TRIAL_BALANCE_STATUS_SJV sjv " +
                		  "  WHERE Sjv.ACCOUNTING_UNIT_ID = " +unitid+
                		  "  )" ); */
                	...
                	/* 	  PreparedStatement ps_dte=con.prepareStatement("SELECT MAX(CASHBOOK_YEAR), " +
                				  "    CASE WHEN MAX(CASHBOOK_YEAR) = " +
                						  "        (SELECT MAX(CASHBOOK_YEAR) FROM FAS_SUPPLEMENT_GJV WHERE STATUS ='L' group by STATUS " +
                								  ...
                								  "    WHERE ACCOUNTING_UNIT_ID =" +unitid+" group by ACCOUNTING_UNIT_ID ");
 ...
 */
 ...
 "    THEN '31/03/' " +
 "		|| coalesce (b.year1 ,extract(YEAR FROM now()))" +
// "      || extract(YEAR FROM now()) " + modified by sathya on 18/06/2018
 "    ELSE '31/03/' " +
 ...
 "    YEAR " +
 "  FROM " +
 "    (SELECT (SUP.CASHBOOK_YEAR) AS YEAR " +
 "    FROM FAS_SUPPLEMENT_GJV SUP " +
 "    WHERE STATUS ='L' " +
 "    order by SUP.CASHBOOK_YEAR " +
 "    )as ps " +
 "  )a " +
 " LEFT OUTER JOIN " +
 "  (SELECT row_number()  over() AS id, " +
 ...
 "  order by a.year");
                  ...
                  %>
                    <input type="text" name="txtCrea_date" id="txtCrea_date" tabindex="3" value="<%=cash_year %>" readonly 
                ...
                <td>
                  <div align="left">
                    <input type="radio" name="rad_sub_CR_DR" id="rad_sub_CR_DR"
                            value="CR" onclick="Acc_HeadCodeValidation();"/>Credit
                                                        &nbsp;&nbsp;&nbsp;&nbsp; 
                    <input type="radio" name="rad_sub_CR_DR" id="rad_sub_CR_DR"
                           checked="checked" value="DR" onclick="Acc_HeadCodeValidation();"/>Debit &nbsp;&nbsp;&nbsp;&nbsp; 
