...
{   
         ...
         {  
        	   var cmbSL_type=common_cmbSL_type;                 
        	   cmbAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;
        	   cmbOffice_code=document.getElementById("cmbOffice_code").value;
           	   if(cmbSL_type==5 )
           	   {
         ...
         {  
	           clearGeneral_Detail();
	           var txtCrea_date= document.frmJournal_Imprest_Cancel.txtCrea_date.value;
	           var cmbAcc_UnitCode= document.frmJournal_Imprest_Cancel.cmbAcc_UnitCode.value;
	        ...
	        //   var  cmbAcc_UnitCode=document.getElementById("cmbAcc_UnitCode").value;alert(cmbAcc_UnitCode);
	           ...
	           {
			            var url="../../../../../Imprest_Journal_Cancel?Command=load_Voucher_No&txtCrea_date="+txtCrea_date+"&cmbAcc_UnitCode="+
         ...
         {  
	           ...
	           if(txtJournalVou_No!="")
	           {
			            var url="../../../../../Imprest_Journal_Cancel?Command=load_Voucher_Details&txtJournalVou_No="+txtJournalVou_No+"&txtCrea_date="+txtCrea_date+"&cmbAcc_UnitCode="+
			            cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"&txtMode_of_creat="+cmbMas_SL_type;   		   		
			            var req=getTransport();
			            req.open("GET",url,true); 
			            req.onreadystatechange=function()
...
function loadcheckCode(baseResponse)
{
		 ...
		 if(flag=="success")
		 {
		        ...
		        if(BalType=="CR")
		        { 		document.frmJournal_Imprest_Cancel.rad_sub_CR_DR[0].checked=true;}
		        else if(BalType=="DR")
		        {
		        		document.frmJournal_Imprest_Cancel.rad_sub_CR_DR[1].checked=true;
		        }
...
{
		 doFunction('Load_SL_Code_grid','null');                
		 var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
		 if(flag=="success")
		 {                    
			 	var hid=baseResponse.getElementsByTagName("hid")[0].firstChild.nodeValue;
...
function clearall()
{
         ...
         var cmbSL_Code=document.getElementById("cmbSL_Code");   
         clear_Combo(cmbSL_Code);   
		 document.frmJournal_Imprest_Cancel.cmdadd.style.display='block';
		 document.frmJournal_Imprest_Cancel.cmdupdate.style.display='none';
		 document.frmJournal_Imprest_Cancel.cmddelete.disabled=true;
}
