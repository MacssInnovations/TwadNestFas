...
function printingPayment(unitcode,offid,yr,mon,recNo)
{
    ...
    document.frmVoucher_list_SL.action=url;
    document.frmVoucher_list_SL.method="POST";
    document.frmVoucher_list_SL.submit();
    return true;
}
/////////-------------------------- end report
...
/// ----------------------------------------------------- for reporting purpose----------------
function printReceipt(unitcode,offid,yr,mon,recNo)
{
    var cmbAcc_UnitCode=unitcode; 
    var cmbOffice_code=offid;
    ...
    "&txtReceipt_No="+txtReceipt_No;
   
    document.frmSelfCheque_ListAll.action=url;
    document.frmSelfCheque_ListAll.method="POST";
    document.frmSelfCheque_ListAll.submit();
    return true;
}
/////////-------------------------- end report

function printAcq(unitcode,offid,yr,mon,cheqnumber,recNo)
{
    var cmbAcc_UnitCode=unitcode; 
    var cmbOffice_code=offid;
    ...
    var txtCB_Year=yr;
    var txtCB_Month=mon;
    var cheqNO=cheqnumber;
    //var drawdate=p_drawdate;
    var txtReceipt_type='ACQ';
    var url="../../../../../Receipt_print.view?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
    "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cheqNO="+cheqNO+
    "&txtReceipt_No="+recNo+"&txtReceipt_type="+txtReceipt_type;
...
function Show(unitcode,offid,yr,mon,cheq_no,recNo)
{
    if (Voucher_list && Voucher_list.open && !Voucher_list.closed) 
    {
       Voucher_list.resizeTo(500,500);
       Voucher_list.moveTo(250,250); 
       Voucher_list.focus();
    }
    else
    {
    ...
    Voucher_list= window.open("../../../../../org/FAS/FAS1/SelfChequeSystem/jsps/Voucher_ListAll.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&cheq_no="+cheq_no+"&recNo="+recNo,"VoucherList","status=1,height=700,width=700,resizable=YES, scrollbars=yes"); 
    Voucher_list.moveTo(250,250);  
    Voucher_list.focus();
...
var Voucher_list_details;

function loadVoucher_Details(unitcode,offid,yr,mon,recNo)
{
    if (Voucher_list_details && Voucher_list_details.open && !Voucher_list_details.closed) 
    {
       Voucher_list_details.resizeTo(500,500);
       Voucher_list_details.moveTo(250,250); 
       Voucher_list_details.focus();
    }
    else
    {
        Voucher_list_details=null
    }
    Voucher_list_details= window.open("../../../../../org/FAS/FAS1/SelfChequeSystem/jsps/Voucher_ListAll_SL.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&recNo="+recNo,"VoucherListDetails","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    Voucher_list_details.moveTo(250,250);  
    Voucher_list_details.focus();
...
function loadAcq_Details(unitcode,offid,yr,mon,recNo,No_Acq_rolls)
{
    if(No_Acq_rolls==0)
    {
        alert("It doesn't have any Acquitance Details");
        return ;
    }
    
    if (Acq_details && Acq_details.open && !Acq_details.closed) 
    {
       Acq_details.resizeTo(500,500);
       Acq_details.moveTo(250,250); 
       Acq_details.focus();
    }
    else
    {
    ...
    Acq_details= window.open("../../../../../org/FAS/FAS1/SelfChequeSystem/jsps/Acq_ListAll.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&recNo="+recNo,"Acq_ListAll","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    Acq_details.moveTo(250,250);  
    Acq_details.focus();
...
{
    if (Voucher_list && Voucher_list.open && !Voucher_list.closed) Voucher_list.close();
    if (Voucher_list_details && Voucher_list_details.open && !Voucher_list_details.closed) Voucher_list_details.close();
    if (Acq_details && Acq_details.open && !Acq_details.closed) Acq_details.close();
...
{   
        ...
        {  
            ...
            {
                var txtCreat_By_Module='SC';
                var url="../../../../../SelfCheque_ListAll.view?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
        ...
        {  
           ...
           if(txtCB_Year.length!=0 && txtCB_Month.length!=0 && txtFrom_date.length!=0 && txtTo_date.length!=0)
            {
                var txtCreat_By_Module='SC';
                var url="../../../../../SelfCheque_ListAll.view?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
              ...
              //alert('test');  
                ...
                /*
                    ...
                    for(i=0;i<leng.length;i++)
                    {
                        ...
                        var items=new Array();
                        items[0]=leng[i].getElementsByTagName("cheq_no")[0].firstChild.nodeValue;
                        items[1]=leng[i].getElementsByTagName("drawled_date")[0].firstChild.nodeValue;
...
{
//alert('test');
var page=document.frmSelfCheque_ListAll.cmbpage.value;
//alert(page);
...
function loadPage(page)
{
         ...
         // alert(page);
            document.frmSelfCheque_ListAll.cmbpage.selectedIndex=page-1;
                var tbody=document.getElementById("tbody");
             ...
             if(service)
                ...
                for(i=p;i<service.length&& c<__pagination;i++)
                {
                     c++;
                        ...
                        items[0]=service[i].getElementsByTagName("cheq_no")[0].firstChild.nodeValue;
                        items[1]=service[i].getElementsByTagName("drawled_date")[0].firstChild.nodeValue;
                      //  items[2]=service[i].getElementsByTagName("Remak")[0].firstChild.nodeValue;
                        ...
                        items[4]=service[i].getElementsByTagName("Rec_no")[0].firstChild.nodeValue;
                      ...
                      for(j=0;j<4;j++)
                        ...
                        cell.align='CENTER';
                        var anc=document.createElement("A");
                        var url="javascript:Show('"+Ucode+"','"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[0]+"','"+items[4]+"')";
                        anc.href=url;
                        var txtedit=document.createTextNode("DETAILS");
                        anc.appendChild(txtedit);
                        cell.appendChild(anc);
                      ...
                      // alert(Ucode);
                         if(document.getElementById("cmbAcc_UnitCode").value==Ucode)
                        {
                            if(document.getElementById("cmbStatus").value=='L')
                            {
                                ...
                                cell.align='CENTER';
                                var anc=document.createElement("A");
                                var url="javascript:printReceipt('"+Ucode+"','"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[4]+"')";
                                anc.href=url;
                                var txtedit=document.createTextNode("PRINT");
                                anc.appendChild(txtedit);
                                cell.appendChild(anc);
                       ...
                       if(document.getElementById("cmbAcc_UnitCode").value==Ucode)
                        {
                            if(document.getElementById("cmbStatus").value=='L')
                            {
                                ...
                                cell.align='CENTER';
                                var anc=document.createElement("A");
                                //var url="javascript:printAcq('"+Ucode+"','"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[4]+"')";
                                var url="javascript:printAcq('"+Ucode+"','"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[0]+"','"+items[4]+"')";
                                anc.href=url;
                                var txtedit=document.createTextNode("PRINT");
                                anc.appendChild(txtedit);
                                cell.appendChild(anc);
...
function changepagesize()
{
           ...
           __pagination=document.frmSelfCheque_ListAll.cmbpagination.value;
...
if(v)
{
    for(i=0;i<v.length;i++)
    {
        if(v[i].checked==true)
        {
           // opener.document.frmSelfCheque_ListAll.txtEmployeeid.value = (v[i].value);
