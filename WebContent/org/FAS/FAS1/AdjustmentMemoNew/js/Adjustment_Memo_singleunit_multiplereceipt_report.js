...
function doFunction_voucher(command,param)
{
		...
		{
			var url="../../../../../SingleUnit_multipleReceipt_Report?Command=load_Voucher_No&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"&txtCrea_date="+txtCrea_date+"&office_id="+office_id+"&advice_type="+advice_type;
			//alert(url);
		...
		else if(command=="load_Receipt_Details")
		{
			...
			//var cmbvocharNo=vspl[0];
			//var serialno=vspl[1];
			var url="../../../../../SingleUnit_multipleReceipt_Report?Command=load_details&cmbAcc_UnitCode="+cmbAcc_UnitCode
			+"&cmbOffice_code="+cmbOffice_code+"&txtCrea_date="+txtCrea_date+"&cmbvocharNo="+cmbvocharNo;                                    
//            alert(url);
			var req=getTransport();
            req.open("GET",url,true); 
            req.onreadystatechange=function()
		   ...
		   if(req.status==200)
	          {
	                ...
	                else if(command=="load_details")
                    { 
	                	details_check(baseResponse);
                    }
	                else if(command=="Add")
	                {
...
function addresponse(baseResponse)
{
	//alert("enter"+baseResponse);
	 var val=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
	//alert(val)
	 if(val=="success")
	 {
		 alert("Record inserted Successfully");
		 document.forms[0].butSub.disabled=true;
...
function receiptNoCheck(baseResponse)
{
    ...
    document.forms[0].cmbvocharNo.length=0;
    var receiptnumber=baseResponse.getElementsByTagName("memono");
   // var sl_no=baseResponse.getElementsByTagName("sl_no");
    var amount=baseResponse.getElementsByTagName("amount");
   /* var town=baseResponse.getElementsByTagName("town");*/
    
    var opt=document.createElement('option');
    opt.text="--Select Voucher No--";
    opt.value="";
    receiptno.appendChild(opt);
	    ...
	    {
	    	/*var val=receiptnumber[i].firstChild.nodeValue+"-"+sl_no[i].firstChild.nodeValue
	    	+"-"+amount[i].firstChild.nodeValue+"-"+town[i].firstChild.nodeValue;*/
	    	
	    	
	    	var val=receiptnumber[i].firstChild.nodeValue;
	        var opt=document.createElement('option');
	        opt.value=val;
	        opt.innerHTML=val;
	        receiptno.appendChild(opt);
	    }
	   // var ttlamt=baseResponse.getElementsByTagName("ttlamt")[0].firstChild.nodeValue;
	    
	    //document.forms[0].txtTotalAmt.value=ttlamt;
...
function details_check(baseResponse)
{
	var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue
	if(flag=="success")
	{
	var totalAmt=baseResponse.getElementsByTagName("amount")[0].firstChild.nodeValue;
	 var particulars=baseResponse.getElementsByTagName("remarks")[0].firstChild.nodeValue;
	 var authorityname=baseResponse.getElementsByTagName("authname")[0].firstChild.nodeValue;
	 var authorityaddress=baseResponse.getElementsByTagName("authadress")[0].firstChild.nodeValue;
	 var letterno=baseResponse.getElementsByTagName("lno")[0].firstChild.nodeValue;
	 var letterdate=baseResponse.getElementsByTagName("ldate")[0].firstChild.nodeValue;
...
function checkNull()
{
	...
	var name=document.getElementById("txtempName").value;
	 var flag = name.includes("-");
	 if(!flag)
		 {
		 alert("Enter Name and Designation like EmpName-Designation");          
         return false;   
		 }
        ...
        if(document.getElementById("cmbvocharNo").value==''  || document.getElementById("cmbvocharNo").value==0)
        {
            alert("Select the cmbvocharNo");          
            return false;    
        }
//        if(document.getElementById("txtempCode").value==''  || document.getElementById("txtempCode").value==0 )
//        {
//            alert("Enter the empCode");          
//            return false;    
//        }
...
function call_clr()
{
	...
	//txtempName
	    ...
	    document.getElementById("cmbvocharNo").selectedIndex=0;
...
function add()
{
	 ...
	 //var btnvalue=document.forms[0].butSub.value;
	 ...
	 var cospl=(document.forms[0].cmbvocharNo.value).split("-");
	 var cmbvocharNo=cospl[0];
	//	var serialno=cospl[1];
	 var particulars=document.forms[0].particulars.value;
	 var txtTotalAmt=document.forms[0].txtTotalAmt.value;
	 var txtOfficeAddress=document.forms[0].txtOfficeAddress.value;
	
	 
	 var url="../../../../../SingleUnit_multipleReceipt_Report?Command=Add&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+
	 "&txtCrea_date="+txtCrea_date+"&cmbvocharNo="+cmbvocharNo+"&particulars="+particulars+
	 "&txtTotalAmt="+txtTotalAmt+"&txtOfficeAddress="+txtOfficeAddress;
...
function report()
{
	if(checkNull())
       {
	 var txtCrea_date=document.forms[0].txtCrea_date.value;
	 var cmbsplitt=document.forms[0].cmbvocharNo.value;
	 var cmbvocharNo=cmbsplitt.split("-");
	 var rpttype=document.getElementById("butSub1").value;
	 var signAutSplit=document.getElementById("txtempName").value;
	 var signAut=signAutSplit.split("-");
	
	 
	 var url="../../../../../SingleUnit_multipleReceipt_Report1?Command=REPORT&txtCrea_date="+txtCrea_date+"&cmbvocharNo="+cmbvocharNo[0]+"&rpttype="+rpttype+"&signAutName="+signAut[0]+"&signAutDesg="+signAut[1];
	// alert(url);
	  document.Adjustment_Memo_Form.action=url;
      document.Adjustment_Memo_Form.submit();
        }
}
function report1()
{
	if(checkNull())
	 ...
	 var cmbsplitt=document.forms[0].cmbvocharNo.value;
	 var cmbvocharNo=cmbsplitt.split("-");
	 var rpttype=document.getElementById("butSub2").value;
	 var signAutSplit=document.getElementById("txtempName").value;
	 var signAut=signAutSplit.split("-");
	// alert(signAutSplit[0]+"--"+signAutSplit[1]);
	 var url="../../../../../SingleUnit_multipleReceipt_Report1?Command=REPORT&txtCrea_date="+txtCrea_date+"&cmbvocharNo="+cmbvocharNo[0]+"&rpttype="+rpttype+"&signAutName="+signAut[0]+"&signAutDesg="+signAut[1];
	 //alert(url);
	  document.Adjustment_Memo_Form.action=url;
      document.Adjustment_Memo_Form.submit();
...
function Load_emp_details()
{
        ...
        var emp_id=document.getElementById("txtempCode").value;
        var url="";
             url="../../../../../SingleUnit_multipleReceipt_Report1?command=loadempdetails&emp_id="+emp_id;
             //alert(url);
             var req=getTransport();
              req.open("GET",url,true);        
              req.onreadystatechange=function()
...
function LoadEmpDetails(baseResponse)
{
                  ...
                  {                       
                         ...
                         var office_name=baseResponse.getElementsByTagName("office_name")[0].firstChild.nodeValue;
                                document.Adjustment_Memo_Form.txtempName.value=emp_name+"-"+desig_name;
                }
                else if(flag=="nodata")
                {
