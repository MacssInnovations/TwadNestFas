...
function Show(unitcode,offid,yr,mon,vocNo,for_unit,accep_status)
{
...
//	alert("for_unit==>"+for_unit);
    ...
    Receipt_list_SL= window.open("../../../../../org/FAS/FAS1/AdjustmentMemoNew/jsps/Adj_Memo_MultipleReceipt_DetailPart.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&vocNo="+vocNo+"&for_unit="+for_unit+"&accep_status="+accep_status,"SingleReceiptList","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    Receipt_list_SL.moveTo(250,250);  
    Receipt_list_SL.focus();
...
{   
        ...
        {  
            ...
            {
	                var url="../../../../../Adj_Memo_MultipleReceipt_List?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
        ...
        {  
           ...
           if(txtCB_Year.length!=0 && txtCB_Month.length!=0 && txtFrom_date.length!=0 && txtTo_date.length!=0)
            {
                var url="../../../../../Adj_Memo_MultipleReceipt_List?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
...
function loadTable(baseResponse)
{
                ...
                {   
	                    ...
	                    if(service)
	                    {
                               ...
                               FOR_ACCOUNTING_UNIT_ID=baseResponse.getElementsByTagName("FOR_ACCOUNTING_UNIT_ID")[0].firstChild.nodeValue;
                               ACCEPTANCE_STATUS=baseResponse.getElementsByTagName("ACCEPTANCE_STATUS")[0].firstChild.nodeValue;
...
function loadPage(page)
{
            ...
            if(service)
		    {
		                ...
		                for(i=p;i<service.length&& c<__pagination;i++)
		                {
		                       ...
		                       if(service[i].getElementsByTagName("OFFICE_NAME")[0] == undefined){
								   items[2] = ''
							   }else {
		                        ...
		                        var url="javascript:Show('"+Ucode+"','"+Offid+"','"+txtCB_Year1+"','"+txtCB_Month1+"','"+items[0]+"','"+FOR_ACCOUNTING_UNIT_ID+"','"+ACCEPTANCE_STATUS+"')";
		                        anc.href=url;
