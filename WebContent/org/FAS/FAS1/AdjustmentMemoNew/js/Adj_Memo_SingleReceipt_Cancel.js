...
function doFunction_voucher(command,param)
{
		...
		{
			var url="../../../../../Adj_Memo_SingleReceipt_Cancel?Command=load_Voucher_No&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"&txtCrea_date="+txtCrea_date;
//			alert(url);
		...
		else if(command=="load_Receipt_Details")
		{
			...
			var url="../../../../../Adj_Memo_SingleReceipt_Cancel?Command=load_Ref_No&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cmbvocharNo="+cmbvocharNo;                                    
            var req=getTransport();
            req.open("GET",url,true); 
            req.onreadystatechange=function()
            {
...
function RefNoCheck(baseResponse)
{
	...
	if(flag=="success")
	{
	 ...
	 if(  particulars=baseResponse.getElementsByTagName("particulars")[0]=null)
	 var particulars=baseResponse.getElementsByTagName("particulars")[0].firstChild.nodeValue;
	 if(  authorityname=baseResponse.getElementsByTagName("authorityname")[0]=null)
	 var authorityname=baseResponse.getElementsByTagName("authorityname")[0].firstChild.nodeValue;
	  if(authorityaddress=baseResponse.getElementsByTagName("authorityaddress")[0]=null)
	...
	// alert(len);
	 ...
	 if(remaks=baseResponse.getElementsByTagName("remaks")[k]=null)
...
function check_TB(req,dateCtrl)
{
	    if(req.readyState==4)
	    {
		        ...
		        {  
			             ...
			             var Command=tagcommand.firstChild.nodeValue;
			             var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
			             var CreateDate=document.Adjustment_Memo_SingleReceipt.txtCrea_date.value;
			             var Date=CreateDate.split("/");
			            // alert("Day" + Date[0] + "Month" + Date[1] + "Year" +Date[2]);
			             if(flag=="success")
			             {
			            	 document.Adjustment_Memo_SingleReceipt.txtCB_Year.value=Date[2];
			                 document.Adjustment_Memo_SingleReceipt.txtCB_Month.value=Date[1];
