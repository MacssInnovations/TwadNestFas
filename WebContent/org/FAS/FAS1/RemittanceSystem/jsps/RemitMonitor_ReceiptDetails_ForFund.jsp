           ...
           try 
            {
                String sqlps="";
                   ...
                   if(cmbAcc_UnitCode ==5)
                         sqlps="select a.RECEIPT_NO,a.RECEIPT_DATE as rec_date, " +
                         "trim(c.ACCOUNTING_UNIT_NAME) || ' -' || d.OFFICE_NAME   as RECEIVED_FROM, a.CHEQUE_DD_NO," +
                         " a.CHEQUE_DD_DATE as cheqDD_date,b.BANK_NAME, " +
                         "trim(to_char(a.TOTAL_AMOUNT,'99999999999999.99')) as  AMOUNT from FAS_FUND_RECEIPT_BY_HO a,FAS_MST_BANKS b," +
                         "FAS_MST_ACCT_UNITS c,COM_MST_OFFICES d  where  RECEIPT_STATUS!='C' and   " +
                         "a.OFFICE_BANK_ID=b.BANK_ID  and a.RECEIVED_FROM_OFFICE_ID=d.OFFICE_ID " +
                         " and a.RECEIVED_FROM_HO_UNIT_ID=c.ACCOUNTING_UNIT_ID " +
                         "and a.ACCOUNTING_UNIT_ID=? and  a.ACCOUNTING_FOR_OFFICE_ID=? and  a.CHALLAN_NO=? and to_char(a.CHALLAN_DATE,'DD-MON-YY')=? ";

                    else
                        sqlps="select a.RECEIPT_NO,to_char(a.RECEIPT_DATE,'DD/MM/YYYY') as rec_date, " +
                         ...
                         "  where  RECEIPT_STATUS!='C' and   " +
                         "a.HO_BANK_ID=b.BANK_ID  " +
                         "and a.ACCOUNTING_UNIT_ID=? and  a.ACCOUNTING_FOR_OFFICE_ID=? and a.CHALLAN_NO=? and to_char(a.CHALLAN_DATE,'DD-MON-YY')=? ";
                      System.out.println("sqlps::::::::::::"+sqlps);   
                    ...
                    while(rs.next()) 
                    {
                      ...
                      out.println("<td align='left'>"+rs.getString("AMOUNT")+"</td></tr>");
