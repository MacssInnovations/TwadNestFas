...
function Show(unitcode,offid,challanNo,challanDate)
{
    ...
    loadRemit_ReceiptDetails= window.open("../../../../../org/FAS/FAS1/RemittanceSystem/jsps/RemitMonitor_ReceiptDetails_ForBank.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&challanNo="+challanNo+"&challanDate="+challanDate,"ReceiptDetails","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    loadRemit_ReceiptDetails.moveTo(250,250);  
    loadRemit_ReceiptDetails.focus();
...
{   
        ...
        {  
            ...
            {
                var txtRemittance_type='B';
                var url="../../../../../BankRemit_ListAll.view?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtRemittance_type="+txtRemittance_type;
               // alert(url);
        ...
        {  
           ...
           if(txtCB_Year.length!=0 && txtCB_Month.length!=0 && txtFrom_date.length!=0 && txtTo_date.length!=0)
            {
                var txtRemittance_type='B';
                var url="../../../../../BankRemit_ListAll.view?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+
                txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtFrom_date="+txtFrom_date+"&txtTo_date="+txtTo_date+"&txtRemittance_type="+txtRemittance_type;
                //alert(url);
                var req=getTransport();
                req.open("GET",url,true); 
                req.onreadystatechange=function()
...
function loadTable(baseResponse)
{
                ...
                {   
                    ...
                    var Offid=document.getElementById("cmbOffice_code").value;
                     var Challan_no=baseResponse.getElementsByTagName("Challan_no");
                    var seq=0;
                    for(k=0;k<Challan_no.length;k++)
                    {
                      
                         //items[0]=baseResponse.getElementsByTagName("pay_no")[k].firstChild.nodeValue;  //  <!-- Payment Voucher number -->
                         //items[1]=baseResponse.getElementsByTagName("pay_date")[k].firstChild.nodeValue; 
                         items[2]=baseResponse.getElementsByTagName("Challan_no")[k].firstChild.nodeValue;   
                         items[3]=baseResponse.getElementsByTagName("challan_date")[k].firstChild.nodeValue;   //******* emitted on is filled with challan date, bcoz both are same
                         items[4]=baseResponse.getElementsByTagName("Amount")[k].firstChild.nodeValue;
                         items[5]=baseResponse.getElementsByTagName("userName")[k].firstChild.nodeValue; 
                       // alert(items[2])
                        seq=parseInt(seq)+1;
                        ...
                        var url="javascript:Show('"+Ucode+"','"+Offid+"','"+items[2]+"','"+items[3]+"')";
                        anc.href=url;
                ...
                for(i=p;i<service.length&& c<__pagination;i++)
                {
                     c++;
                        ...
                        items[2]=service[i].getElementsByTagName("Challan_no")[0].firstChild.nodeValue;   
                         items[3]=service[i].getElementsByTagName("challan_date")[0].firstChild.nodeValue;   //******* emitted on is filled with challan date, bcoz both are same
