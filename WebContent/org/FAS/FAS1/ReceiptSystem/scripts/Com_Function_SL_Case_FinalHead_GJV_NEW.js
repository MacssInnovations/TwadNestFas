    ...
    var txtCrea_date=document.getElementById("txtCrea_date").value;
        ...
        {  
         ...
         // alert("haiiii");
           ...
           {
               var url="../../../../../Receipt_SL_FinalHead_GJV_New.kv?Command=checkCode_BLOCK&txtAcc_HeadCode="+txtAcc_HeadCode+"&cmbOffice_code="+cmbOffice_code+"&vr_date="+txtCrea_date+"&cmbAcc_UnitCode="+cmbAcc_UnitCode;                
    ...
    var cmbOffice_code=document.getElementById("cmbOffice_code").value;
        ...
        {  
          ...
          // alert("haiiii");
            ...
            if(txtAcc_HeadCode.length>=6)
            {
                var url="../../../../../Receipt_SL_FinalHead_GJV.kv?Command=checkCode&txtAcc_HeadCode="+txtAcc_HeadCode+"&cmbOffice_code="+cmbOffice_code;                
                var req=getTransport();
                req.open("GET",url,true); 
                req.onreadystatechange=function()
                {
          ...
          if(cmbSL_type!="")                              // called only not equal to null
            {
               ...
               //  alert(txtAcc_HeadCode);
                 ...
                 "&other_dept_off_alias_id="+other_dept_off_alias_id+"&cmbAcc_UnitCode="+cmbAcc_UnitCode+
                 "&cmbOffice_code="+cmbOffice_code+"&addtional_field_value="+addtional_field_value+"&txtAcc_HeadCode="+txtAcc_HeadCode;
              //  alert(url);
                var req=getTransport();
                req.open("GET",url,true); 
                req.onreadystatechange=function()
          ...
          if(cmbSL_type!="")                              // called only not equal to null
            {
                 ...
                 var url="../../../../../Receipt_SL_FinalHead_GJV.kv?Command=Load_SL_Code1&cmbSL_type="+cmbSL_type+"&cmbMas_SL_type="+cmbMas_SL_type+
                 "&other_dept_off_alias_id="+other_dept_off_alias_id+"&cmbAcc_UnitCode="+cmbAcc_UnitCode+
                 "&cmbOffice_code="+cmbOffice_code+"&addtional_field_value="+addtional_field_value;
                //alert(url);
                var req=getTransport();
...
{  
    ...
    { 
        ...
        {  
            ...
            if(Command=="checkCode" || Command == "checkCode_BLOCK")
            {
...
function loadcheckCode(baseResponse)
{
    ...
    var Crea_date=document.getElementById("txtCrea_date").value;
    //alert("Crea_date==>"+Crea_date);
    var date=Crea_date.split("/");
    var mm=date[1];
    //alert("Month==>"+mm.toString());
    var yy=date[2];
    //alert("Year==>"+yy.toString());
    ...
    if(flag=="success")
    {
    	...
    	var unit_id=baseResponse.getElementsByTagName("verifyunit_id")[0].firstChild.nodeValue;
    	var office_id=baseResponse.getElementsByTagName("verifiedoffice_id")[0].firstChild.nodeValue;
    	var hcode=document.getElementById("txtAcc_HeadCode").value;
    	// added on 23-Jan-19 by kanaga
...
//    	alert("cmbAcc_UnitCode>>>>>>"+cmbAcc_UnitCode);
    	...
    	var major_hd=baseResponse.getElementsByTagName("MAJOR_HEAD_CODE")[0].firstChild.nodeValue;
    	//hcode!="550310"
    	if((major_hd=="L" && sl_man=="Y")){
    		alert("Account Head Code '"+document.getElementById("txtAcc_HeadCode").value+"' not Allowed here ");
...
//    	alert("Verified Units---->"+unit_id);
//    	alert("Verified Office------>"+office_id);
//    	
//    	alert("cmbAcc_UnitCode====>"+cmbAcc_UnitCode);
     ...
     try{   
       ...
       if(SL_YN=="Y")
       {
            ...
            for(var k=0;k<SLCODE.length;k++)
            {
                ...
                //alert("SLCODE--->"+items_SLcode[k]);
               
                //alert("inside--->"+mm);
                
                if(unit_id!=cmbAcc_UnitCode && office_id!=cmbOffice_code)
                {
                if(mm>=4 && yy>=2018)
                {
                if(items_SLcode[k]==10)
                	{
                	alert("Account Head Code '"+document.getElementById("txtAcc_HeadCode").value+"' not Allowed here ");
                    document.getElementById("txtAcc_HeadCode").value="";
                    document.getElementById("txtAcc_HeadDesc").value="";
           ...
           // alert("<<<>>>>>"+SLCODE.length);
            ...
            {   
              var option=document.createElement("OPTION");
              option.text=items_SLdesc[k]+"("+items_SLcode[k]+")";
           ...
           // alert("be4");
           // alert("common_cmbSL_type:::"+cmbSL_type);
