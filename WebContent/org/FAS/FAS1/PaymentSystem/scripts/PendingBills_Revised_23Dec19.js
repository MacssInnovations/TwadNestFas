    ...
    var doc=window.opener.document;             // get the parent form
      ...
      var unitcode=doc.frmBankPay_PendingBill_create.cmbAcc_UnitCode.value ;    
        var offid=doc.frmBankPay_PendingBill_create.cmbOffice_code.value
          var dateval=doc.frmBankPay_PendingBill_create.txtCrea_date.value;    // used to get cash book month and year 
     ...
     /*   if(val!="")
        {
            if(val==550401 && type_MasSL!=9)
            {
                alert("select 'Other Departments'  Sub-Ledger type in General details part");
            ...
            var url="../../../../../PendingBills_Revised.view?cmbAcc_HeadCode="+val+"&cmbMaS_SL_code="+code_MasSL+"&cmbMas_SL_type="+type_MasSL+
       ...
       // document.getElementById("img1").style.visibility="visible";
         ...
         {
          var url="../../../../../PendingBills_Revised.view?cmbAcc_HeadCode="+val+"&cmbMaS_SL_code="+code_MasSL+"&cmbMas_SL_type="+type_MasSL+
            "&txtCrea_date="+dateval+"&cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&frmDate="+frmDate+"&toDate="+toDate;
            var req=getTransport();
	...
	for(var i=1;i<=tot;i++)
		{
		  ...
		  /*  if(i==cur){
		    	 get_cells.item(9).lastChild.nodeValue=
		    }*/
		     if(i!=cur){
			  get_cells.item(10).firstChild.disabled=true;
		  }
...
function Changebtext(cur_val,ori_val,cur,tot){
	...
	if(ori_val<cur_val)
		...
		document.getElementById(cur).cells.item(10).firstChild.value="";
		
		  var nex=document.getElementById(cur).cells.item(9).firstChild.nextSibling;
          nex=nex.nextSibling;
            nex=nex.nextSibling;
            nex=nex.nextSibling;
            nex=nex.nextSibling;
	...
	for(var i=1;i<=tot;i++)
	{
	  var getrow=document.getElementById(i);
	    ...
	    if(i==cur){
	    	...
	    	if(ori_val!=cur_val)
			{
	    	var nex= get_cells.item(9).firstChild.nextSibling;
             nex=nex.nextSibling;
               nex=nex.nextSibling;
               nex=nex.nextSibling;
               nex=nex.nextSibling;
        ...
        if(req.status==200)
        {
             ...
             var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
                ...
                if(flag=="success")
                {
                    ...
                    items[22]=baseResponse.getElementsByTagName("JOURNAL_TYPE_CODE")[k].firstChild.nodeValue;
                    items[23]=baseResponse.getElementsByTagName("vrAMOUNT")[k].firstChild.nodeValue;
                    items[24]=baseResponse.getElementsByTagName("MULTIPLE_PVR_DETAILS")[k].firstChild.nodeValue;
                   ...
                   vou_no.setAttribute("onchange", "checkboxAlert()");
                   /*@NK included for onchange function*/
                  ...
                  cell2=document.createElement("TD");
                    ...
                    cell2=document.createElement("TD");                             // added on 27th April 07
                         cell2.align="CENTER";
                             ...
                             /* cell2.style.display="none";*/  /*<!-- @NK HIDE ON 01072019 -->*/
                    ...
                    /*                                                              within the star indicates to avoid the column added in b/w two cells
                        ...
                        cell2=document.createElement("TD");
                         ...
                         cell2=document.createElement("TD");
                              ...
                              var MULTIPLE_PVR_DETAILS=document.createElement("input");           // Particulars Added to grid b4 the Amount Text Node but after  amount text box    
                              MULTIPLE_PVR_DETAILS.type="hidden";
                              MULTIPLE_PVR_DETAILS.name="MULTIPLE_PVR_DETAILS";
                              MULTIPLE_PVR_DETAILS.value=items[24];
                             ...
                             // mycurrent_row.appendChild(cell2);
                              ...
                              Vr_amt.setAttribute("onclick","disabtext("+seq+","+cnt+");");
                           
                              if(part=='N'){
                            	   Vr_amt.setAttribute("disabled", true);
                              }
                              Vr_amt.setAttribute("onchange","Changebtext(this.value,"+items[23]+","+seq+","+cnt+");");
                              Vr_amt.value=items[23];
 ...
 function btnsubmit()
    {
        ...
        var sele=document.getElementsByName("vou_no").length;       //"vou_no" is check box name
        var val=0;
        //var vouNO[20][],vouDATE[20][],vouTYPE[20][],AcHeadCode[20][],AcHeadName[20][],CR_DB_indic[20][],subLed_type[20][],subLed_typeName[20][];
        //var subLed_code[20][],subLed_codeName[20][],cheque_dd[20][],cheque_dd_No[20][],cheque_dd_date[20][],billNO[20][],billdate[20][],billtype[20][];
        //var agree_NO[20][],agree_date[20][],paid_to[20][],subamount[20][],particular[20][];
      ...
      //  alert(sele);
            ...
            if(sele==1)
            {
                ...
                if(document.frmPending_Bills_revised.vou_no.checked==true)
                {
                 //vouNO[0]=document.frmPending_Bills_revised.vou_no.value;
                                r=document.getElementById(document.frmPending_Bills_revised.vou_no.value);      // choose the particular row
                                rcells=r.cells;
                                
                                vouNO[0]=rcells.item(3).firstChild.value;
                                vouSL_NO[0]=rcells.item(4).firstChild.value;
                                vouDATE[0]=rcells.item(2).firstChild.value;
                                vouTYPE[0]=rcells.item(1).firstChild.value;
                                
                                AcHeadCode[0]=rcells.item(5).firstChild.value;
                                AcHeadName[0]=rcells.item(5).lastChild.nodeValue;
                                CR_DB_indic[0]=rcells.item(6).firstChild.value;
                                subLed_type[0]=rcells.item(7).firstChild.value;
                                subLed_typeName[0]=rcells.item(7).lastChild.nodeValue;
                                subLed_code[0]=rcells.item(8).firstChild.value;
                                subLed_codeName[0]=rcells.item(8).lastChild.nodeValue;
                               ...
                               // cheque_dd_date[0]=rcells.item(9).firstChild.value;
                                billNO[0]=rcells.item(9).firstChild.value;
                                var nex=rcells.item(9).firstChild.nextSibling;
                                ...
                                nex=nex.nextSibling;
                                MVR_no[0]=nex.value; 
                                vrAmt[0]=rcells.item(10).firstChild.value;
                    ...
                    { 
                        ...
                        if(document.frmPending_Bills_revised.vou_no[i].checked==true)
                        {
                               // vouNO[j]=document.frmPending_Bills_revised.vou_no[i].value;
                                r=document.getElementById(document.frmPending_Bills_revised.vou_no[i].value);
                                rcells=r.cells;
                                
                                vouNO[j]=rcells.item(3).firstChild.value;
                                vouSL_NO[j]=rcells.item(4).firstChild.value;
                             ...
                             // alert("vouTYPE[j] "+vouTYPE[j]);
                              //alert("vouTYPE[j] "+vouNO[j]);
                                ...
                                AcHeadCode[j]=rcells.item(5).firstChild.value;
                                AcHeadName[j]=rcells.item(5).lastChild.nodeValue;
                                CR_DB_indic[j]=rcells.item(6).firstChild.value;
                                
                                subLed_type[j]=rcells.item(7).firstChild.value;
                                subLed_typeName[j]=rcells.item(7).lastChild.nodeValue;
                                subLed_code[j]=rcells.item(8).firstChild.value;
                                subLed_codeName[j]=rcells.item(8).lastChild.nodeValue;
                               ...
                               // cheque_dd_date[j]=rcells.item(9).firstChild.value;
                                billNO[j]=rcells.item(9).firstChild.value;
                                var nex=rcells.item(9).firstChild.nextSibling;
                                ...
                                nex=nex.nextSibling;
                                MVR_no[j]=nex.value; 
                                vrAmt[j]=rcells.item(10).firstChild.value;
        ...
        if(tot_amount<0){
       	alert("Debit Amount should be greater than Credit Amount .. ");
        	...
        	//alert("flag"+flag);
        opener.doParentPendingbills(vouNO,vouSL_NO,vouDATE,vouTYPE,AcHeadCode,AcHeadName,CR_DB_indic,subLed_type,subLed_typeName,subLed_code,subLed_codeName,billNO,billdate,billtype,agree_NO,agree_date,paid_to,subamount,particular,vrAmt,MULTIPLE_PVR_DETAILS,MVR_no,j,flag);
        window.history.go();
...
/*  var VOUCHER_NO=baseResponse.getElementsByTagName("VOUCHER_NO");
   ...
   tbody=document.getElementById("grid_body");
                       ...
                       Vou_no.id="Vou_no";
                       Vou_no.name="Vou_no";
                       Vou_no.value=items[0];
                       cell2.appendChild(Vou_no);
                       mycurrent_row.appendChild(cell2); 
...
function loading_Journal_Details()
{
    var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
    if(flag=="success")
    {
        ...
        for(var k=0;k<VOUCHER_NO.length;k++)
        {
           ...
           cell2=document.createElement("TD");
           var VOUCHER_NO=document.createElement("input");
           VOUCHER_NO.type="checkbox";
           VOUCHER_NO.id="VOUCHER_NO";
           VOUCHER_NO.name="VOUCHER_NO";
...
function btnsubmit()
    {
        ...
        var vouNO,vouDATE,vouTYPE,AcHeadCode,AcHeadName,CR_DB_indic,subLed_type,subLed_typeName;
        var subLed_code,subLed_codeName,cheque_dd,cheque_dd_No,cheque_dd_date,billNO,billdate,billtype;
        var agree_NO,agree_date,paid_to,subamount,particular;
        vouNO=new Array();
        vouDATE=new Array();
        vouTYPE=new Array();
        AcHeadCode=new Array();
        ...
        if(sele>0)
        {
            if(sele==1)
            {
                ...
                if(document.frmPending_Bills_revised.vou_no.checked==true)
                {
                 ...
                 r=document.getElementById(vou_no);
                 rcells=r.cells;
                    ...
                    particular[0][21]=rcells.item(21).firstChild.value;
                }
            }
            else
            {      //alert("else"+sele); 
                    ...
                    { 
                        ...
                        if(document.frmPending_Bills_revised.vou_no[i].checked==true)
                        {
                                ...
                                r=document.getElementById(vou_no[i]);
                                rcells=r.cells;
                                ...
                                particular[i][21]=rcells.item(3).firstChild.value;
                                i++;
                        }
                    }
             }
...
*/
 ...
 function sel()
 {
	  ...
	  checkboxAlert();/*@NK Included on 26062019*/
 }
    	...
    	else
    	{
    		...
    		for(var i=0;i<sele;i++)
            {
    			...
    			if(document.frmPending_Bills_revised.vou_no[i].checked==true)
                {
                     ...
                     vouTYPE[j]=rcells.item(1).firstChild.value; 
                     if(vouTYPE[j]=='GJV'){checkedvouTYPE.push("JV");}   
                     else if(vouTYPE[j]=='LJV'){checkedvouTYPE.push("JV");} 
                     else if(vouTYPE[j]=='TDCAJ'){checkedvouTYPE.push("JV");}
                     else{checkedvouTYPE.push(vouTYPE[j]);}
