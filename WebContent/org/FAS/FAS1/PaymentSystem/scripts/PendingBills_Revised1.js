...
function GetVoucherInfo(val) {
	// alert('1');
	document.frmPending_Bills_revised.selOff.value=" ";
...
//		var url = "../../../../../PendingBills_Revised.view?Command=get_Voucher&cmbAcc_HeadCode="
...
//				+ "&toDate=" + toDate;
		var url = "../../../../../PendingBills_Revised1.view?Command=get_Voucher&cmbAcc_HeadCode="
			+ val + "&cmbMaS_SL_code=" + code_MasSL + "&cmbMas_SL_type="
			+ type_MasSL + "&txtCrea_date=" + dateval + "&cmbAcc_UnitCode="
			+ unitcode + "&cmbOffice_code=" + offid + "&frmDate=" + frmDate
			+ "&toDate=" + toDate;
...
function Changebtext(cur_val, ori_val, cur, tot) {
	...
	if (ori_val < cur_val) {
		...
		document.getElementById(cur).cells.item(12).firstChild.value = "";

		var nex = document.getElementById(cur).cells.item(11).firstChild.nextSibling;
		nex = nex.nextSibling;
		nex = nex.nextSibling;
		nex = nex.nextSibling;
		nex = nex.nextSibling;
		...
		for (var i = 1; i <= tot; i++) {
			...
			if (i == cur) {
				...
				if (ori_val != cur_val) {
					var nex = get_cells.item(11).firstChild.nextSibling;
					nex = nex.nextSibling;
					nex = nex.nextSibling;
					nex = nex.nextSibling;
					nex = nex.nextSibling;
			...
			if (flag == "success") {
				...
				for (var k = 0; k < AHcode.length; k++) {
					...
					if(baseResponse.getElementsByTagName("desc_type")[k]!=undefined)
						...
						items[6] ="";
					/*if(items[6] = baseResponse.getElementsByTagName("desc_type")[k]=null)
					items[6] = baseResponse.getElementsByTagName("desc_type")[k].firstChild.nodeValue;
					if (items[6] == "null")
						items[6] = "";*/
					...
					if(baseResponse.getElementsByTagName("Bill_date")[k].firstChild==null){
						items[11] = "";
					}else{
					...
					if(baseResponse.getElementsByTagName("Agree_date")[k].firstChild==null){
						items[14] = "";
					}else{
					...
					items[25] = baseResponse.getElementsByTagName("MULTIPLE_PVR_DETAILS")[k].firstChild.nodeValue;
					}
					/*@NK on 22Jun2020*/
					items[26] = baseResponse
					.getElementsByTagName("OFFICE_ID")[k].firstChild.nodeValue;
					if (items[26] == "null")
						items[26] = "";
					/*@NK on 22Jun2020*/
	              ...
	              /*@NK ON 22JUN2020*/
					...
					Off_Id.type = "hidden";
					Off_Id.name = "Office_Id";
					Off_Id.id = "Off_Id" + seq;
					Off_Id.value = items[26];
					cell2.appendChild(Off_Id);
					var currentText = document.createTextNode(items[26]);
					cell2.appendChild(currentText);
					mycurrent_row.appendChild(cell2);					
					...
					MULTIPLE_PVR_DETAILS.id = "MULTIPLE_PVR_DETAILS" + seq;
...
function btnsubmit() {
	...
	if (sele > 0) {
		...
		if (sele == 1) {
			...
			if (document.frmPending_Bills_revised.vou_no.checked == true) {
				...
				seloff[0] = rcells.item(6).firstChild.value;
				AcHeadCode[0] = rcells.item(7).firstChild.value;
				AcHeadName[0] = rcells.item(7).lastChild.nodeValue;
				CR_DB_indic[0] = rcells.item(8).firstChild.value;
				subLed_type[0] = rcells.item(9).firstChild.value;
				subLed_typeName[0] = rcells.item(9).lastChild.nodeValue;
				subLed_code[0] = rcells.item(10).firstChild.value;
				subLed_codeName[0] = rcells.item(10).lastChild.nodeValue;
				...
				billNO[0] = rcells.item(11).firstChild.value;
				var nex = rcells.item(11).firstChild.nextSibling;
				...
				nex = nex.nextSibling;
				MVR_no[0] = nex.value;
				vrAmt[0] = rcells.item(12).firstChild.value;
			...
			for (var i = 0; i < sele; i++) {
				...
				if (document.frmPending_Bills_revised.vou_no[i].checked == true) {
					...
					if (vouTYPE[j] == 'GJV')
						...
						dr_count = dr_count + 1;
					seloff[j] = rcells.item(6).firstChild.value;
					AcHeadCode[j] = rcells.item(7).firstChild.value;
					AcHeadName[j] = rcells.item(7).lastChild.nodeValue;
					CR_DB_indic[j] = rcells.item(8).firstChild.value;

					subLed_type[j] = rcells.item(9).firstChild.value;
					subLed_typeName[j] = rcells.item(9).lastChild.nodeValue;
					subLed_code[j] = rcells.item(10).firstChild.value;
					subLed_codeName[j] = rcells.item(10).lastChild.nodeValue;
					...
					billNO[j] = rcells.item(11).firstChild.value;
					var nex = rcells.item(11).firstChild.nextSibling;
					...
					nex = nex.nextSibling;
					MVR_no[j] = nex.value;
					vrAmt[j] = rcells.item(12).firstChild.value;
	...
	if (tot_amount < 0) {
		...
		//var selOffice=	document.getElementById("selOff").value;
		// alert("flag"+flag);
		opener.doParentPendingbills(vouNO, vouSL_NO, vouDATE, vouTYPE,
				...
				particular, vrAmt, MULTIPLE_PVR_DETAILS, MVR_no, seloff, j, flag);
		window.history.go();
		self.close();
	}
...
function checkboxAlert(val) {
	...
	var offid = doc.frmBankPay_PendingBill_create.cmbOffice_code.value;
	var sele = document.getElementsByName("vou_no").length;
	var check1=document.frmPending_Bills_revised.vou_no1.checked;
	var vouNO = new Array();
	var vouTYPE = new Array();
	var checkedvouTYPE = new Array();
	var checkedOfficeId = new Array();
    var currOfficeId=document.getElementById("off_code").value; 
    var offCount=0;
	...
	if (GetPayDate(val)) {
		...
		if (sele > 0) {
			if (sele == 1) {
				...
				for (var i = 0; i < sele; i++) {
					/*@NK on 17Mar21*/
				/*	document.getElementById("selOff").value=seloff[i];*/
					/*@NK on 17Mar21*/
					if (document.frmPending_Bills_revised.vou_no[i].checked == true) {
						r = document
								.getElementById(document.frmPending_Bills_revised.vou_no[i].value);
						...
						vouTYPE[j] = rcells.item(1).firstChild.value;
						checkedOfficeId.push(rcells.item(6).firstChild.value);
						if (vouTYPE[j] == 'GJV') {
				...
				if (checkVouType(checkedvouTYPE,sele)) {
					checkPayment(sele);
			/*		var selvouDate = new Array();
					var selvouNo = new Array();
					var selsNo = new Array();

					var selslType = new Array();
					var selslCode = new Array();
					var selslAmt = new Array();

					for (var i = 0; i < sele; i++) {

						if (document.frmPending_Bills_revised.vou_no[i].checked == true) {
							// document.frmPending_Bills_revised.vou_no[i].checked
							// = false;
							tableRow = document.getElementById("grid_body").children[i];
							selvouDate[i] = tableRow.children[2].firstChild.value;
							selvouNo[i] = tableRow.children[4].firstChild.value;
							selsNo[i] = tableRow.children[5].firstChild.value;
							selslType[i] = tableRow.children[8].firstChild.value;
							selslCode[i] = tableRow.children[9].firstChild.value;
							selslAmt[i] = tableRow.children[10].innerText;
							console.log(selslAmt);
						}
						if( (selvouDate != "" && selvouNo != "" && selsNo != "" && selslType != "" && selslCode != "" && selslAmt != "") 
								&& (document.frmPending_Bills_revised.vou_no[i].checked == true) ){
							
							 var url="../../../../../PendingBills_Revised.view?Command=load_journalAmt&selvouDate="+selvouDate[i]+"&selvouNo="+
							 selvouNo[i]+"&selsNo="+selsNo[i]+"&selslType="+selslType[i]+"&selslCode="+selslCode[i]+"&selslAmt="+selslAmt[i]+
							 "&cmbAcc_UnitCode="+ unitcode + "&cmbOffice_code=" + offid + "&row_id="+i;
							
							 var url="../../../../../PendingBills_Revised.view?Command=load_journalAmt&selvouDate="+selvouDate+"&selvouNo="+
							 selvouNo+"&selsNo="+selsNo+"&selslType="+selslType+"&selslCode="+selslCode+"&selslAmt="+selslAmt;
							// alert(url);
							var req = getTransport();
...
function GetPayDate(val) {
	...
	if (paycurrentSplit[1] == 02) {
		paycurrentFormat = paycurrentSplit[0] + "/February/"
				+ paycurrentSplit[2];
	}
	...
	if (paycurrentSplit[1] == 09) {
		...
		paycurrentFormat = paycurrentSplit[0] + "/September/"
				+ paycurrentSplit[2];
	}
...
function checkVouType(checkedvouTYPE,sele) {
	var k = 0, l = 1;
	for (k; k < checkedvouTYPE.length; k++) {
		for (l; l < checkedvouTYPE.length; l++) {
			if (checkedvouTYPE[k] != checkedvouTYPE[l]) {
				for (var i = 0; i < sele; i++) {
	...
	if (k == checkedvouTYPE.length) {
		return true;
	}
}

function checkPayment(sele)
{
	document.frmPending_Bills_revised.selOff.value=" ";
	var doc = window.opener.document; 
	var unitcode = doc.frmBankPay_PendingBill_create.cmbAcc_UnitCode.value;
	...
	for (var j=1; j <= sele; j++) {
		//document.getElementById("vou_no"+i).checked
		if (document.getElementById("vou_no"+j).checked == true) {
			// document.frmPending_Bills_revised.vou_no[i].checked
			// = false;
			tableRow = document.getElementById("grid_body").children[j-1];
			
			selOff[j] = tableRow.children[6].firstChild.value;
			selvouDate[j] = tableRow.children[2].firstChild.value;
			selvouNo[j] = tableRow.children[4].firstChild.value;
			selsNo[j] = tableRow.children[5].firstChild.value;
			selslType[j] = tableRow.children[9].firstChild.value;
			selslCode[j] = tableRow.children[10].firstChild.value;
			selslAmt[j] = tableRow.children[11].innerText;
			//console.log(selslAmt);
		}
		if( (selvouDate != "" && selvouNo != "" && selsNo != "" && selslType != "" && selslCode != "" && selslAmt != "") 
				&& (document.getElementById("vou_no"+j).checked == true) ){
			
			 var url="../../../../../PendingBills_Revised1.view?Command=load_journalAmt&selvouDate="+selvouDate[j]+"&selvouNo="+
			 selvouNo[j]+"&selsNo="+selsNo[j]+"&selslType="+selslType[j]+"&selslCode="+selslCode[j]+"&selslAmt="+selslAmt[j]+
			 "&cmbAcc_UnitCode="+ unitcode + "&cmbOffice_code=" + offid + "&row_id="+j;
	...
	//for (var sel=1;sel < selOff.length;sel++)
		//{
			document.frmPending_Bills_revised.selOff.value=selOff;
			//}
	
	
}

function ProcessJournalAmtInfo(req)
{  //alert("handle");
    if(req.readyState==4)
    {
        if(req.status==200)
        {   //alert("helloi")
            ...
            var rowid=baseResponse.getElementsByTagName("rowid")[0].firstChild.nodeValue;
            
            if(status=="fullpmt")
              {
            	document.getElementById("vou_no"+rowid).checked=false;
            	document.getElementById("vou_no"+rowid).disabled=true;
            	alert("Payment already made.Please contact Head office.");
              }
            else if(status=="partpmt")
            {
            var balamt=baseResponse.getElementsByTagName("paidedAmt")[0].firstChild.nodeValue;
            	document.getElementById("Vr_amt"+rowid).value=balamt;
            ...
            else if(status=="excesspmt")
            {
          	document.getElementById("vou_no"+rowid).checked=false;
          	document.getElementById("vou_no"+rowid).disabled=true;
          	alert("Excess Payment made");
            }
