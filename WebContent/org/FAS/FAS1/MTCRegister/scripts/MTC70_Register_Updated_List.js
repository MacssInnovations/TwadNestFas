...
function doFunction(Command,param)
{
 ...
 var optionId;
        ...
        if(document.frmMtc_upd_list.optionId[0].checked==true)
	    {
        	optionId=document.frmMtc_upd_list.optionId[0].value;
	    }else{
	    	optionId=document.frmMtc_upd_list.optionId[1].value;
	    }
        if(txtCB_Year=="")
        {
        ...
        else
        {
                ...
                var url="../../../../../MTC_Register_List?Command=searchByMonthUpdated&cmbAcc_UnitCode="+cmbAcc_UnitCode+
                "&param="+param+"&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&optionId="+optionId;			                
...
{ 
	    if(req.readyState==4)
	    {
		        ...
		        {  
			            ...
			            if(Command=="searchByMonthUpdated")
			            {
			            	if(param=="MTCOFF")
                ...
                if(service)
                 {
                	 for(i=0;i<service.length;i++)
                     {
		                   ...
		                   var items=new Array();
                             ...
                             items[7]=service[i].getElementsByTagName("CHECKED_AND_PASSED_DATE")[0].firstChild.nodeValue;
                             items[8]=service[i].getElementsByTagName("CHECKED_AND_PASSED_BY")[0].firstChild.nodeValue;
                           //  items[9]=service[i].getElementsByTagName("REGISTER_UPDATED_DATE")[0].firstChild.nodeValue;
                             items[9]=service[i].getElementsByTagName("remarks")[0].firstChild.nodeValue;
            ...
            if(service)
             {
            	 for(i=0;i<service.length;i++)
                 {
	                   ...
	                   items[0]=service[i].getElementsByTagName("unitname")[0].firstChild.nodeValue;
                         ...
                         items[8]=service[i].getElementsByTagName("CHECKED_AND_PASSED_DATE")[0].firstChild.nodeValue;
                         items[9]=service[i].getElementsByTagName("CHECKED_AND_PASSED_BY")[0].firstChild.nodeValue;
                       //  items[9]=service[i].getElementsByTagName("REGISTER_UPDATED_DATE")[0].firstChild.nodeValue;
...
function Show_new(unitcode,offid,yr,mon,bilno)
{
    ...
    Voucher_list_SL= window.open("../../../../../org/FAS/FAS1/MTCRegistesr/jsps/MTC_Register_Updated_SubList.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&yr="+yr+"&mon="+mon+"&bilno="+bilno,"ReceiptDateSearch","status=1,height=500,width=500,resizable=YES, scrollbars=yes"); 
    Voucher_list_SL.moveTo(250,250);  
    Voucher_list_SL.focus();
