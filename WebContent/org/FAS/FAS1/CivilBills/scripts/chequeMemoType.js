...
{
        if(document.getElementById("txtChequeMemoTypeDesc").value==0)
        {
                alert("Enter the chequeMemotypedesc");
        }
        else
        {
            var ChequeMemoTypeDesc=document.getElementById("txtChequeMemoTypeDesc").value;
            var url="../../../../../ChequeMemoType?command=add&ChequeMemoTypeDesc1="+ChequeMemoTypeDesc;
            //alert(url);
    ...
    if(req.readyState==4)   // Completed
      {
          ...
          {   
              ...
              if(command=="addResponse")
              {
                    ...
                    if(flag=="success")
                    {
                        var cheqMemoCode=baseResponse.getElementsByTagName("cheqmemeocode")[0].firstChild.nodeValue;
                        alert("Cheque Memo Type Code " + cheqMemoCode + " is inserted into the database successfully...");
                        getChequeMemo();
                        clearAll();
                    }
                }
...
function clearAll()
    {
            document.getElementById("txtChequeMemoTypeCode").value="";
            document.getElementById("txtChequeMemoTypeDesc").value="";
...
/************************************************************************************************************/
function callchequeMemoList()
    {
        winemp= window.open("chequeMemotype_list.jsp","list1","status=1,height=500,width=600,resizable=YES,scrollbars=yes"); 
        winemp.moveTo(250,250);  
        winemp.focus();
    }
function doParentEmp(cheqmemo_code)    
    {   
       // clearAll();
        document.forms[0].txtChequeMemoTypeCode.value=cheqmemo_code;
        var url="../../../../../ChequeMemoType?command=retrieve&cheqmemo_code1="+cheqmemo_code;
        var req=getTransport();
                req.open("GET",url,true); 
                req.onreadystatechange=function()
                {
...
function retrieveChecking(baseResponse)   
    {
          var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue; 
                  ...
                  {       
                      ...
                      var cheqmemo_desc =  baseResponse.getElementsByTagName("cheqmemo_desc");
                      
                      //document.forms[0].txtChequeMemoTypeCode.value=cheqmemo_code[0].firstChild.nodeValue;
                      document.forms[0].txtChequeMemoTypeDesc.value=cheqmemo_desc[0].firstChild.nodeValue;
...
var seq=0;
var pagesize = 10;
function getChequeMemo(){	
	var url="../../../../../ChequeMemoType?command=getcheque";
    var req=getTransport();
    req.open("GET",url,true);                
    req.onreadystatechange=function(){
    	viewResponse(req);
...
function changepage() {
	...
	try{		
	for ( var i = ll; i < ul; i++) {
		var billCode = response.getElementsByTagName("cheqmemo_code")[i].firstChild.nodeValue;
...
function viewDetails(id){
	 ...
	 url="../../../../../ChequeMemoType?command=edit"+jid;
    var req=getTransport();
     req.open("GET",url,true);        
     req.onreadystatechange=function()
     {
...
function editResponse(response){
	 var res=response.getElementsByTagName("flag")[0].firstChild.nodeValue;
	 	if(res=="success"){ 		
	 		document.getElementById('txtChequeMemoTypeCode').value=response.getElementsByTagName("cheqmemo_code")[0].firstChild.nodeValue;
	 		document.getElementById('txtChequeMemoTypeDesc').value=response.getElementsByTagName("cheqmemo_desc")[0].firstChild.nodeValue;	 		 			
	 	}else{
