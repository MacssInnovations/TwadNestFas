...
function manipulate(xmlrequest) {
	...
	if (xmlrequest.readyState == 4) {
		if (xmlrequest.status == 200) {
			...
			if (command == "printFunc") {
                ...
                var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
                       if(flag=="success")
                       {
                    	   ...
                    	   if(confirm("PassBookBalance is Zero ..."))
                    		   {
                    		  document.getElementById("pasdiv").style.display="block";
                    		   }else
                    			   {
			...
			else if (command == "checkFreezeStatus") {
                       ...
                       if(flag=="success")
                       {
                    	...
                    	if(status=="freeze"){
                    		//Lachu check freeze
                       	  ...
                       	  //alert("Use VIEW ALREADY FREEZED REPORT Button");
                       }
                      else
                       {                                                   
                              //alert("not freeze ");
...
function checkFreezeStatus(){
	...
	{
		var url="../../../../../BRSReport_Part2A?command=checkFreezeStatus&cmbAcc_UnitCode="
			+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
			+ cboCashBook_Month + "&cmbBankAccNo="
			+ cmbBankAccNo;
...
function freeze_brs_Report(path)
{
   // alert("test");
	...
	{
	var url = path
			+ "/BRSReport_Part2A?command=Brs_Freezed&cmbAcc_UnitCode="
			+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
			+ cboCashBook_Month + "&cmbBankAccNo="
			+ cmbBankAccNo;
...
//	var cboCashBook_Month = document.getElementById("txtCB_Month").value;
//	var cmbBankAccNo = document.getElementById("cmbBankAccNo").value;
//	var hid = document.getElementById("old").value;
//	
//	if (document.getElementById("txtCB_Year").value == "") {
//		alert("Enter Cash Book Year in the Field");
//		document.frmBRSReport1.cboCashBook_Year.focus();
...
//	else 
//	{
//	var url = path	+ "/BRSReport_Part2A?command=f_brs&cmbAcc_UnitCode="
//			+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
//			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
//			+ cboCashBook_Month + "&cmbBankAccNo="
//			+ cmbBankAccNo+"&hid="+hid;
//	   document.frmBRSReport_Part2A.action=url;
//       document.frmBRSReport_Part2A.submit();
//	}
//}
...
function freeze_check(path)
{
	...
	{
	var url = path	+ "/BRSReport_Part2A?command=f_brs_check&cmbAcc_UnitCode="
			+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
			+ cboCashBook_Month + "&cmbBankAccNo="
			+ cmbBankAccNo+"&hid="+hid;
...
//          		  alert("xmlrequest.status==>"+xmlrequest.status);
                ...
                //alert("response==>"+xmlrequest.responseText);
                   ...
                   //alert("flag==>"+flag);
                    ...
                    else if("Check the Part2A Report")
                    	{
                    	alert("Check the Part2A Report Caption Difference between 5 and (5a-5b) value is Zero ... ");
                    	}
...
//                    document.getElementById("freezeBRS").disabled=true;
//                    document.getElementById("freezeBRS_Report").disabled=true;
//                	}
//                    else if("Check the Part2A Report")
//                    	{
//                    	alert("Check the Part2A Report Caption Difference between 5 and (5a-5b) value is Zero ... ");
...
function printFunc(path)
{
	...
	{
	var url = path
			+ "/BRSReport_Part2A?command=printFunc&cmbAcc_UnitCode="
			+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
			+ "&cboCashBook_Year=" + cboCashBook_Year + "&cboCashBook_Month="
			+ cboCashBook_Month + "&cmbBankAccNo="
			+ cmbBankAccNo;
