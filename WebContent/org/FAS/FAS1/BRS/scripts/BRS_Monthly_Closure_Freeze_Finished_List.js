//                 BRS_Monthly_Closure_Freeze_Finished_List             //
...
function printFunc(path)
{
	...
	if (document.getElementById("txtCB_Year").value == "") {
		...
		document.frmBRS_Monthly_Closure_Freeze_Finished_List.cboCashBook_Year.focus();
	} else if (cboCashBook_Month == "" || cboCashBook_Month == "s") {
		alert("Select Cash Book Month in the Field");
		document.frmBRS_Monthly_Closure_Freeze_Finished_List.cboCashBook_Month.focus();
	} else 
	{
	var url = path
			+ "/BRS_Monthly_Closure_Freeze_Finished_List?command=printFunc&cboCashBook_Year="
			+ cboCashBook_Year + "&cboCashBook_Month=" + cboCashBook_Month;
	//alert(url);
	   document.frmBRS_Monthly_Closure_Freeze_Finished_List.action=url;
       document.frmBRS_Monthly_Closure_Freeze_Finished_List.submit();

	
	}
}

function printFunc_unitwise(path)
{
	
	var cmbAcc_UnitCode = document.getElementById("cmbAcc_UnitCode").value;
	
	var url = path
			+ "/BRS_Monthly_Closure_Freeze_Finished_List?command=unitwise&cmbAcc_UnitCode="
			+ cmbAcc_UnitCode;
	
	   document.Monthly_Closure_Freeze_unitwise.action=url;
       document.Monthly_Closure_Freeze_unitwise.submit();
