...
function doFunction_brs(Command, ListSeq, Filter) {
	...
	if (Command == "LoadTWADTransactions") {
		if (Filter == "DateWise") {
			...
			if (document.getElementById("txtFromDate").value == "") {
				...
				var url = "../../../../../BRS_MainForm_BS?Command=LoadTWADTransactions&cmbAcc_UnitCode="
						+ cmbAcc_UnitCode
						+ "&cmbOffice_code="
						+ cmbOffice_code
						+ "&txtCB_Year="
	...
	if (Command == "TotalTransactions") {
		var url = "../../../../../BRS_MainForm_BS?Command=TotalTransactions&cmbAcc_UnitCode="
				+ cmbAcc_UnitCode
				+ "&cmbOffice_code="
				+ cmbOffice_code
				+ "&txtCB_Year="
	...
	if (Command == "LoadNONTWADTransactions") {
		...
		var url = "../../../../../BRS_MainForm_BS?Command=LoadNONTWADTransactions&cmbAcc_UnitCode="
				+ cmbAcc_UnitCode
				+ "&cmbOffice_code="
				+ cmbOffice_code
				+ "&txtCB_Year="
...
function handleResponse_BRS_new(req) {
	if (req.readyState == 4) {
		if (req.status == 200) {
	...
	if (flag == "success") {
		...
		for ( var k = 0; k < r_no.length; k++) {
			...
			if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1) {
				...
				sel.checked = false;
				sel.value = "Y";
				sel.setAttribute('onclick', "list(" + seq + ")");
			}
...
function list(val) {
	if (document.getElementById("ClearanceEntry" + val).checked == true) {
		...
		if (document.getElementById("EntryFoundInPassBook" + val).checked == true) {
			winemp = window
					.open("BRS_Clearance_Entry.jsp?cmbAcc_UnitCode="
...
function list1(val) {
	if (document.getElementById("ClearanceEntry_NT" + val).checked == true) {
		...
		winemp = window.open("BRS_Clearance_Entry.jsp?cmbAcc_UnitCode="
				...
				+ "&cmbBankAccNo=" + cmbBankAccNo, "list",
				"status=1,height=450,width=800,resizable=YES, scrollbars=yes");
		winemp.moveTo(200, 200);
		winemp.focus();
	} else {
