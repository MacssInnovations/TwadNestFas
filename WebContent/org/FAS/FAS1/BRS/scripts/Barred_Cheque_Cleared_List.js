...
function list(path) {
	...
	if (txtCB_Year == "") {
		...
		document.frmBarred_Cheque_Cleared_List.txtCB_Year.focus();
	} else if (txtCB_Month == "") {
		alert("Enter Cash Book Month in the Field");
		document.frmBarred_Cheque_Cleared_List.txtCB_Month.focus();
	} else if (cmbBankAccNo == "") {
		alert("Select Bank Account No in the Field");
		document.frmBarred_Cheque_Cleared_List.cmbBankAccNo.focus();
	} else {
		var url = path
				+ "/Barred_Cheque_Cleared_List?command=list&cmbAcc_UnitCode="
	...
	if (flag == "success") {
		...
		if (len != 0) {
			for ( var k = 0; k < len; k++) {
				...
				cell20.appendChild(ClearedDate);
