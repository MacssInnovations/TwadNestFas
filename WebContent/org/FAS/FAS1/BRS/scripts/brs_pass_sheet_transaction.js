...
//	{
//		alert("Choose CashBookMonth");return false;
//	}
	
	var url = path+ "/BRS_Start_Month_and_Year?command=getbankbal&cmbAcc_UnitCode="+ cmbAcc_UnitCode + "&cmbBankAccNo=" + cmbBankAccNo+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month;
		...
		if (xmlrequest.status == 200) {
			...
			if (command == "LoadMonthYear") {
				...
				passBal_one(baseResponse);
                        ...
                        else if (command == "getbankbal") {
                        	bankbal_fetch(baseResponse);
            			}
		}
	}
}
...
function bankbal_fetch(baseResponse)
{
	var flag = baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
    if(flag=="success")
    {
      var passBalance = baseResponse.getElementsByTagName("pb")[0].firstChild.nodeValue;
         document.getElementById("txtPBBalance").value=passBalance;
    }
    else
    {
    document.getElementById("txtPBBalance").value="";
...
function LoadMonthYear1(baseResponse) {
	...
	//	document.getElementById("txtCB_Month").value=5;
...
function doFunction_brs(Command, ListSeq, Filter) {
	...
	if (Command == "LoadTWADTransactions") {
		if (Filter == "DateWise") {
			...
			if (document.getElementById("txtFromDate").value == "") {
				...
				var url = "../../../../../brs_pass_sheet_transaction?Command=LoadTWADTransactions&cmbAcc_UnitCode="
						+ cmbAcc_UnitCode
						+ "&cmbOffice_code="
						+ cmbOffice_code
						+ "&txtCB_Year="
	...
	if (Command == "TotalTransactions") {
		var url = "../../../../../brs_pass_sheet_transaction?Command=TotalTransactions&cmbAcc_UnitCode="
				+ cmbAcc_UnitCode
				+ "&cmbOffice_code="
				+ cmbOffice_code
				+ "&txtCB_Year="
	...
	if (Command == "LoadNONTWADTransactions") {
		...
		var url = "../../../../../brs_pass_sheet_transaction?Command=LoadNONTWADTransactions&cmbAcc_UnitCode="
				+ cmbAcc_UnitCode
				+ "&cmbOffice_code="
				+ cmbOffice_code
				+ "&txtCB_Year="
	...
	if (flag == "success") {
		...
		for ( var k = 0; k < r_no.length; k++) {
			...
			if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1) {
				var sel = document
						.createElement("<INPUT type='checkbox' name='EntryFoundInPassBook"
								+ seq
	...
	if (document.getElementById("ClearanceEntry" + val).checked == true) {
		...
		if (document.getElementById("EntryFoundInPassBook" + val).checked == true) {
			...
			winemp = window.open("BRS_Clearance_Entry.jsp?cmbAcc_UnitCode="
							...
							+ Amt_in_pasbook+"&doctype="+doctype+"&docno="+docno+"&docdate="+docdate+"&indicator="+ind, "list",
...
function list1(val) {
	if (document.getElementById("ClearanceEntry_NT" + val).checked == true) 
	{
		...
		winemp = window.open("BRS_Clearance_Entry.jsp?cmbAcc_UnitCode="
				...
				+ "&cmbBankAccNo=" + cmbBankAccNo+"&Amt_in_pasbook="+Amt_in_pasbook+"&doctype="+doctype+"&docno="+docno+"&docdate="+docdate+"&indicator="+ind, "list",
				"status=1,height=450,width=800,resizable=YES, scrollbars=yes");
		winemp.moveTo(200, 200);
		winemp.focus();
	} 
...
function LoadNONTWADTransactions(baseResponse) {
	...
	if (flag == "success") {
		...
		cr_amount_NT.setAttribute('onchange', "CRAmount_Check(" + seqNT + ")");
		cr_amount_NT.value = "";
		...
		dr_amount_NT.setAttribute('onchange', "DRAmount_Check(" + seqNT + ")");
		...
		if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1) {
			try {
				Trans_Type_NT = document
						.createElement("<select name='Trans_Type_NT" + seqNT+ "' id='Trans_Type_NT'"+seqNT+" onchange=jrnl_check("+seqNT+")>");
			} catch (e) {
				alert(e.description)
			}
		} else {
			...
			Trans_Type_NT.setAttribute('onChange',"jrnl_check("+seqNT+")");
...
//			sel.checked = false;
//			sel.value = "Y";
//			sel.setAttribute('onclick', "list1(" + seqNT + ")");
//		}
//		cell14.appendChild(sel);
//		mycurrent_row.appendChild(cell14);
...
function checkNull() {
	...
	if (txtCB_Year == "") {
		...
		return false;
         ...
         //pradha
       // else if (document.getElementById("EntryFoundInPassBook" + sam).checked == true) {
		var tbody = document.getElementById("grid_body_TWAD");
