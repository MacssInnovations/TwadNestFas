...
{  
        ...
        var txtJournal_type="TCACB_journal";
       var tbody=document.getElementById("tbody");
        ...
        {             
		           if(txtCB_Year.length!=0 && txtCB_Month.length!=0)
		           {
			                ...
			                var url="../../../../../Post_TDA_List?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cmbStatus="+cmbStatus+"&txtCreat_By_Module="+txtCreat_By_Module+"&journal_type="+txtJournal_type;
        ...
        {  
		           ...
		           {
			                var url="../../../../../Post_TDA_List?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+
			                txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtFrom_date="+txtFrom_date+"&txtTo_date="+txtTo_date+"&cmbStatus="+cmbStatus+"&journal_type="+txtJournal_type+"&txtCreat_By_Module="+txtCreat_By_Module;
			                var req=getTransport();
			                req.open("GET",url,true); 
			                req.onreadystatechange=function()
			                {
        ...
        {                       
                       ...
                       if(service)
	               {
		                    ...
		                    txtCB_Month=baseResponse.getElementsByTagName("txtCB_Month")[0].firstChild.nodeValue;	
                                    journal_type=baseResponse.getElementsByTagName("jtype")[0].firstChild.nodeValue;
                                    var tbody=document.getElementById("tbody");                    
		                    var i=0;
		                    totalblock=0;
		                    if(service.length>0)
...
function changepage()
{
		var page=document.frmBank_journalList.cmbpage.value;
...
function loadPage(page)
{
        ...
        document.frmBank_journalList.cmbpage.selectedIndex=page-1;
        var tbody=document.getElementById("tbody");
        try{tbody.innerHTML="";}
        catch(e) {tbody.innerText="";}  
        if(service)
        {
        		   ...
        		   for(i=p;i<service.length&& c<__pagination;i++)
        		   {
	                        ...
	                        mycurrent_row.appendChild(cell);
                               if(journal_type=="TDAO")
                              {
                                            payment_no=service[i].getElementsByTagName("payment_no")[0].firstChild.nodeValue;
                                            payment_date=service[i].getElementsByTagName("payment_date")[0].firstChild.nodeValue;
                                                var cell=document.createElement("TD");
                                                cell.align='CENTER';
                              ...
                              else if(journal_type=="TCACB_journal")
                               {
                                        receipt_no=service[i].getElementsByTagName("receipt_no")[0].firstChild.nodeValue;
                                        receipt_date=service[i].getElementsByTagName("receipt_date")[0].firstChild.nodeValue;
                                      
                                            var cell=document.createElement("TD");
                                            cell.align='CENTER';
                                            var txtedit=document.createTextNode(receipt_no);
                                            cell.appendChild(txtedit);
                                            mycurrent_row.appendChild(cell);
                                          
                                            var cel2=document.createElement("TD");
                                            cel2.align='CENTER';
                                            var txtedit=document.createTextNode(receipt_date);
                                            cel2.appendChild(txtedit);
                                            mycurrent_row.appendChild(cel2);
...
function changepagesize()
{
        ...
        __pagination=document.frmBank_journalList.cmbpagination.value;
