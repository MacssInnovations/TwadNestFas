...
function Show(unitcode,offid,yr,mon,vou_no)
{
	     ...
	     Voucher_list_SL= window.open("../../../../../org/FAS/FAS1/TDA/jsps/TDA_Raised_List_SL.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&cashbook_yr="+yr+"&cashbook_mn="+mon+"&voucher_no="+vou_no,"ReceiptDateSearch","status=1,height=500,width=500,resizable=YES, scrollbars=yes");
...
{  
        ...
        var cmbStatus=document.getElementById("cmbStatus").value;//alert(cmbStatus);
        ...
        {             
		           if(txtCB_Year.length!=0 && txtCB_Month.length!=0)
		           {
			                ...
			                var url="../../../../../TDA_Raised_List?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cmbStatus="+cmbStatus+"&cmbJournal_type="+cmbJournal_type+"&txtCreat_By_Module="+txtCreat_By_Module;
        ...
        {  
		           ...
		           {
			                var url="../../../../../TDA_Raised_List?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+
			                txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtFrom_date="+txtFrom_date+"&txtTo_date="+txtTo_date+"&cmbStatus="+cmbStatus+"&cmbJournal_type="+cmbJournal_type+"&txtCreat_By_Module="+txtCreat_By_Module;
			                var req=getTransport();
			                req.open("GET",url,true); 
			                req.onreadystatechange=function()
			                {
...
function loadTable(baseResponse)
{
        ...
        {                       
	               ...
	               if(service)
	               {
		                    Ucode=baseResponse.getElementsByTagName("Ucode")[0].firstChild.nodeValue;//alert(Ucode);
		                    Offid=baseResponse.getElementsByTagName("Offid")[0].firstChild.nodeValue;//alert(Offid);
		                    txtCB_Year=baseResponse.getElementsByTagName("txtCB_Year")[0].firstChild.nodeValue;//alert(txtCB_Year);
		                    txtCB_Month=baseResponse.getElementsByTagName("txtCB_Month")[0].firstChild.nodeValue;	//alert(txtCB_Month);
		                    var tbody=document.getElementById("tbody");                    
		                    var i=0;
		                    totalblock=0;
		                    if(service.length>0)
...
function changepage()
{
		var page=document.raised_List.cmbpage.value;
...
{      
        ...
        document.raised_List.cmbpage.selectedIndex=page-1;
        var tbody=document.getElementById("tbody");
        try{tbody.innerHTML="";}
        catch(e) {tbody.innerText="";}  
        if(service)
        {
        		   ...
        		   for(i=p;i<service.length&& c<__pagination;i++)
        		   {
	                        c++;
	                        var items=new Array();
	                        items[0]=service[i].getElementsByTagName("vou_no")[0].firstChild.nodeValue;//alert(items[0]);
	                        items[1]=service[i].getElementsByTagName("vou_date")[0].firstChild.nodeValue;//alert(items[1]);
	                        items[2]=service[i].getElementsByTagName("Remak")[0].firstChild.nodeValue;//alert(items[2]);
	                        ...
	                        payment_no=service[i].getElementsByTagName("Trans_Unit")[0].firstChild.nodeValue;
	                        payment_date=service[i].getElementsByTagName("Trans_Rea")[0].firstChild.nodeValue;
                     	    var cell=document.createElement("TD");
                            cell.align='CENTER';
                            var txtedit=document.createTextNode(payment_no);
                            cell.appendChild(txtedit);
                            mycurrent_row.appendChild(cell);
                          
                            var cel2=document.createElement("TD");
                            cel2.align='CENTER';
                            var txtedit=document.createTextNode(payment_date);
...
function changepagesize()
{
        ...
        __pagination=document.raised_List.cmbpagination.value;
