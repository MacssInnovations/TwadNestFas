...
function Show_accepted(unitcode,offid,yr,mon,vou_no)
{
	     if (Voucher_list_SL && Voucher_list_SL.open && !Voucher_list_SL.closed) 
	     {
		       Voucher_list_SL.resizeTo(500,500);
	     ...
	     else
	     {
	    	   ...
	    	   Voucher_list_SL= window.open("../../../../../org/FAS/FAS1/TDA/jsps/TDA_TCA_ListAll_AccUnit.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&cashbook_yr="+yr+"&cashbook_mn="+mon+"&voucher_no="+vou_no,"ReceiptDateSearch","status=1,height=500,width=500,resizable=YES, scrollbars=yes");
...
var type1;
        ...
        if(cmbJournal_type=="62A")
        {
        type1="TDAO";
        }
       else if(cmbJournal_type=="62B")
       {
        type1="TCAO";
       }
       else if(cmbJournal_type=="63A")
       {
        type1="TDAO";
       }
       else if(cmbJournal_type=="63B")
       {
        type1="TCAO";
       }
        ...
        {             
		           if(txtCB_Year.length!=0 && txtCB_Month.length!=0)
		           {
			                ...
			                var url="../../../../../TDA_TCA_List_Rejected_servlet?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cmbStatus="+cmbStatus+"&cmbJournal_type="+cmbJournal_type+"&type1="+type1;			                
			          //     alert(url);
        ...
        {  
		           ...
		           {
			                var url="../../../../../TDA_TCA_List_Rejected_servlet?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtFrom_date="+txtFrom_date+"&txtTo_date="+txtTo_date+"&cmbStatus="+cmbStatus+"&cmbJournal_type="+cmbJournal_type+"&type1="+type1;
			            //         alert(url);
                                        var req=getTransport();
			                req.open("GET",url,true); 
			                req.onreadystatechange=function()
...
function changepage()
{
		var page=document.list_rejected.cmbpage.value;
...
function loadPage(page)
{
        ...
        document.list_rejected.cmbpage.selectedIndex=page-1;
        var tbody=document.getElementById("tbody");
        try{tbody.innerHTML="";}
        catch(e) {tbody.innerText="";}  
        ...
        if(service)
        {
                ...
                for(i=p;i<service.length&& c<__pagination;i++)
			    {
		                        ...
		                        var anc=document.createElement("A");
                                       ...
                                       // alert(jtype);
                                        if(jtype=="63A" || jtype=="63B"){
                                         var url="javascript:Show_accepted('"+Ucode+"','"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[0]+"')";
                                         anc.href=url;
...
function changepagesize()
{
           ...
           __pagination=document.list_rejected.cmbpagination.value;
