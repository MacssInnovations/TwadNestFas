...
function Show(unitcode,offid,yr,mon,vou_no,vr_dte)
{
	//alert(unitcode+"::offid::"+offid);
	     ...
	     Voucher_list_SL= window.open("../../../../../org/FAS/FAS1/TDA/jsps/TDA_RespondDetails_List.jsp?cmbAcc_UnitCode="+unitcode+"&cmbOffice_code="+offid+"&cashbook_yr="+yr+"&cashbook_mn="+mon+"&voucher_no="+vou_no+"&trf_date="+vr_dte,"ReceiptDateSearch","status=1,height=500,width=500,resizable=YES, scrollbars=yes");
...
function showGrid()
{
		 if(document.getElementById("radActive").value=="tdaPending" || document.getElementById("radActive").value=="tcaPending")
		 {	  
	     ...
	     {        
                           document.getElementById("bymonth").style.display="block";
	         	   document.getElementById("bydate").style.display="block";
...
{  
        ...
        var radActive=document.getElementById("radActive").value;
        ...
        if(Command=="searchByMonths")
        {             
		           if(txtCB_Year.length!=0 && txtCB_Month.length!=0)
		           {
		        	   		var txtCreat_By_Module='GJV';
		        	   	   var url="../../../../../TDA_Responding_List?Command=searchByMonth&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+txtCB_Year+"&txtCB_Month="+txtCB_Month+"&cmbStatus="+cmbStatus+"&active="+radActive+"&txtCreat_By_Module="+txtCreat_By_Module+"&hid="+hid;
        ...
        {  
		           ...
		           {
			                var url="../../../../../TDA_Responding_List?Command=searchByDate&cmbAcc_UnitCode="+cmbAcc_UnitCode+
			                "&cmbOffice_code="+cmbOffice_code+"&txtCB_Year="+
			                txtCB_Year+"&txtCB_Month="+txtCB_Month+"&txtFrom_date="+txtFrom_date+"&txtTo_date="+txtTo_date+"&cmbStatus="+cmbStatus+"&active="+radActive+"&txtCreat_By_Module="+txtCreat_By_Module;
			                var req=getTransport();
			                req.open("GET",url,true); 
			                req.onreadystatechange=function()
			                {
...
{ 
	    if(req.readyState==4)
	    {
			       ...
			       {  
				            ...
				            else if(Command=="PendingDate")
				            {
				                loadTable(baseResponse);
				            }
				            else if(Command=="ClearedDate")
				            {
				                loadTable(baseResponse);
				            }
			       }
...
function loadTable(baseResponse)
{
        ...
        {   
	               ...
	               service=baseResponse.getElementsByTagName("leng");
                     //  alert("service"+service.length);
	               if(service)
	               {
	            	   if(document.getElementById("radActive").value=="tdaClearedDuring")
	            	   {
	            		   Ucode=baseResponse.getElementsByTagName("Ucode")[0].firstChild.nodeValue; 
	            	   }
	            	   else if(document.getElementById("radActive").value=="tcaClearedDuring")
	            	   {
	            		   Ucode=baseResponse.getElementsByTagName("Ucode")[0].firstChild.nodeValue;
	            	   }
	            	   else{
		                    Ucode=baseResponse.getElementsByTagName("acc_Unit")[0].firstChild.nodeValue;
	            	   }
		                   //alert("Ucode:::"+Ucode);
		                    Offid=baseResponse.getElementsByTagName("off_Unit")[0].firstChild.nodeValue;
		                    ...
		                    txtCB_Month=baseResponse.getElementsByTagName("txtCB_Month")[0].firstChild.nodeValue;
                                     if(document.forms[0].radActive.value=="tdaPending" || document.forms[0].radActive.value=="tcaPending")
                                   {
		                    var tbody=document.getElementById("tbody");                    
		                    var i=0;
		                    totalblock=0;
...
function changepage()
{
		var page=document.respond_List.cmbpage.value;
...
{    
        ...
        document.respond_List.cmbpage.selectedIndex=page-1;
        if(document.forms[0].radActive.value=="tdaPending" || document.forms[0].radActive.value=="tcaPending")
         {
            var tbody=document.getElementById("tbody");
            try{tbody.innerHTML="";}
        			  ...
        			  if(document.forms[0].radActive.value=="tdaPending" || document.forms[0].radActive.value=="tcaPending")
        			   ...
        			   document.getElementById("clearedDiv").style.display="none";
    	         	   document.getElementById("pendingDiv").style.display="block";
	                        ...
	                        items[4]=service[i].getElementsByTagName("acc_Name")[0].firstChild.nodeValue;
	                      items[5]=service[i].getElementsByTagName("acc_Unit")[0].firstChild.nodeValue;
	                      items[6]=service[i].getElementsByTagName("res_date")[0].firstChild.nodeValue;
	                     // alert("   ... items[5]"+items[5]) 
	                      var tbody=document.getElementById("tbody");
	                       ...
	                       var url="javascript:Show("+items[5]+",'"+Offid+"','"+txtCB_Year+"','"+txtCB_Month+"','"+items[0]+"','"+items[6]+"')";
	                        anc.href=url;
	                      ...
	                      //  alert("items[5]"+items[5]) 
	                        ...
	                        jvr_no=service[i].getElementsByTagName("res_jvrNo")[0].firstChild.nodeValue;
	                        jvr_date=service[i].getElementsByTagName("res_date")[0].firstChild.nodeValue;
                     	   
                            var cel2=document.createElement("TD");
                            cel2.align='CENTER';
                            var txtedit=document.createTextNode(jvr_no);
                            cel2.appendChild(txtedit);
                            mycurrent_row.appendChild(cel2);
                            
                            var cel3=document.createElement("TD");
                            cel3.align='CENTER';
                            var txtedit=document.createTextNode(jvr_date);
                            cel3.appendChild(txtedit);
                            mycurrent_row.appendChild(cel3);
...
function changepagesize()
{
        ...
        __pagination=document.respond_List.cmbpagination.value;
