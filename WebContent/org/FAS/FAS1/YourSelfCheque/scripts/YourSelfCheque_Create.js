...
/**-------------------- if 'Whether DD is obtained from Bank ' is NO, Details Part is optional  --------------*/
function Details_Optional()
{	
	if ( document.YourSelfChequeForm.txtDDObtained[1].checked == true )
	{ 
		ADD_GRID();
	}	
}
...
function ADD_GRID()
{
   ...
   {		   
       ...
       var commission=document.getElementById("txtCommissionCharge").value;
       var ddamount=document.getElementById("txtDDAmount").value;
      // alert("commission:::"+commission);
      // alert(ddamount);
       /*  DD amount cannot be less than commission charge  */
       if(parseInt(commission)>parseInt(ddamount))
       {
      ...
      // var ddate =document.getElementById("txtDDDate").value.split("/");
      // alert(ddate);
   ...
   else
   {
	   /* Array Declaration */
       var items=new Array();
       items[0]=0;      // document.getElementById("txtDDNumber").value;
       items[1]=''; // document.getElementById("txtDDDate").value;
       items[2]=0;      // document.getElementById("txtDDAmount").value;
       items[3]='-';    // document.getElementById("txtDDFavourOf").value;
       items[4]=0;      // document.getElementById("txtCommissionCharge").value; 
...
{  
    ...
    if(req.readyState==4)
    {
        ...
        {  
            ...
            else if(Command=="LoadChequeNumber")
            {
            ...
            var flag=baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
            	 ...
            	 if(flag=="success")
            	    {
            	           var bank_ac_no1 =new Array();
            	           var acc_desc1   =new Array();
            	           
            	           /** Bank Account Number Object to find length */ 
            	           var acc_no=baseResponse.getElementsByTagName("acc_no1");
            	           
            	           /** Get Combo box Object */
            	           var cmbBankAccNo = document.getElementById("txtChequeNo");
            	           
            	           
            	            for(var k=0;k<acc_no.length;k++)
            	            {
            	            	bank_ac_no1[k]=baseResponse.getElementsByTagName("acc_no1")[k].firstChild.nodeValue;
            	            	acc_desc1[k]=baseResponse.getElementsByTagName("acc_no1")[k].firstChild.nodeValue;
            	            }
            	            ...
            	            {   
            	                  var option=document.createElement("OPTION");
            	                  option.text=acc_desc1[k];
            	                  option.value=bank_ac_no1[k];
            	                  try
            	                  {
            	                	  cmbBankAccNo.add(option);
            	                  }
                    ...
                    {
                       alert("NO Cheque Number is found");
...
function limit_amt(field,e)
{
    ...
    if(field.value.length<17)
    {
      ...
      if (unicode!=8 && unicode !=9  )
      {
          ...
          if (unicode<45 || unicode==47 || unicode>57   )        
              return false 
      }
    }
    else 
...
function LoadChequeNumber()
{
 ...
 var url="../../../../../YourSelfCheque_Create.kv?command=LoadChequeNumber&cmbAcc_UnitCode="+cmbAcc_UnitCode+"&cmbOffice_code="+cmbOffice_code+"&txtCrea_date="+txtCrea_date+"&cmbBankAccNo="+cmbBankAccNo;
 //alert(url);
	 var req=getTransport();
     req.open("GET",url,true); 
     req.onreadystatechange=function()
...
function CheckDate()
{
    ...
    var txtDDDate=document.getElementById("txtDDDate").value.split("/");
    
    var txtChequeDate=document.getElementById("txtChequeDate").value.split("/");
    
    var Dd=new Date();

   Dd.setFullYear(txtDDDate[2],parseInt(txtDDDate[1])-1,txtDDDate[0]);

  var Cd=new Date();

   Cd.setFullYear(txtChequeDate[2],parseInt(txtChequeDate[1])-1,txtChequeDate[0]);
...

{
   ...
   {
   alert("Cheque Date is Greater than DD Date ");
...
chequeRange=function(){	
		...
		var accountNo=document.getElementById('cmbBankAccNo').value;
                var txtCrea_date=document.getElementById('txtCrea_date').value;
