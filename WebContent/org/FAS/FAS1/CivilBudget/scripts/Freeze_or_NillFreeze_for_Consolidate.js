//			Freeze_or_NillFreeze_for_Consolidate			//
...
function initialLoad1(path) {
	//alert(path);
	var url = path + "/Freeze_or_NillFreeze_for_Consolidate?command=getFormatName";
	// alert(url);
...
function FuncSave(path) {
	...
	if (cmbFreezeType == "") {
		...
		document.frmFreeze_or_NillFreeze_for_Consolidate.cmbFreezeType.focus;
	} else if (cmbFormat_Name == "") {
		alert("select Format Name in the Filed");
		document.frmFreeze_or_NillFreeze_for_Consolidate.cmbFormat_Name.focus;
	} else {
		var url = path
				+ "/Freeze_or_NillFreeze_for_Consolidate?command=save&cmbAcc_UnitCode="
