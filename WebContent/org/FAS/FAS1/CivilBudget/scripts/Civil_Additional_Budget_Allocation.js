//	Civil_Additional_Budget_Allocation	//
var seq = 0;
var seq1 = 1;
...
function load()
{
	var url = "../../../../../Civil_Additional_Budget_Allocation?command=getofficeid";
//alert(url);
...
function checkNull(){
    var cmbAcc_UnitCode=document.frmCivil_Additional_Budget_Allocation.cmbAcc_UnitCode.value;
       var txtRegionId = document.frmCivil_Additional_Budget_Allocation.txtRegionId.value;
       var cmbStatementName=document.frmCivil_Additional_Budget_Allocation.cmbStatementName.value;
       var financial_year = document.frmCivil_Additional_Budget_Allocation.cmbFinancialYear.value;
...
function LoadOfficeName()
{
	...
	var url = "../../../../../Civil_Additional_Budget_Allocation?command=getOfficeName&cmbAcc_UnitCode=" + cmbAcc_UnitCode;
//alert(url);
var xmlrequest = AjaxFunction();
//alert("11111");
xmlrequest.open("GET", url, true);
...
function loadstatname(path)
{
	
	//alert("loadstatname ");
	var url =  path + "/Civil_Additional_Budget_Allocation?command=getStatementName";
	//alert(url);
	var xmlrequest = AjaxFunction();
	xmlrequest.open("GET", url, true);
	xmlrequest.onreadystatechange = function() {

		//alert("loadstatname5555 ");
		manipulate1(xmlrequest);
	};
	xmlrequest.send(null);
}
...
function initialLoad(path) {
	...
	//document.frmCivil_Additional_Budget_Allocation.butView.disabled = false;
	document.frmCivil_Additional_Budget_Allocation.butSub.disabled = false;
	document.frmCivil_Additional_Budget_Allocation.butDelete.disabled = true;
	//document.frmCivil_Additional_Budget_Allocation.butUpdate.disabled = true;
...
function getStatementName1(baseResponse) {
	document.frmCivil_Additional_Budget_Allocation.cmbStatementName.length = 1;
...
function loadofficeid(baseResponse) {
	...
	document.frmCivil_Additional_Budget_Allocation.cmbAcc_UnitCode.length = 0;
	//alert("flag");
	var flag = baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
	//alert("flag"+flag);
	if (flag == "success") {
...
function loadofficename(baseResponse) {
	//alert("catherine");
	//System.out.println("inside the office id generation");
	document.frmCivil_Additional_Budget_Allocation.cmbOffice_code.length = 1;
	//alert("flag");
	var flag = baseResponse.getElementsByTagName("flag")[0].firstChild.nodeValue;
	//alert("flag"+flag);
	if (flag == "success") {
...
function LoadGrid() {
	...
	if (cmbStatementName == "") {
		...
		document.frmCivil_Additional_Budget_Allocation.cmbStatementName.focus();
	} else {
			
			
				var url = "../../../../../Civil_Additional_Budget_Allocation?command=get_head_office&y1="
...
function LoadGrid_head(baseResponse)
{
	...
	//	alert("flag:::"+flag);
		...
		document.getElementById("txtTotalAmount_Req").value="";
		document.getElementById("txtTotalAmount_allotted").value="";

		var r_no = baseResponse.getElementsByTagName("STATEMENT_GROUP_NO");
		seq = 0;
		//xml = xml + "<AMOUNT_ALLOTTED>"+ rs.getString("AMOUNT_ALLOTTED")+ "</AMOUNT_ALLOTTED>";
		//xml = xml + "<ADDL_BUDGET_REQ>"+ rs.getString("ADDL_BUDGET_REQ")+ "</ADDL_BUDGET_REQ>";
		//ttl_amount=ttl_amount+rs.getDouble("AMOUNT_ALLOTTED");
		//ttl_amount_req=ttl_amount_req+rs.getDouble("ADDL_BUDGET_REQ");
		var item = new Array();
		for ( var k = 0; k < r_no.length; k++) {
			...
			item[3] = baseResponse.getElementsByTagName("AMOUNT_ALLOTTED")[k].firstChild.nodeValue;
			item[4] = baseResponse.getElementsByTagName("ADDL_BUDGET_REQ")[k].firstChild.nodeValue;
			if(item[2]>0)
			{
				
				item[5] = baseResponse.getElementsByTagName("ADDL_BUDGET_ALLOTTED")[k].firstChild.nodeValue;
			}
			/** Create Table Row */
			...
			Amount.type = "Text";
			Amount.name = "AMOUNT_ALLOTTED" + seq;
			Amount.id = "AMOUNT_ALLOTTED" + seq;
			Amount.value = item[3];
			...
			var Amount = document.createElement("input");
			Amount.type = "Text";
			Amount.name = "ADDL_BUDGET_REQ" + seq;
			Amount.id = "ADDL_BUDGET_REQ" + seq;
			Amount.value = item[4];
			Amount.readOnly = true;
			Amount.style.textAlign = "right";
			Amount.setAttribute('onblur', "focus_next("+seq+","+r_no.length+")");
			...
			if(item[2]>0)
			{
				...
				var Amount = document.createElement("input");
				Amount.type = "Text";
				Amount.name = "ADDL_BUDGET_ALLOTTED" + seq;
				Amount.id = "ADDL_BUDGET_ALLOTTED" + seq;
				Amount.value = item[5];
				Amount.style.textAlign = "right";
				Amount.setAttribute('onblur', "focus_next("+seq+","+r_no.length+")");
				cell20.appendChild(Amount);
			...
			document.getElementById("frmCivil_Additional_Budget_Allocation").appendChild(
					Statement_Group_No);
		...
		var tamount1=baseResponse.getElementsByTagName("ttl_amount_req")[0].firstChild.nodeValue;
		document.getElementById("txtTotalAmount_Req").value= tamount1;
		document.getElementById("txtTotalAmount_Req").style.textAlign = "right";
		
		if(item[2]>0)
		{
			var tamount2=baseResponse.getElementsByTagName("ttl_amount_req_allo")[0].firstChild.nodeValue;
			document.getElementById("txtTotalAmount_allotted").value= tamount2;
...
function LoadData(path) {
	//alert(path);
	//document.frmCivil_Additional_Budget_Allocation.butView.disabled = false;
	document.frmCivil_Additional_Budget_Allocation.butSub.disabled = true;
	document.frmCivil_Additional_Budget_Allocation.butDelete.disabled = false;
	//document.frmCivil_Additional_Budget_Allocation.butUpdate.disabled = false;
	...
	else if(cmbOffice_code=="")
	{
		...
		document.frmCivil_Additional_Budget_Allocation.txtRegionId.focus();
	}
	else if(cmbFinancialYear=="")
	{
		alert("Select Financial Year");
		document.frmCivil_Additional_Budget_Allocation.cmbFinancialYear.focus();
	}
	else {
	var url = path + "/Civil_Additional_Budget_Allocation?filter=view&cmbFinancialYear="
...
function funcSave() {
	...
	var totalAmt_req=document.getElementById("txtTotalAmount_Req").value;
	var totalAmt_allot=document.getElementById("txtTotalAmount_allotted").value;
	...
	if(document.getElementById("txtRegionId").value==5000)
			{
				...
				if (rowcount != 0) {
						for ( var i = 0; i < rowcount; i++) {
							if (document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value == "") {
								alert("Enter Amount in the Field");
								document.getElementById("ADDL_BUDGET_ALLOTTED" + i).focus();
								return false;
							}
							else
							{
								
								gridAmt=(gridAmt+parseFloat(document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value));
						...
						if(totalAmt_allot!=parseFloat(gridAmt))
						{
	...
	else
		{
				...
				if (rowcount != 0) {
						for ( var i = 0; i < rowcount; i++) 
						{
							...
							{
								gridAmt=gridAmt+parseInt(document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value);
							}
						}
						if(totalAmt_allot!=parseInt(gridAmt))
						{
...
function funcUpdate() {
	...
	if (rowcount != 0) {
		for ( var i = 0; i < rowcount; i++) {
			if (document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value == "") {
				alert("Enter ADDL BUDGET ALLOTTED Amount in the Field");
				document.getElementById("ADDL_BUDGET_ALLOTTED" + i).focus();
				return false;
			} 
		}
...
function focus_next(ii,len)
{
	//	alert(ii+"::::"+len);
		if(ii==(len-1))
		{
			document.getElementById("txtTotalAmount_allotted").focus();
		}
		else{
			var orderno=ii+1;
			document.getElementById("ADDL_BUDGET_ALLOTTED" +orderno).focus();
		}
}
...
function CalculateTotalAmount() {
	...
	var tbody = document.getElementById("grid_body");
	var rowcount = tbody.rows.length;
	var total_Amt=0,total_Amt_req=0,total_Amt_allotted=0;
		for ( var i = 0; i < rowcount; i++) {
			if (document.getElementById("AMOUNT_ALLOTTED" + i).value != "") {
			total_Amt=parseFloat(total_Amt)+parseFloat(document.getElementById("AMOUNT_ALLOTTED" + i).value);			
			}
			if (document.getElementById("ADDL_BUDGET_REQ" + i).value != "") {
				total_Amt_req=parseFloat(total_Amt_req)+parseFloat(document.getElementById("ADDL_BUDGET_REQ" + i).value);			
				}
			if (document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value != "") {
				total_Amt_allotted=parseFloat(total_Amt_allotted)+parseFloat(document.getElementById("ADDL_BUDGET_ALLOTTED" + i).value);			
				}
		...
		document.getElementById("txtTotalAmount_Req").value = total_Amt_req;
		document.getElementById("txtTotalAmount_Req").style.textAlign = "right";
