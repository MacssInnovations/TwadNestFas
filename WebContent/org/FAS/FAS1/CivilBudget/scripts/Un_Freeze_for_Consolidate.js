...
function LoadAccountingUnitID_OfficeID(path) {
	//alert(path);
	var url = path + "/Un_Freeze_for_Consolidate?command=LoadAccountingUnitID";
	//alert(url);
	var xmlrequest = AjaxFunction();
	xmlrequest.open("POST", url, true);
	xmlrequest.onreadystatechange = function() {
...
function FuncSave(path) {
	...
	if (cmbFormat_Name == "") {
		...
		document.frmUn_Freeze_for_Consolidate.cmbFormat_Name.focus;
	} else {
		var url = path + "/Un_Freeze_for_Consolidate?command=save&cmbAcc_UnitCode="
