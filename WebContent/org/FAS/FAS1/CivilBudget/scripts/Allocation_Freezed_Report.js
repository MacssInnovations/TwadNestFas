...
function LoadAccountingUnitID_OfficeID(path) {
	//alert(path);
	var url = path
			+ "/Allocation_Freezed_Report?command=LoadAccountingUnitID";
	//alert(url);
	var xmlrequest = AjaxFunction();
	xmlrequest.open("POST", url, true);
	xmlrequest.onreadystatechange = function() {
...
function Funclist(path) {
	...
	if (Statement_Name == "") {
		...
		var url = path
				+ "/Allocation_Freezed_Report?command=list&cmbAcc_UnitCode="
				+ cmbAcc_UnitCode + "&cmbOffice_code=" + cmbOffice_code
				+ "&Statement_Name=" + Statement_Name + "&cmbFinancialYear="
				+ cmbFinancialYear;
	alert(url);
		var xmlrequest = AjaxFunction();
		xmlrequest.open("POST", url, true);
	...
	if (flag1 == "success") {
		...
		if (len4 != 0) {
			for ( var i = 0; i < len4; i++) {
				...
				var se = document.getElementById("Statement_Name");
				var op = document.createElement("OPTION");
				op.value = FormatNo;
	...
	if (flag == "success") {
		...
		if (r_no.length != 0) {
			...
			for ( var k = 0; k < r_no.length; k++) {
				...
				var offid = baseResponse.getElementsByTagName("offid")[k].firstChild.nodeValue;
			
				var statement_name = baseResponse.getElementsByTagName("statement_name")[k].firstChild.nodeValue;
				var Freezed_Date = baseResponse.getElementsByTagName("Freezed_Date")[k].firstChild.nodeValue;
...
//				mycurrent_row.appendChild(cell1);
				...
				var statement_name = document.createTextNode(statement_name);
				cell2.appendChild(statement_name);
...
function clrForm() {
	...
	document.getElementById("Statement_Name").value = "";
