...
function callServer(id,command)
{
    ...
    if(command=="Collection")
    {
        ...
        var url="../../../../../BankBalanceMontoringServlet.con?Command=Collection&BankId="+Bank+"&BranchId="+Branch+"&cmbOffice_code="+office+"&CollAcCode="+txtCol_accode1+"&BankAcNo="+cmbbank_accno+"&AccUnit="+unit+"&txtCB_Year="+Year+"&txtCB_Month="+Month+"&AccHead="+AccHead+"&txtverify_date="+Adate+"&OpeningBal="+OpeningBal;
        //alert(url);
        var req=getTransport();
        req.open("POST",url,true); 
        req.onreadystatechange=function()
