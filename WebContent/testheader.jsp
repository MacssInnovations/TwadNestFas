...
try
{
  ...
  System.out.println("from cookie testheader.jsp: "+flag_chk);
  //Getting  Remote host IP
  Remote_host=request.getRemoteHost();
  System.out.println("Remote Host in testheader:"+Remote_host);
...
try
{
	if(profile.equalsIgnoreCase("twad")) {
	    ...
	    String userid=(String)session.getAttribute("UserId");
            ...
            System.out.println("this is for checking user role like ho,su:");
	...
	//	roles.add(ii);
		...
		if (results.getInt("Role_Id") == 677)
			session.setAttribute("FAS_TDA_HO_ROLE","YES");
		
		if (results.getInt("Role_Id") == 194)
			session.setAttribute("FAS_TPA_AUDIT_ROLE","YES");
...
System.out.println("comes;;;");
	String MajorSystemDesc="select Major_System_Short_Desc,Major_System_Id from SEC_MST_MAJOR_SYSTEMS where Major_System_Id in( select distinct(Major_System_Id) from SEC_MST_ROLE_MENUS where Role_Id in("+temp+")  and Major_System_Id not in ('HRS','DBD','HRM'))";
	System.out.println("MajorSystemDesc;;;"+MajorSystemDesc);
