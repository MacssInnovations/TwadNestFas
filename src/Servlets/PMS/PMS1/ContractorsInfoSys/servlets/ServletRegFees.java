...
//                    ":" + strsid.trim();
    ...
    public void doGet(HttpServletRequest request,
        ...
        if (strCommand.equalsIgnoreCase("Add")) {
         ...
         ps=connection.prepareStatement(query);
          ...
          while(results.next())
          {
            ...
            String sql =
                "insert into PMS_MST_CON_REGN_FEES(Office_Level_Id,Regn_Class_Id,Entire_State,Regn_Fees,Date_effective_from,Last_Updated_Date,Last_Updated_By)values(?,?,?,?,?,?,?)";
      ...
      System.out.println("after");*/
            ...
            try {
                ...
                String sql =
                    "select * from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
 ...
 xml + "<officeid>" + rs.getString("Office_Level_Id") +
   ...
   rs.getString("Entire_State") + "</state>";
                    xml = xml + "</value>";
                } else {
                    xml = xml + "<flag>failure</flag>";
                }
            ...
            try {
                ...
                ps =
  connection.prepareStatement("select * from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?");
                ps.setString(1, strOffice);
                ps.setInt(2, cid3);
                ps.setString(3, strState);
                results = ps.executeQuery();
 ...
 xml + "<flag>success</flag><officeid>" + results.getString("Office_Level_Id") +
   ...
   "</classid><state>" + results.getString("Entire_State") + "</state><fees>" +
   results.getInt("Regn_Fees") + "</fees><date>" + StrDate1 + "</date>";
                } else {
                    xml = xml + "<flag>failure</flag>";
                }
                results.close();
            ...
            try {
                System.out.println("inside try catch");
                String sql =
                    "update PMS_MST_CON_REGN_FEES set Regn_Fees=?,Date_effective_from=?,Last_Updated_Date=?,Last_Upated_By=? where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
                //ps=connection.prepareStatement(sql);
            ...
            try {
                String sql =
                    "delete from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
                ps = connection.prepareStatement(sql);
                ps.setString(1, strOffice);
                ps.setInt(2, cid5);
                ps.setString(3, strState);
