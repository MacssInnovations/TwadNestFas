...
public class NonStandardTest_Servlet extends HttpServlet {
...
//                    ":" + strsid.trim();
       ...
       catch(Exception e)
       {
        ...
        if (strCommand.equals("Get")) {
            ...
            try {
                ...
                results2 =
                        statement.executeQuery("select * from WQS_MST_NONSTD_TEST order by NON_STD_CODE");
                System.out.println("aft res");
                try {
                    xml = xml + "<flag>success</flag>";
                    while (results2.next()) {

                        String StandardCode1 =
                            results2.getString("NON_STD_CODE");
                        String SampleCode1 =
 ...
 xml + "<PurposeCode>" + PurposeCode1 + "</PurposeCode>";
            ...
            try {
                PreparedStatement pstmt =
                    connection.prepareStatement("update WQS_MST_NONSTD_TEST set ELEMENT_SYMBOL=?,TEST_NAME=?,TEST_TYPE=?,CPHE_STD=?,BIS_STD=?,WHO_STD=?,PRAC_STD=?,PRINT_PRIORITY=? where NON_STD_CODE=? and SAMPLE_TYPE_CODE=? and TEST_PURPOSE_CODE=?");
            ...
            try {
                PreparedStatement pstmt =
                    connection.prepareStatement("delete from WQS_MST_NONSTD_TEST where NON_STD_CODE=? and SAMPLE_TYPE_CODE=? and TEST_PURPOSE_CODE=?");
                System.out.println(pstmt);
 ...
 xml + "<StandardCode>" + StandardCode + "</StandardCode>";
            ...
            try {
                PreparedStatement pstmt =
                    connection.prepareStatement("insert into WQS_MST_NONSTD_TEST(NON_STD_CODE,SAMPLE_TYPE_CODE,TEST_PURPOSE_CODE,ELEMENT_SYMBOL,TEST_NAME,TEST_TYPE,CPHE_STD,BIS_STD,WHO_STD,PRAC_STD,PRINT_PRIORITY)values(?,?,?,?,?,?,?,?,?,?,?)");
                pstmt.setString(1, StandardCode);
                pstmt.setString(2, SampleCode);
                pstmt.setString(3, PurposeCode);
