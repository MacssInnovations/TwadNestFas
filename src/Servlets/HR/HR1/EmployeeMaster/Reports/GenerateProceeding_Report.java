...
public class GenerateProceeding_Report extends HttpServlet {
 ...
 xml + "<flag>success</flag><eid>" + eid + "</eid><ename>" +
   ...
   "</offid>";
                                ...
                                try {
                                    ...
                                    ps =
  con.prepareStatement("select OFFICE_ID,EMPLOYEE_ID,RELIEVAL_SLNO,DATE_OF_RELIEVAL ,RELIEVAL_FN_AN, RELIEVAL_REASON_ID, REMARKS,PROCESS_FLOW_STATUS_ID from HRM_EMP_RELIEVAL_DETAILS where OFFICE_ID=? and EMPLOYEE_ID=?");
                                    ps.setInt(1, offid);
                                    ps.setInt(2, eid);
 ...
 xml + "<post_oid>" + rs.getInt("OFFICE_TO_POSTED_ID") +
   ...
   "</t_pr_no><dep_id>" + rs.getString("DEPT_ID") + "</dep_id>";
                                            }
                                            int ofid = 0;
                                            String depid = "";
                                            ofid =
...
///*   remove the SR Controlling office and moved it to log table
                             ...
                             try
                              {
                                   ...
                                   if(Proc_Status.equalsIgnoreCase("FR"))
                                   {
                                    ...
                                    /*   psorg=con.prepareStatement("insert into SEC_MST_USER_ROLES(EMPLOYEE_ID,ROLE_ID,UPDATED_BY_USER_ID,UPDATED_DATE,LIST_SEQ_NO) values(?,?,?,?,?)");
                                       ...
                                       psorg.setInt(5,999);
                                       psorg.executeUpdate();
                                       System.out.println("step 10");                                       /* End of the comment:   remove the SR Controlling office and moved it to log table  */
                    ...
                    try
                     {
                          ...
                          if(Proc_Status.equalsIgnoreCase("FR"))
                          {
                            ...
                            if(rs.next()) {
                              String rid=rs.getString("RELIEVAL_REASON_ID");
                              if(!(rid.equalsIgnoreCase("SUS")||rid.equalsIgnoreCase("VRS") || rid.equalsIgnoreCase("DIS") || rid.equalsIgnoreCase("SAN")) ) {
                                  String sql1="";
                                  if(rid.equalsIgnoreCase("TRN")) {
                                      sql1="delete from HRM_EMP_RELIEVAL_TRN where OFFICE_ID=? and RELIEVAL_SLNO=?";
                                  }
    ...
    public void doGet(HttpServletRequest request,
        ...
        int eid = 0;
        //System.out.println(xml);
        int txtRel_SLNO = 0, txtOffId = 0, txtEmployeeid = 0;
        String sub = null, ref = null, addlParaOne = null, addlParaTwo = null;
 ...
 DriverManager.getConnection(ConnectionString, strdbusername.trim(),
        ...
        try {
            psnew =
con.prepareStatement("select PROCESS_FLOW_STATUS_ID from HRM_EMP_RELIEVAL_DETAILS where OFFICE_ID=? and EMPLOYEE_ID=? and RELIEVAL_SLNO=?");
            System.out.println("select PROCESS_FLOW_STATUS_ID from HRM_EMP_RELIEVAL_DETAILS where OFFICE_ID=" +
                               txtOffId + " and EMPLOYEE_ID=" + txtEmployeeid +
                               " and RELIEVAL_SLNO=" + txtRel_SLNO);
            psnew.setInt(1, txtOffId);
            psnew.setInt(2, txtEmployeeid);
            psnew.setInt(3, txtRel_SLNO);
            rsnew = psnew.executeQuery();
            if (rsnew.next()) {
                Proc_Status = rsnew.getString("PROCESS_FLOW_STATUS_ID");
            }
        } catch (Exception e) {
            System.out.println("Error in retrieving process flow status id" +
                               e);
        }
        ...
        try {
            ...
            if (Proc_Status.equalsIgnoreCase("CR") ||
                Proc_Status.equalsIgnoreCase("MD")) {
                reportFile =
                        new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Relieval_Proceeding_Report.jasper"));
            } else {
                reportFile =
                        new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Relieval_Proceeding_Report_FairCopy.jasper"));
            }
