 ...
 */
public class ListOfEmp_YearExp_Office extends HttpServlet {
     ...
     */
    public ListOfEmp_YearExp_Office() {
        super();
    ...
    protected void doGet(HttpServletRequest request,
        ...
        if (option.equalsIgnoreCase("Desig")) {
            ...
            int officeId =
                Integer.parseInt(request.getParameter("officeselected"));
            System.out.println("designation selected.." + des);
            String office = request.getParameter("allofficeid");
            reportFile =
                    new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Listof_Employees_Desig_office.jasper"));
            map.put("desig", des);
            ...
            try {
                ps =
  connection.prepareStatement("select designation_id,cadre_name from ( select designation_id,cadre_id from hrm_mst_designations where cadre_id=? ) a left outer join ( select cadre_id as cad_id,cadre_name from hrm_mst_cadre) b on a.cadre_id=b.cad_id");
                ps.setInt(1, cad);
                rs1 = ps.executeQuery();
                int j = 0;
                while (rs1.next()) {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (option.equalsIgnoreCase("Desig")) {
            int des = Integer.parseInt(request.getParameter("cmbDesignation"));
            String officeId = request.getParameter("officeselected");
            String office = request.getParameter("allofficeid");

            System.out.println("designation selected.." + des);
            System.out.println("Office selected1.." + office);
            if (office.equalsIgnoreCase("all")) {
                reportFile =
                        new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Listof_Employees_Desig_headoffice.jasper"));
                map.put("desig", des);
  ...
  connection.prepareStatement("select designation_id,cadre_name from ( select designation_id,cadre_id from hrm_mst_designations where cadre_id=? ) a left outer join ( select cadre_id as cad_id,cadre_name from hrm_mst_cadre) b on a.cadre_id=b.cad_id");
            ...
            if (office.equalsIgnoreCase("all")) {
                reportFile =
                        new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Listof_Employees_Cadreheadoffice.jasper"));
                System.out.println("cadre name.." + cad_name);
                map.put("cadre", cad);
                map.put("name1", cad_name);
            } else {

                reportFile =
                        new File(getServletContext().getRealPath("/WEB-INF/ReportSrc/Listof_Employees_Cadre_office.jasper"));
                System.out.println("cadre name.." + cad_name);
                map.put("cadre", cad);
                map.put("name1", cad_name);
                map.put("officeid", office);
