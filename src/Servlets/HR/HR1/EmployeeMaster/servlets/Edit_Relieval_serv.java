...
public class Edit_Relieval_serv extends HttpServlet {
 ...
 "<response><command>sessionout</command><flag>sessionout</flag></response>";
        ...
        System.out.println("comm----------------------->0000000000" +
                           strCommand);
        try {
 ...
 xml + "<flag>success</flag><eid>" + eid + "</eid><ename>" +
   ...
   "</offid>";
                                ...
                                try {
                                    ...
                                    ps =
  con.prepareStatement("select OFFICE_ID,EMPLOYEE_ID,RELIEVAL_SLNO,DATE_OF_RELIEVAL ,RELIEVAL_FN_AN, RELIEVAL_REASON_ID, REMARKS,PROCESS_FLOW_STATUS_ID,PROCEEDING_NO,ORDER_NO,ORDER_DATE from HRM_EMP_RELIEVAL_DETAILS where OFFICE_ID=? and EMPLOYEE_ID=? and (PROCESS_FLOW_STATUS_ID='CR' or PROCESS_FLOW_STATUS_ID='MD')");
                                    ps.setInt(1, offid);
                                    ps.setInt(2, eid);
 ...
 xml + "<proceed_no>" + rs.getString("PROCEEDING_NO") +
   "</proceed_no><ord_no>" + rs.getString("ORDER_NO") + "</ord_no><ord_date>" +
   rs.getDate("ORDER_DATE") + "</ord_date>";
...
con.prepareStatement("delete from HRM_EMP_CURRENT_WING where EMPLOYEE_ID=? ");
  ...
  con.prepareStatement("delete from SEC_MST_USER_ROLES where EMPLOYEE_ID=? ");
                            ...
                            ps =
  con.prepareStatement("delete from sec_mst_user_roles where employee_id=? and  role_id in (select role_id from sec_mst_roles where role_category is null or role_category='C')");
                            ps.setInt(1, txtEmployeeid);
                            ps.executeUpdate();
