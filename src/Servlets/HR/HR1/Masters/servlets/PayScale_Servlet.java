    ...
    public void doGet(HttpServletRequest request,
        ...
        if (strCommand.equalsIgnoreCase("Delete")) {
            ...
            try {
                ...
                ps.executeUpdate();
                xml =
 xml + "<flag>success</flag><PayScaleId>" + strPayScaleId + "</PayScaleId>";
                ps.close();
            } catch (Exception e) {
            ...
            String sql =
                "update HRM_MST_PAYSCALES set PAY_SCALE=? where PAY_SCALE_ID=?";
            try {
                ps = connection.prepareStatement(sql);
            ...
            String sql =
                "insert into HRM_MST_PAYSCALES(PAY_SCALE_ID,PAY_SCALE) values(?,?)";
            try {
                ps = connection.prepareStatement(sql);
                ps.setString(1, strPayScaleId);
            ...
            try {
                String sql =
                    "SELECT * FROM HRM_MST_PAYSCALES WHERE PAY_SCALE_ID=? AND PAY_SCALE=?";
                try {
                    ps = connection.prepareStatement(sql);
                    ps.setString(1, strPayScaleId);
                    ps.setString(2, strPayScale);
