...
//                    ":" + strsid.trim();
        ...
        try {
            ...
            if (res.next()) {
                ...
                xml =
 "<response><flag>success</flag><reportdate>" + DateToBeDisplayed +
   "</reportdate><issuepriority>" + res.getString("issue_priority") +
   "</issuepriority><majorsystem>" + res.getString("major_system_id") +
   "</majorsystem><majorsystemdesc>" + res.getString("major_system_desc") +
   "</majorsystemdesc><minorsystemid>" + res.getString("minor_system_id") +
   "</minorsystemid><minorsystem>" + res.getString("minor_system_desc") +
   "</minorsystem><issuetitle>" + res.getString("issue_title") +
   "</issuetitle><issuedesc>" + res.getString("issue_desc") +
   "</issuedesc><issuestatus>" + res.getString("issue_status") +
   "</issuestatus><userid>" + res.getString("REPORTED_BY_USER_ID") +
   "</userid><solution>" + res.getString("Issue_solution") + "</solution>";
                int officeid = res.getInt("office_id");
                System.out.println("office id:" + officeid);
                String userid = res.getString("REPORTED_BY_USER_ID");
                String officename = "", officename1 = "", empname =
                    "", username = "";
                int Emp_Id = 0;
                String User = "";
                try {
  ...
  connection.prepareStatement("select employee_id from sec_mst_users where user_id=?");
                        ...
                        ps =
  connection.prepareStatement("select employee_name||employee_name||case when employee_initial is null then ' ' else '.'||employee_initial end as employee_name from hrm_mst_employees where employee_id=?");
                        ps.setInt(1, Emp_Id);
                        res = ps.executeQuery();
                        if (res.next()) {
                        ...
                        ps =
  connection.prepareStatement("select a.office_name from com_mst_offices a where a.office_id=?");
                    ...
                    ps =
  connection.prepareStatement("select user_id from SEC_MST_OTHER_USERS_PROFILE where user_id=?");
                    ps.setString(1, userid);
                    res = ps.executeQuery();
                    if (res.next()) {
                    ...
                    ps =
  connection.prepareStatement("select user_name from SEC_MST_OTHER_USERS_PROFILE where user_id=?");
                    ps.setString(1, User);
                    res = ps.executeQuery();
                    if (res.next()) {
                    ...
                    ps =
  connection.prepareStatement("select a.office_name from SEC_MST_OTHER_USERS_PROFILE a where a.user_id=?");
                    ps.setString(1, User);
                    res = ps.executeQuery();
                    if (res.next()) {
                        officename = res.getString("office_name");
                    ...
                    System.out.println("Error in Office Name:" + e);
                }
                if (empname != null) {

                    xml =
 xml + "<empname>" + empname + "</empname><officename>" + officename +
   "</officename></response>";
                } else if (officeid == 0) {
                    xml =
 xml + "<empname>" + username + "</empname><officename>" + officename +
   "</officename></response>";
                }
