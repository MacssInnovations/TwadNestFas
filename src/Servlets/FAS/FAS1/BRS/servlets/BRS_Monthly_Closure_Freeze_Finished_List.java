...
/**
 * Servlet implementation class BRS_Monthly_Closure_Freeze_Finished_List
 */
public class BRS_Monthly_Closure_Freeze_Finished_List extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			reportFile = new File(getServletContext().getRealPath(
					"/org/FAS/FAS1/BRS/jaspers/BRS_Monthly_Closure_Freeze_Finished_List.jasper"));
			...
			if (rtype.equalsIgnoreCase("HTML")) {
				...
				response.setHeader("Content-Disposition",
						"attachment;filename=\"BRS_Monthly_Closure_Freeze_Finished_List.pdf\"");
		  ...
		  else  if(cmd.equalsIgnoreCase("unitwise"))
		  {
		...
		try {
			...
			reportFile = new File(getServletContext().getRealPath("/org/FAS/FAS1/BRS/jaspers/BRS_Monthly_Closure_Freeze_Finished_List_unitwise.jasper"));
			if (!reportFile.exists())
			...
			System.out.println(rtype);
			 if (rtype.equalsIgnoreCase("PDF")) {
				...
				response.setContentLength(buf.length);
				response.setHeader("Content-Disposition",
						"attachment;filename=\"Monthly_Closure_Freeze_Finished_List_unitwise.pdf\"");
				OutputStream out = response.getOutputStream();
				out.write(buf, 0, buf.length);
				out.close();
			} 
