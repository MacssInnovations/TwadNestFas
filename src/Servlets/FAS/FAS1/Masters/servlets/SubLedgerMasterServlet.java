...
public class SubLedgerMasterServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request,
        ...
        try {
            SubLedgerTypeCode =
                    Integer.parseInt(request.getParameter("SubLedgerTypeCode"));
        ...
        if (strCommand.equalsIgnoreCase("Delete")) {
            ...
            try {
                PreparedStatement pstmt =
                    connection.prepareStatement("delete from COM_MST_SL_TYPES where sub_ledger_type_code=?");
                System.out.println(pstmt);
                pstmt.setInt(1, SubLedgerTypeCode);
                pstmt.executeUpdate();
                xml =
 xml + "<flag>success</flag><SubLedgerTypeCode>" + SubLedgerTypeCode +
   "</SubLedgerTypeCode>";
                pstmt.close();
            } catch (SQLException e) {
            ...
            try {
                CallableStatement pstmt =
                    connection.prepareCall("{call FAS_SUBLEDGERPROCEDURE(?,?,?,?)}");
                System.out.println(pstmt);
                pstmt.setInt(1, SubLedgerTypeCode);
            ...
            try {
                ...
                xml =
 xml + "<SubLedgerTypeCode>" + SubLedgerCode + "</SubLedgerTypeCode><SubLedgerTypeDesc>" +
   SubLedgerTypeDesc + "</SubLedgerTypeDesc>";
                pstmt.close();
            } catch (SQLException e) {
            ...
            try {
                ...
                results2 =
                        statement.executeQuery("select sub_ledger_type_code,sub_ledger_type_desc from com_mst_sl_types order by sub_ledger_type_code");
                System.out.println("aft res");
                try {
                    xml = xml + "<flag>success</flag>";
                    while (results2.next()) {

                        int SubLedgerTypeCode1 =
                            results2.getInt("sub_ledger_type_code");
                        String SubLedgerTypeDesc1 =
                            results2.getString("sub_ledger_type_desc");


                        //<PayName>" + PayName + "</PayName>;
                        xml =
 xml + "<SubLedgerTypeCode>" + SubLedgerTypeCode1 +
   "</SubLedgerTypeCode><SubLedgerTypeDesc>" + SubLedgerTypeDesc1 +
   "</SubLedgerTypeDesc>";
                    }
                } catch (Exception aee) {
