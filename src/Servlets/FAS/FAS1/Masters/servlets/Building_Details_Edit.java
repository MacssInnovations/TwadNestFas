...
public class Building_Details_Edit extends HttpServlet 
{
    /**
	 * 
	 */
...
/* cmbAcc_UnitCode cmbOffice_code txtDate cmbFinyr txtAsset txtBuilding cmbOwner txtSurvey txtDoorNo txtVillage txtBuidConsYear txtBuildType txtFloors txtFoundationType txtStructuralElements txtCivilCost txtElectricalCost txtExternalServCost txtAdditionCost txtBookValue cmbAcHeadCode txtRemarks cmdAddBuild cmdUpdateBuild cmdDeleteBuild cmdCancel cmdListBuild cmdClearBuild    */
        ...
        /**************************************************************************************************
         * 									Building Details - Update
         **************************************************************************************************/
        
        try
        {
            PreparedStatement ps = connection.prepareStatement("UPDATE fas_building_details  " +
																"SET trans_date = to_date(?,'dd-mm-yyyy'), " +
																"  building_name = ?, " +
																"  owner_code = ?, " +
																"  survey_no = ?, " +
																"  door_no = ?, " +
            ...
            ps.setString(2,building_name);
            ps.setInt(3,owner_code);
            ps.setInt(4,survey_no);
            ps.setString(5,door_no);
            ps.setString(6,village);
            ps.setInt(7,year_of_construction);
            ps.setString(8,type_of_building);
            ps.setInt(9,no_of_floors);
            ps.setString(10,type_of_foundation);
            ps.setString(11,structured_bldg_elements);
            ps.setInt(12,total_civil_cost);
            ps.setInt(13,total_electrical_cost);
            ps.setInt(14,total_external_cost);
            ps.setInt(15,total_additions_cost);
            ps.setInt(16,total_book_value);
            ps.setInt(17,account_head_code);
            ps.setString(18,remarks);
            ps.setString(19,userid);
            
            ps.setInt(20,accounting_unit_id);
            ps.setInt(21,accounting_unit_office_id);
            ...
            msg="Building Details Updated Successfully!";
            msg=msg+"<br><br>";
	    ...
	    try
	    {
	            ...
	            PreparedStatement ps1 = connection.prepareStatement("DELETE FROM FAS_BUILDING_FLOOR_DETAILS " +
																	"WHERE ACCOUNTING_UNIT_ID = ? " +
																	"  AND ACCOUNTING_UNIT_OFFICE_ID = ? " +
																	"  AND FINANCIAL_YEAR = ? " +
																	"  AND ASSET_CODE = ? " +
		        ...
		        try
		        {
		            PreparedStatement ps = connection.prepareStatement("INSERT INTO FAS_BUILDING_FLOOR_DETAILS " +
																		"( " +
																		"  ACCOUNTING_UNIT_ID, " +
		            ...
		            {			           
			            ...
			            ps.setString(4,financial_year);
			            ps.setInt(5,asset_code);
			            ps.setInt(6,Integer.parseInt(flr_no[i]));
			            ps.setInt(7,Integer.parseInt(flr_year[i]));
			            ps.setInt(8,Integer.parseInt(flr_height[i]));
			            ps.setInt(9,Integer.parseInt(flr_area[i]));
			            ps.setInt(10,Integer.parseInt(flr_civilCost[i]));
			            ps.setInt(11,Integer.parseInt(flr_electricalCost[i]));
			            ps.setInt(12,Integer.parseInt(flr_externalCost[i]));
			            ps.setInt(13,Integer.parseInt(flr_addCost[i]));
			            ps.setInt(14,Integer.parseInt(flr_bookVal[i]));
			            ps.setInt(15,Integer.parseInt(flr_BPNo[i]));
			            ps.setInt(16,Integer.parseInt(flr_AcHead[i]));
			            ps.setString(17,flr_Remarks[i]);
		            ...
		            msg+="Floor Details Updated Successfully!";
		            msg=msg+"<br><br>";
	    ...
	    {
	    	System.out.println("Floor Not Updated - Exception deleting existing Floor(s) ==> " + e);
            try
            {connection.rollback();}catch(Exception e1){System.out.println("Exception in Catch"+e1);}
            msg+="<br><br>Office Not Updated - Exception deleting existing Floor(s) ==> "+e+"<br>";
            sendMessage(response,msg,"ok");
	    }
	    ...
	    try
	    {
	            ...
	            PreparedStatement ps1 = connection.prepareStatement("DELETE FROM FAS_FLOOR_OFFICE_DETAILS " +
																	"WHERE ACCOUNTING_UNIT_ID = ? " +
																	"  AND ACCOUNTING_UNIT_OFFICE_ID = ? " +
																	"  AND FINANCIAL_YEAR = ? " +
																	"  AND ASSET_CODE = ? " +
		        ...
		        try
		        {
		            PreparedStatement ps = connection.prepareStatement("INSERT INTO FAS_FLOOR_OFFICE_DETAILS " +
																		"( " +
																		"  ACCOUNTING_UNIT_ID, " +
																		"  ACCOUNTING_UNIT_OFFICE_ID, " +
																		"  TRANS_DATE, " +
		            ...
		            for(int i=0; i<off_flrNo.length; i++)
		            {
			            ...
			            ps.setString(4,financial_year);
			            ps.setInt(5,asset_code);
		            	ps.setInt(6,Integer.parseInt(off_flrNo[i]));
			            ps.setString(7,off_TN[i]);
			            ...
			            ps.setInt(8,TWAD_Office_id);
			            ps.setString(9,NonTWAD_Office_name);
			            ps.setString(10,off_Remarks[i]);
		            ...
		            try
		            {connection.rollback();}catch(Exception e1){System.out.println("Exception in Catch"+e1);}
		            msg+="<br><br>Exception inserting Updated Office(s) ==> "+e+"<br>";
		            sendMessage(response,msg,"ok");
		        }
	    }catch(Exception e)
	    {
	    	System.out.println("Office Not Updated - Exception deleting existing Office(s) ==> " + e);
            try
            {connection.rollback();}catch(Exception e1){System.out.println("Exception in Catch"+e1);}
            msg+="<br><br>Office Not Updated - Exception deleting existing Office(s) ==> "+e+"<br>";
            sendMessage(response,msg,"ok");
	    }
    ...
    public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException 
    {
         ...
         **************************************************************************************************/
	        ...
	        try
	        {
	            PreparedStatement ps1 = connection.prepareStatement("UPDATE fas_building_details " +
	            													"SET STATUS = 'N' " +
											                		"WHERE ACCOUNTING_UNIT_ID = ? " +
											                		"  AND ACCOUNTING_UNIT_OFFICE_ID = ? " +
											                		"  AND FINANCIAL_YEAR = ? " +
	            ...
	            xml += "<ACCOUNTING_UNIT_ID>" + accounting_unit_id + "</ACCOUNTING_UNIT_ID>";
	            xml += "<ACCOUNTING_UNIT_OFFICE_ID>" + accounting_unit_office_id + "</ACCOUNTING_UNIT_OFFICE_ID>";
	            xml += "<FINANCIAL_YEAR>" + financial_year + "</FINANCIAL_YEAR>";
	            xml += "<ASSET_CODE>" + asset_code + "</ASSET_CODE>";
	
	            ps1.close();
	        
	            System.out.println("Building Details Deleted Successfully!");
	            
	        }
	        catch(SQLException e) {
	            System.out.println("SQLException executing 'Delete Building Query' ==> "+e);
	    ...
	    /**************************************************************************************************
	     * 									Floor Details - Delete
	     **************************************************************************************************/
		    
		    try
		    {
		        PreparedStatement ps2 = connection.prepareStatement("UPDATE FAS_BUILDING_FLOOR_DETAILS " +
		        													"SET STATUS = 'N' " +
																	"WHERE ACCOUNTING_UNIT_ID = ? " +
																	"  AND ACCOUNTING_UNIT_OFFICE_ID = ? " +
																	"  AND FINANCIAL_YEAR = ? " +
		            ...
		            System.out.println("Floor Details Deleted Successfully!");
		            
		    }
		    catch(SQLException e) {
		        System.out.println("SQLException executing 'Delete floor Query' ==> "+e);
		        xml = "<respone><flag>failure</flag>";
		    }
	     ...
	     **************************************************************************************************/
		    ...
		    try
		    {
		        PreparedStatement ps3 = connection.prepareStatement("UPDATE FAS_FLOOR_OFFICE_DETAILS " +
		        													"SET STATUS = 'N' " +
																	"WHERE ACCOUNTING_UNIT_ID = ? " +
																	"  AND ACCOUNTING_UNIT_OFFICE_ID = ? " +
																	"  AND FINANCIAL_YEAR = ? " +
		        ...
		        System.out.println("Office Details Deleted Successfully!");
		    }
		    catch(SQLException e) {
		        System.out.println("SQLException executing 'Delete Office Query' ==> "+e);
		        xml = "<respone><flag>failure</flag>";
		    }
