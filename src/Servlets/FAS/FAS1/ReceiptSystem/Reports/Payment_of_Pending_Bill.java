...
/**
 * Servlet implementation class Payment_of_Pending_Bill
 */
public class Payment_of_Pending_Bill extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Payment_of_Pending_Bill() {
        super();
	...
	System.out.println("txtCB_Month:::"+txtCB_Month);
		...
		System.out.println("cboOffice_code:::::::"+cboOffice_code);
		 ...
		 String monthInWords="";
			...
			String title="";
			if(hdid.equalsIgnoreCase("pay")){
					title="Payment Of Pending Bills Against Liability For "+monthInWords+" "+cboCashBook_Year;
			}else if(hdid.equalsIgnoreCase("jrnl")){
				title="List of LJV and Payment Details for  "+monthInWords+" "+cboCashBook_Year;
			}
			/*@NK on 29july2019*/
			else if(hdid.equalsIgnoreCase("rcpt"))
			{
				title="List of Receipt and Payment Details for "+monthInWords+" "+cboCashBook_Year;
			}
			/*@NK on 29july2019*/
			else if(hdid.equalsIgnoreCase("jrnlandPay"))
			{
				title="JOURNAL VOUCHERS AND PAYMENT MADE "+monthInWords+" "+cboCashBook_Year;
			}
			...
			try {
				...
				if(cboAcc_UnitCode==0)
				
				{
					System.out.println("calling servlet All Units...");
					if(hdid.equalsIgnoreCase("pay")){
						reportFile = new File(getServletContext().getRealPath(
								"/org/FAS/FAS1/Reports/ReceiptSystem/jasper/payment_of_pending_bill_Report.jasper"));
						System.out.println("reportFile===>"+reportFile);
						
				}else if(hdid.equalsIgnoreCase("jrnl")){
					reportFile = new File(getServletContext().getRealPath(
							"/org/FAS/FAS1/Reports/ReceiptSystem/jasper/payment_of_pending_bill_Report1.jasper"));
					System.out.println("reportFile===>"+reportFile);
				}
					/*@NK on 29july2019*/
				else if(hdid.equalsIgnoreCase("rcpt")){
					reportFile = new File(getServletContext().getRealPath(
							"/org/FAS/FAS1/Reports/ReceiptSystem/jasper/payment_of_Receipt_bill_Report.jasper"));
					System.out.println("reportFile===>"+reportFile);
				}
					/*@NK on 29july2019*/
				else if(hdid.equalsIgnoreCase("jrnlandPay")){
					reportFile = new File(getServletContext().getRealPath(
							"/org/FAS/FAS1/Reports/ReceiptSystem/jasper/Jrnl_and_Payment_Details.jasper"));
					System.out.println("reportFile===>"+reportFile);
				}
				}
				...
				map.put("cashbookyear", cboCashBook_Year);
			...
			//	map.put(title, value)
				...
				if (rtype.equalsIgnoreCase("HTML")) {
					...
					response.setHeader("Content-Disposition",
							"attachment;filename=\"Journal and Payment Details.html\"");
					...
					response.setHeader("Content-Disposition",
							"attachment;filename=\"Journal and Payment Details.pdf\"");
...
//                                             Boolean.TRUE);
					 ...
					 /*@Nk on 30072019*/
					 String sheetName=jasperPrint.getName();
					 exporterXLS.setParameter(JRXlsExporterParameter.SHEET_NAMES, new String[]{sheetName});
					 /*@Nk on 30072019*/
					 exporterXLS.exportReport();
					 byte[] buf1 = xlsReport.toByteArray();
					 ServletOutputStream s1 = response.getOutputStream();
					 //s1 = response.getOutputStream();
...
//                    ouputStream.close();
					...
					response.setHeader("Content-Disposition",
							"attachment;filename=\"Journal and Payment Details.txt\"");
