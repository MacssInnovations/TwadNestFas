...
public class Receipt_SL_FinalHead_GJV extends HttpServlet {
...
//                    ":" + strsid.trim();
                        	...
                        	if(cmbOffice_code==offid)
                        	{
                        		...
                        		//System.out.println("cmbOffice_code:"+cmbOffice_code);
		                        		 if (rs.getString("SUB_LEDGER_TYPE_APPLICABLE").equalsIgnoreCase("Y")) {
		                                    int sl_cnt=0;
		                        			 ps =con.prepareStatement("select SUB_LEDGER_TYPE_CODE from FAS_APPLICABLE_SL_TYPE where ACCOUNT_HEAD_CODE=? and STATUS='Y'");//STATUS Added 05-07-19
		                                     ps.setInt(1, txtAcc_HeadCode);
		                                     rs = ps.executeQuery();
		                                     while (rs.next()) {
		                                        sl_cnt++;
			 ...
			 xml + "<flag>success</flag><hid>" + txtAcc_HeadCode + "</hid><hdesc>" +
			   ...
			   "</sl_man>";
                       ...
                       if(h_count==0)
                       {
		                        ...
		                        if (rs.getString("SUB_LEDGER_TYPE_APPLICABLE").equalsIgnoreCase("Y")) {
		                            int sl_cnt=0;
		                        	ps =con.prepareStatement("select SUB_LEDGER_TYPE_CODE from FAS_APPLICABLE_SL_TYPE where ACCOUNT_HEAD_CODE=? and STATUS='Y'");
		                            ps.setInt(1, txtAcc_HeadCode);
		                            rs = ps.executeQuery();
		                            while (rs.next()) {
		                                sl_cnt++;
		                            ...
		                            if(sl_cnt==0) {
                                   	 System.out.println("Account Head Not Allowed. sl_cnt is zero!!!!!! ");
   		                              xml ="<response><command>" + strCommand + "</command><flag>failure</flag>";
                                    }
		                        }
                        }
 ...
 xml + "<cid>" + rs.getInt("ASSET_CODE") + "</cid><cname>" +
   rs.getString("ASSET_DESCRIPTION").trim() + "</cname>";
            ...
            //old MODIFIED BY SHERON ON 30_07_2024
 ...
 xml + "<state>" + rs.getString("OFFICE_STATUS_ID") + "</state>";
          ...
          //NEW MODIFIED BY SHERON ON 30_07_2024
...
//                    }
//                    ps =
//  con.prepareStatement("select ACT.ACCOUNTING_UNIT_ID as OFFICE_ID, ACT.ACCOUNTING_UNIT_OFFICE_ID  , ACT.accounting_unit_name  as office_name , OFF.OFFICE_STATUS_ID from COM_MST_OFFICES OFF LEFT JOIN FAS_MST_ACCT_UNITS ACT "
//  		+ "  		  ON ACT.ACCOUNTING_UNIT_OFFICE_ID = OFF.OFFICE_ID  where OFFICE_ID=? ");
//                    ps.setInt(1, addtional_field_value);
//                    rs = ps.executeQuery();
//                    System.out.println("inside offices 5");
//
