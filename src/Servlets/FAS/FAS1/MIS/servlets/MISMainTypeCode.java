 ...
 */
public class MISMainTypeCode extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (strCommand.equalsIgnoreCase("add")) {
			...
			try{
				...
				xml = mismasterservice.misMainTypeList(categoryCode);
			}catch (Exception e) {
			...
			try {   				
   				MISCategoryImpl add = new MISCategoryImpl();				
   				xml = add.editMainCode(mainCategoryId,mainTypeCode);   				
			} catch (SQLException e) {
				xml="<response><status>fail</status></response>";
				e.printStackTrace();
			}
