...
public class SJV_TB_UnFreezed_Offices extends HttpServlet {
        ...
        */
        ...
        try {
            ...
            try {
                if (RegionId == 101) {
                    rst =
 statement.executeQuery("select office_id from com_mst_offices where office_status_id not in ('CL','RD','NC')");
                    while (rst.next()) {
                        officeid = officeid + rst.getString("office_id") + ",";
                    }
                    ...
                    rst =
 statement.executeQuery("select office_id from com_mst_all_offices_view where region_office_id=" +
                        RegionId);
                    while (rst.next()) {
                        officeid = officeid + rst.getString("office_id") + ",";
                    }
                ...
                reportFile =
                        new File(getServletContext().getRealPath("/org/FAS/FAS1/Reports/ReceiptSystem/jasper/SJV_TB_UnFreezed_Offices.jasper"));
                ...
                map.put("monthvalue", monthInWords + " " + CashBookYear);
