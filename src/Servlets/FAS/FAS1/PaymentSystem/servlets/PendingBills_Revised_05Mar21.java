...
public class PendingBills_Revised_05Mar21 extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(strCommand.equalsIgnoreCase("load_journalAmt"))
        {
			...
			System.out.println(selvouDate);
			String selvouNo =request.getParameter("selvouNo");
			System.out.println("selvouNo>>>>"+selvouNo);
			String selsNo=request.getParameter("selsNo");
			System.out.println("selsNo>>>>"+selsNo);
			String selslType=request.getParameter("selslType");
			System.out.println("selslType>>>>"+selslType);
			String selslCode=request.getParameter("selslCode");
			System.out.println("selslCode>>>>"+selslCode);
			String selslAmt=request.getParameter("selslAmt");
			System.out.println("selslAmt>>>>"+selslAmt);
			try {
			...
			String sql="SELECT t.voucher_no,\r\n" + 
					...
					" AND t.payable_voucher_no     in (" + selvouNo +")" +
					" AND t.payable_voucher_date in ("+ selvouDate +")" +
					" AND t.payable_voucher_slno  in ("+ selsNo +")" +
					" AND t.sub_ledger_type_code in ("+ selslType +")" + 
					" AND t.sub_ledger_code  in  ("+ selslCode +")" +
					" AND t.amount      in ("+ selslAmt +")" ;
...
//						+ "		AND trs.MULTIPLE_PVRS!        ='Y' "
						...
						+ "    AND trs.CB_REF_NO            =0 "
						+ "    AND trs.CB_REF_DATE         IS NULL "
						+ "    AND mas.JOURNAL_TYPE_CODE   IN (53,54,61,62,66,79,75,110) "
						+ "    UNION ALL "
						+ "    SELECT trs.VOUCHER_NO, "
						+ "      trs.SL_NO, "
						+ "  mas.accounting_for_office_id as office_id, "
